...
РАЗМЕН ПО БЛОКИРОВКАМ НАЗВАН ЯВНО. Ревизия 0014 отдельно объясняет, почему в
ней нет ни одного переписывания строк («не держит долгую блокировку»). Эта
ревизия ту дисциплину НАРУШАЕТ и делает это осознанно: она выполняет UPDATE,
UPDATE ссылок расписаний, DELETE и `ALTER TABLE ... ADD CONSTRAINT UNIQUE`, а
последний на PostgreSQL строит уникальный индекс под `ACCESS EXCLUSIVE` — то
есть на время построения таблица `groups` недоступна и на чтение. Размен принят
по размеру таблицы: на сегодняшних объёмах это доли секунды, а альтернатива
(`CREATE UNIQUE INDEX CONCURRENTLY` + `ADD CONSTRAINT ... USING INDEX`) требует
ревизии БЕЗ транзакции и не может стоять в одной ревизии с чисткой дублей —
индекс строился бы по данным, которые ещё не схлопнуты. Если таблица вырастет
настолько, что окно блокировки станет заметным, разделение на две ревизии —
готовый следующий шаг, а не переоткрытие вопроса.
