...
⚠️ ГРАНИЦА СРОКА ЗАКРЫТА В ПОЛЬЗУ ИСТЕЧЕНИЯ, И С 2026-08-21 РЕВИЗИЯ ЕЁ
ДЕЙСТВИТЕЛЬНО ПРОВОДИТ, А НЕ ТОЛЬКО ОБЪЯВЛЯЕТ. Сравнение по сроку стоит в
предикате ветки П-о-2 (`_RESET_FREE_EXPIRY`) и оно СТРОГОЕ: строка, чей срок в
точности равен «момент исполнения + пробное окно», не трогается — переписыванием
она ничего не получила бы. Строгость не выбор вкуса: `subscription_is_live`
сравнивает СТРОГО (`app/application/billing/subscription_period.py`), и ревизия,
разошедшаяся с предикатом на строгости, дала бы человеку, чья секунда совпала с
миграцией, один ответ от миграции и противоположный от продукта.

⚠️ ДО РЕШЕНИЯ ВЛАДЕЛЬЦА ОТ 2026-08-21 ЭТОТ АБЗАЦ ОПИСЫВАЛ ГРАНИЦУ, КОТОРОЙ В КОДЕ
НЕ БЫЛО. Ветка П-о-2 переписывала срок БЕЗУСЛОВНО — предиката по сроку в ней не
стояло вовсе, и «граница» существовала только как следствие того, что платных
строк операция не касается. Расхождение названо здесь, а не затёрто, потому что
следующий читатель обязан знать, что абзац описывает исполняемое правило.
...
ТРИ ПОПУЛЯЦИИ И ЧТО С НИМИ ДЕЛАЕТСЯ (решение D-G):
  ...
  * П-о-2 — активная строка с БЕСПЛАТНЫМ тарифом: срок ПОДТЯГИВАЕТСЯ до
    `TRIAL_DAYS` от момента исполнения и НИКОГДА не укорачивается. Строка, чей
    срок уже дальше этого окна, не трогается вовсе. Решение владельца от
    2026-08-21 (CR-01, ветка `b-never-shorten`): строку с тарифом `free` и
    дальней датой заводили административная выдача и ветка первой вставки
    денежного пути, различить её от по-настоящему бесплатной после наката НЕЧЕМ —
    колонка тарифа снимается этой же ревизией, — а укороченный срок не
    восстанавливается ничем. Цена решения названа прямо: БУКВА D-G («пробный срок
    от даты миграции») выполняется не для всех, зато ни одна строка не теряет
...
СЛЕД ПО П-о-2 — ПОИМЁННЫЙ, А НЕ СЧЁТНЫЙ. Прежний срок КАЖДОЙ активной бесплатной
строки снимается ДО правки и уходит в журнал наката вместе с новым — по строке на
строку, и для подтянутых, и для оставленных как есть. Откат данных не возвращает,
и журнал — единственное, чем можно разобрать жалобу «мне сдвинули срок»; запись
«тронуто N строк» разобрать её не даёт.
...
# П-о-2 — активные бесплатные, СНИМОК ДО ПРАВКИ. Читает то, что операция ниже
# вот-вот перепишет, и делает это ДО неё: откат данных не возвращает, и
# единственная возможность разобрать жалобу «мне сдвинули срок» — журнал наката,
# снятый заранее. Счётная запись («тронуто N строк») жалобу разобрать не даёт: в
# ней нет ни идентификатора, ни прежней даты.
#
# Тот же запрос исполняется ВТОРОЙ раз ПОСЛЕ правки — чтобы в журнал ушла не
# вычисленная заново, а НАБЛЮДЁННАЯ новая дата. Повторять правило предиката на
# Python значило бы завести второе место, где оно записано, и разойтись с первым
# однажды молча.
...
#
# ⚠️ ПРЕДИКАТ ПО СРОКУ — РЕШЕНИЕ ВЛАДЕЛЬЦА ОТ 2026-08-21 (CR-01, ветка
# `b-never-shorten`), А НЕ ОФОРМЛЕНИЕ. Ревизия трактует тариф `free` как «человек
# ничего не оплачивал», но это СЛЕДСТВИЕ, а не факт схемы: строку с тарифом
# `free` и произвольной датой заводили две сущности — снятая административная
# функция назначения тарифа, принимавшая ЛЮБОЕ имя тарифа и ЛЮБУЮ дату, и ветка
# первой вставки денежного пути с запасным значением тарифа. Безусловная
# перезапись сожгла бы такой строке оплаченное (или подаренное) время необратимо
# и без следа, а различить её от по-настоящему бесплатной после наката НЕЧЕМ:
# колонка тарифа снимается этой же ревизией. Основание владельца записано его
# словами в `.planning/phases/05.1-edinaya-podpiska/05.1-11-SUMMARY.md`:
# необратимость дороже буквы — лишний день доступа стоит ничего, а укороченный
# срок после наката не восстановить.
#
# Решение D-G продолжает выполняться ПО СУЩЕСТВУ: тот, у кого срока меньше окна
# (включая всех, у кого он в прошлом), получает пробное окно от момента
# исполнения. Не выполняется его БУКВА — у строки с дальней датой срок остаётся
# прежним, и это названо ценой решения, а не упущением.
#
# ⚠️ СРАВНЕНИЕ СТРОГОЕ, И ЭТО ТОЖЕ РЕШЕНИЕ. Строка, чей срок в ТОЧНОСТИ равен
# окну, переписыванием ничего не получила бы; строгость совпадает со строгостью
# `subscription_is_live` в продукте, и миграция, разошедшаяся с предикатом на
# строгости, отвечала бы на вопрос о границе иначе, чем продукт. Нестрогая форма
# (`<=`) названа здесь ровно для того, чтобы объяснить, почему её тут нет;
# закреплено это утверждением по СИНТАКСИЧЕСКОМУ ДЕРЕВУ
# (`test_the_free_expiry_predicate_compares_strictly`), которое комментариев не
# видит, — греп по файлу краснел бы от этой самой строки.
...
def upgrade():
    ...
    # СНИМОК ДО ПРАВКИ. Снимается ДО `_RESET_FREE_EXPIRY`, и порядок здесь
    # несущий: после правки прежних дат в базе не остаётся, а откат их не
    # возвращает.
    before_rows = connection.execute(_SNAPSHOT_FREE_EXPIRY).fetchall()
    previous_expiry = {row[0]: row[1] for row in before_rows}

    result = connection.execute(
        _RESET_FREE_EXPIRY, {"trial_expires_at": trial_expires_at}
    )
    pulled_forward = result.rowcount if result.rowcount is not None else -1

    # СНИМОК ПОСЛЕ ПРАВКИ — чтобы в журнал ушла НАБЛЮДЁННАЯ новая дата, а не
    # вычисленная здесь заново по правилу, которое уже записано в предикате.
    after_expiry = {
        row[0]: row[1] for row in connection.execute(_SNAPSHOT_FREE_EXPIRY).fetchall()
    }

    # ПОИМЁННЫЙ СЛЕД: строка журнала на КАЖДУЮ бесплатную активную строку —
    # тронутую и оставленную. Молчание об оставленных неотличимо от того, что их
    # забыли, а счётная запись не даёт разобрать жалобу конкретного человека.
    for user_id, was in previous_expiry.items():
        now_is = after_expiry.get(user_id, was)
        logger.info(
            "0020: user %s free-plan subscription expires_at %s -> %s (%s)",
            user_id,
            was,
            now_is,
            "PULLED FORWARD" if now_is != was else "LEFT UNTOUCHED",
        )

    left_untouched = len(previous_expiry) - max(pulled_forward, 0)
    logger.info(
        "0020: %s free-plan active subscription row(s) pulled forward to %s (%s "
        "days from the moment this revision ran); %s row(s) LEFT UNTOUCHED because "
        "their expires_at is already farther than that window — this revision "
        "never shortens an expiry (owner decision of 2026-08-21, CR-01). Every row "
        "of both groups is listed above with its previous and its current date.",
        pulled_forward,
