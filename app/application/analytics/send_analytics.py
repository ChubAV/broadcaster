...
`fail`, оставляя `account_disconnected` не посчитанным нигде. Расхождение
такого рода не роняет ни один тест и не даёт пятисотки — оно просто печатает
пользователю два разных числа на один вопрос. Поэтому определение живёт ЗДЕСЬ,
а потребители его ВЫЗЫВАЮТ; ни один из них не владеет собственной копией.

РАСХОЖДЕНИЕ ПО ПОЛУНОЧИ ЗАКРЫТО В ОБЕ СТОРОНЫ. Границу суток обоим экранам
отдаёт ОДИН хелпер `local_day_start_utc`, и «сегодня» у дашборда и у фильтра
истории — буквально одно и то же значение. Скользящее окно из модуля при этом
не исчезло: оно осталось ровно там, где выбрано сознательно, — у API-сводки
истории (тридцать суток) и у админского «Обзора», чей вопрос операционный и
полуночи не имеет вовсе.
...
async def send_metrics(
    ...
    *,
    ...
    «Ошибки», и сумма двух плиток молча разошлась бы с плиткой «Отправок
    сегодня» — то есть неклассифицируемая запись исчезла бы из счёта ради
    ровных чисел (прохибиция P-04-01). Сторона отказа выбрана несимметрично по образцу
...
# а журналом владеет ЭТОТ модуль (D-35). Календарное окно МЕСЯЦА не собирается
# ни из суток читателя, ни из фильтров истории: `local_day_bounds` режет ровно
# сутки, а фильтр умеет только `today` / `7d` / `30d`. Поэтому окно живёт
# здесь, рядом с остальными запросами к `send_logs`, а не в модуле осей тарифа.
