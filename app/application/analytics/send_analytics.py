...
async def activity_heatmap(
    ...
    *,
    ...
    ОКНО СКОЛЬЗЯЩЕЕ (D-12), НО ЯКОРИТСЯ НА ЛОКАЛЬНУЮ ПОЛНОЧЬ ЧИТАТЕЛЯ. Ряд `i` —
    КАЛЕНДАРНЫЕ сутки `local_origin + i дней`, где `local_origin` есть полночь
    текущих локальных суток минус `days - 1` суток; последний ряд — сегодняшние
    сутки, ещё не полные. Календарной недели ПН-ВС макета здесь по-прежнему нет:
    окно едет посуточно вместе с читателем, а не стоит на понедельнике, который
    в понедельник утром показывал бы одну колонку данных и шесть пустых.
    Подписи рядов следуют окну — `day_labels[i]` есть день недели даты
    `local_origin + i дней`.

    ПОЧЕМУ ИМЕННО ПОЛНОЧЬ, А НЕ МОМЕНТ ЗАПРОСА. Колонка ряда — АБСОЛЮТНЫЙ
    локальный час (`grid[i][local.hour]`), а не смещение от начала ряда. Пока
    ряд начинался в час запроса, эти две системы координат расходились: ряд,
    начатый в полдень, держал в колонках 0-11 данные СЛЕДУЮЩИХ суток, то есть
    левая часть ряда была ПОЗЖЕ правой. Сетке, читаемой как «час суток × день»,
    это не мешало, а `activity_chart` читает те же колонки слева направо КАК
    ВРЕМЯ — и рисовала более позднюю отправку левее более ранней (CR-03).
    Якорь на полночь делает порядок колонок порядком времени по построению и
    заодно делает подпись суток честной: до него половина столбцов ряда
    ...
    # Полночь берётся В ЗОНЕ ЧИТАТЕЛЯ, а не в UTC: локальные сутки москвича
    # начинаются на три часа раньше UTC-суток, и якорь по UTC сдвинул бы ему
    # границу ряда обратно внутрь суток — то есть вернул бы CR-03 в меньшем
    # масштабе. `normalize_utc` обязателен: `now` приходит и из теста, и из
    # `datetime.now`, и naive-значение уронило бы `astimezone` не там, где
    # видно.
    local_origin = normalize_utc(now).astimezone(tz).replace(
        hour=0, minute=0, second=0, microsecond=0
    ) - timedelta(days=days - 1)
    # Граница запроса СЛЕДУЕТ за сеткой, а не считается отдельно: разъехавшись,
    # они дали бы записи, прошедшие фильтр базы и попавшие в клампованный ряд —
    # то есть чужие сутки, приписанные крайнему столбцу.
    window_start = local_origin.astimezone(timezone.utc)
    ...
    stream = await session.stream(
        ...
        .where(
            ...
            # Верхняя граница — по той же причине, что у плиток: запись с
            # `sent_at` в будущем иначе доезжала бы до раскладки и клампом
            # приписывалась бы последним суткам окна, то есть рисовала бы
            # активность в часе, в котором её ещё не было.
            SendLog.sent_at <= now,
        )
    ...
    async for row in stream:
        ...
        # Клампы, а не `continue`. С якорем на локальную полночь запись в `now`
        # даёт `offset_hours = (days-1)*24 + local.hour ≤ days*24 - 1`, то есть
        # верхний кламп для записи в пределах окна НЕДОСТИЖИМ — так было не
        # всегда: пока ряд начинался в момент запроса, смещение `days*24` было
        # обычным делом. Оба клампа остаются защитой в глубину на случай, когда
        # `sent_at` и запрошенный `now` разъедутся (расхождение часов воркера и
        # базы, явный `now` от вызывающего): такая запись принадлежит крайним
        # суткам окна, и её пропуск был бы тем самым молчаливым выбрасыванием,
        # которое прохибиция плана запрещает. Записи из БУДУЩЕГО до сюда больше
        # не доезжают — их отсекает верхняя граница запроса.
        if day_index >= days:
            day_index = days - 1
        elif day_index < 0:
            day_index = 0
