...
def unclosed_payment_clause():
    ...
    одно. Скопированное во второе место, оно разошлось бы с первым молча —
    достаточно, чтобы платёжный провайдер завёл четвёртый статус, — и залипший
    платёж пропал бы из журнала ровно тогда, когда за ним и приходят.
    """
