...
# Текст плашки для НЕОЖИДАННОГО отказа — того, что ловится широким `except`.
#
# Почему не `str(e)`. Значение `last_sync_result.error` шаблон печатает
# пользователю ДОСЛОВНО (`account_groups/list.html`), а широкий except ловит всё
# подряд, включая исключения слоя данных: после появления
# `uq_groups_account_external` реальный кандидат — `IntegrityError`, чей `str()`
# несёт полный SQL и значения параметров, а в фоновых путях — `RuntimeError`
# менеджера контейнеров с внутренним адресом моста. Раскрытие ограничено
# владельцем аккаунта, но это всё равно детали схемы и внутренние адреса на
# экране, и это прямо противоречит T-03-17.
#
# Исходный текст не теряется: обе точки вызова уже пишут его в лог с
# `exc_info=True`. Тексты УЗКИХ веток (`MessengerFetchError`, состояние моста,
# таймаут) остаются своими — они формируются нами и подконтрольны.
...
# Числа продублированы здесь намеренно: проверка длины обязана происходить ДО
# того, как значение попадёт в сессию, а модель на этом уровне не читается —
# хелпер знает про недоверенность входа, а не про DDL.
#
# Почему граница вообще проверяется: на SQLite (вся тестовая суита) длина не
...
# может.
#
# РЕАКЦИИ ДВЕ, И ОНИ РАЗНЫЕ.
#
# `name` ОБРЕЗАЕТСЯ: обрезанное имя чата читается хуже полного, но цена ошибки
# косметическая, а синхронизация целого аккаунта из-за одного длинного названия
# падать не должна.
#
# `group_external_id` ПРОПУСКАЕТСЯ вместе со всей группой: это КЛЮЧ
# МАРШРУТИЗАЦИИ — он уходит в мессенджер при каждой отправке
# (`app/application/scheduling/use_cases.py`: `group_id=group.group_external_id`).
# Обрезанный ключ не «читается хуже» — он не адресует ничего: строка создалась
# бы, показалась пользователю, была бы выбрана им в расписании, и каждая
# отправка по ней тихо проваливалась бы. Побочно два разных длинных
# идентификатора с одинаковым префиксом после обрезки схлопывались бы в один и
# нарушали `uq_groups_account_external`, роняя синк всего аккаунта. Пропуск —
# та же реакция, что и на любой другой негодный ЭЛЕМЕНТ ответа (см. цикл ниже);
# границу не проходит ни один реальный идентификатор трёх поддержанных
# мессенджеров.
...
async def apply_group_resync(
    ...
    *,
    ...
    # Скоуп — РОВНО тот же, что у `uq_groups_account_external`: только
    # `account_id`. Изоляция T-03-06 («внешний идентификатор уникален внутри
    # мессенджера, но не внутри нашей базы — у другого пользователя может лежать
    # группа с тем же group_external_id») этим уже обеспечена: чужая группа
    # висит на чужом аккаунте, а владение аккаунтом проверено вызывающими.
    #
    # Добавочное условие по `user_id` было ШИРЕ ограничения и потому опасно:
    # строка, чей `user_id` разошёлся с владельцем аккаунта (миграция данных,
    # ручная правка), в словарь не попадала, вставлялась заново и упиралась в
    # ограничение — то есть защитное условие превращало безобидное расхождение
    # в отказ синхронизации ВСЕГО аккаунта. Скоуп поиска существующих строк
    # обязан совпадать со скоупом ограничения, иначе хелпер не видит ровно тех
    # строк, из-за которых упадёт его же коммит.
    ...
    for item in fetched:
        ...
        if len(external_id) > _EXTERNAL_ID_MAX:
            # Ключ маршрутизации не обрезается — группа пропускается целиком
            # (обоснование у _EXTERNAL_ID_MAX выше). Строка-призрак, по которой
            # отправка молча проваливается, хуже отсутствующей строки.
            continue
...
async def record_sync_failure(
    ...
    ПОЧЕМУ `async`, ЕСЛИ ВНУТРИ НЕТ НИ ОДНОГО `await`. Тело — одно
    присваивание, к БД функция не обращается, и `async` здесь стоит РАДИ
    СИММЕТРИИ с `apply_group_resync`: обе функции — две ветки одного решения
    («синк удался» / «синк не удался»), их вызывают шесть точек в трёх модулях
    подряд, и разная форма вызова у соседних строк читалась бы как разная
    природа операции. Сделать её обычной пришлось бы вместе со снятием `await`
    во всех точках вызова — и первая же будущая правка, которой понадобится
    запрос (например, чтение аккаунта заново), вернула бы `async` обратно,
    сломав всех вызывающих. Сигнатура объявлена стабильной намеренно; отсутствие
    `await` внутри — не недосмотр.
