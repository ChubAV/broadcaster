...
# ⚠️ ПЛАТЯЩИЙ — ЭТО НЕ ТО ЖЕ, ЧТО «ДОСТУП ОТКРЫТ», И ТРЕТЬЕ УСЛОВИЕ ЗДЕСЬ
# НЕСУЩЕЕ (D-38). У пользователя с бесплатным доступом дверь открыта, а денег
# нет; слив его в один счётчик с платящими, «Обзор» сообщил бы, что
# АДМИНИСТРАТИВНАЯ ЛЬГОТА накручивает выручку — в отчёте, по которому принимают
# решение о цене. Поэтому условий три, а не два: строка активна, льгота НЕ
# выдана, срок жив.
#
# ⚠️ ВЫРАЖЕНИЕ ЛЕЖИТ В СЛОЕ ДОСТУПА К ДАННЫМ ПО ТОЙ ЖЕ ПРИЧИНЕ, ЧТО И ОСЬ ВЫШЕ.
# Признак бесплатного доступа читается в `app/application/` РОВНО ОДНИМ файлом —
# предикатом доступа, — и это машинный гейт
# (`test_the_free_access_flag_is_read_in_exactly_one_place_of_the_decision`), а
# не соглашение. Ослабить гейт ради того, чтобы условие стояло поближе к своему
# счётчику, значило бы разменять единственность правила доступа на раскладку
# файлов.
#
# СРАВНЕНИЕ ДАТ СТРОГОЕ — той же строгости, что `_live_by_term` и
# `subscription_is_live`: момент, равный `now`, живым не считается, потому что
# оплаченный момент уже прошёл. Две разных строгости на один вопрос дали бы
# расхождение ровно на границе суток, то есть там, где его никто не ищет.
def paying_subscription_clauses(now: datetime) -> tuple:
    """Три условия платящей подписки — ЕДИНСТВЕННОЕ место этого правила.

    Отдаются кортежем, а не готовым `select`: их читают и счёт платящих, и
    подпись выручки под ним, и вторая копия набора разошлась бы с первой молча —
    число говорило бы одно, а деньги под ним считались бы по другому множеству.
    """
    return (
        Subscription.is_active.is_(True),
        Subscription.has_free_access.is_(False),
        Subscription.expires_at > now,
    )


# ⚠️ ЛЬГОТНЫЙ ИСКЛЮЧАЕТСЯ И ЗДЕСЬ, И ЭТО НЕ СИММЕТРИЯ РАДИ СИММЕТРИИ. У человека
# с выданным бесплатным доступом дата окончания МЁРТВАЯ и остаётся такой
# навсегда — сроком его доступ не описывается вовсе. Посчитанный ушедшим, он
# сообщил бы администратору, что от продукта отказался тот, кому администратор
# сам же открыл дверь бессрочно, — и величина «истекло и не продлено» начала бы
# расти от каждой выданной льготы.
#
# ⚠️ НАБОР ЛЕЖИТ В СЛОЕ ДОСТУПА К ДАННЫМ ПО ТОЙ ЖЕ ПРИЧИНЕ, ЧТО И ДВА СОСЕДА
# ВЫШЕ: признак бесплатного доступа читается в `app/application/` РОВНО ОДНИМ
# файлом — предикатом доступа, — и это машинный гейт
# (`test_the_free_access_flag_is_read_in_exactly_one_place_of_the_decision`), а
# не соглашение. Прикладной модуль подраздела «Платежи» дописывает к этому
# набору только своё, платёжное условие: «после этой даты не было ни одного
# успешного платежа».
#
# ГРАНИЦЫ ОКНА РАЗНОЙ СТРОГОСТИ, И РАЗНИЦА НЕСУЩАЯ. Верхняя нестрогая
# (`<= now`) — дополнение к строгому «срок жив» соседа: момент, равный `now`,
# живым не считается, и обе величины обязаны разбивать популяцию без дыры на
# границе. Нижняя строгая (`> since`) — иначе платёж ровно на краю окна попадал
# бы в величину и завтра, и послезавтра.
def expired_not_renewed_clauses(now: datetime, since: datetime) -> tuple:
    """Условия подписки, чей срок истёк ВНУТРИ окна: активна, не льготная, мертва.

    Отдаются кортежем, а не готовым `select`: платёжное условие дописывает
    прикладной модуль, и готовая выборка отняла бы у него эту возможность.
    """
    return (
        Subscription.is_active.is_(True),
        Subscription.has_free_access.is_(False),
