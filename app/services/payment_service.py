...
# НАБЛЮДАЕМЫЕ СТАТУСЫ — те, ЗА ИСХОДОМ КОТОРЫХ АДМИНИСТРАТОР ЖДЁТ. Множество
# объявлено ОТДЕЛЬНО от терминальных, потому что вопросов о статусе платежа НЕ
# ОДИН, а два, и они разные: «из какого статуса платёж уже не выйдет» и «за
# каким статусом ждут исхода». Одно множество на оба вопроса не отвечает, и
# дополнение первого на второй не отвечает тем более. Читателей у этого
# множества два — признак денежного инцидента и чипс «В обработке» журнала
# платежей, — и оба берут ЕГО. Закреплено
# `test_an_expired_intent_does_not_raise_the_payment_incident`.
#
# ⚠️ ДОПОЛНЕНИЕ ТЕРМИНАЛЬНЫХ ОСТАЁТСЯ ЗАКОННЫМ ОТВЕТОМ НА СВОЙ ВОПРОС, И ЭТОТ
# ПУТЬ НЕ ТРОНУТ. Его продолжает задавать `_claim_payment`: заплативший по
# старой ссылке обязан выиграть заявку на просроченной строке, и сужение того
# условия до наблюдаемых значило бы принять деньги и не выдать доступ.
# Закреплено `test_a_claim_is_won_on_an_expired_intent`.
#
# ⚠️ НА ВТОРОЙ ВОПРОС ДОПОЛНЕНИЕ ОТВЕТИЛО НЕВЕРНО РОВНО В ТОТ ДЕНЬ, КОГДА ФАЗА
# ЗАВЕЛА ЧЕТВЁРТЫЙ СТАТУС (D-01; цена решения выписана в `08-CONTEXT.md`).
# `STATUS_EXPIRED` в это множество не входит и не войдёт: снятое по сроку
# давности намерение — штатный исход, а не аномалия. Порог уборки
# (`_expire_stale_intents`) и порог залипания признака считаются ОТ ОДНОЙ
# константы `PENDING_INTENT_TTL_HOURS` и совпадают ПО ПОСТРОЕНИЮ, поэтому
# каждая благополучно снятая строка иначе становилась бы постоянным денежным
# инцидентом, а массовый перевод строк ревизией `0021` дал бы разовый залп.
# Закреплено `test_an_expired_intent_does_not_raise_the_payment_incident` и
# `test_the_ledger_selects_unclosed_payments_by_the_declared_awaiting_set`.
