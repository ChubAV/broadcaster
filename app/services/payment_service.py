...
async def _extend_subscription(
    ...
    действующей подписке не предлагается карточкой и не принимается гардом
    `POST /billing/subscribe`. Прохибиция плана `05-01` — «MUST NOT сжигать
    неистраченный остаток уже оплаченного периода» — тем самым СОБЛЮДЕНА, а не
    переопределена: ни один разрешённый переход остатка не сжигает, и поведение
    `next_expiry` этим решением не меняется ни на строку.
    ...
    ⚠️ ГАРД СТОИТ НА ВХОДЕ, А НЕ ЗДЕСЬ, И ЭТО НАМЕРЕННО. Сюда платёж приходит
    УЖЕ ОПЛАЧЕННЫМ: деньги списаны, уведомление подтверждено. Отказать ему в
    начислении значило бы взять деньги и не выдать ничего — худший из возможных
    исходов, и куда худший, чем понижение, проскочившее по устаревшей странице
    до того, как правило появилось. Поэтому подтверждённый платёж исполняется
    здесь ЛЮБОЙ, а решает, продавать ли, обработчик формы.
    """
