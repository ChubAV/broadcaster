...
записано прямым текстом в шапке модуля приёма загрузки,
`app/services/image_upload.py` (ссылка обновлена переездом, Фаза 12, план 12-05:
прежде тот абзац жил в снятом модуле маршрута слоя JSON-API). Запрет снят здесь не
