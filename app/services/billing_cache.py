...
# ГЕЙТ ДОСТУПА КЭШИРУЕТСЯ ПО ФОРМЕ ГЕЙТА БАЛАНСА — ТОЙ ЖЕ ЦЕЛИКОМ, А НЕ ПОХОЖЕЙ.
# Сигнатура `(db, user_id, action) -> tuple[bool, str]` и ранний выход по
# `action != "send"` есть КОНТРАКТ параметра `check_limit` у
# `collect_due_schedules`: только при их сохранении три точки пути отправки
# правятся заменой имени, а не правкой поведения.
#
# ⚠️ СТАРАЯ ПАРА `check_balance_cached` / `invalidate_balance_cache` ОСТАЁТСЯ
# ЖИВОЙ, И ЭТО ПОРЯДОК, А НЕ ЗАБЫВЧИВОСТЬ. Её снимает план `05.1-06` вместе с
# самим списанием сообщений. Между введением нового гейта и снятием старого путь
# отправки обязан быть закрыт хотя бы одним из них: снятие первым оставило бы
# окно между двумя коммитами, в котором рассылка идёт без гейта вовсе.
...
async def check_access_cached(
    ...
    ⚠️ КЛЮЧ НАЗЫВАЕТСЯ `access:{user_id}`, А НЕ `balance:{user_id}`, И
    ПЕРЕИМЕНОВАНИЕ ЗДЕСЬ — РЕШЕНИЕ, А НЕ ОФОРМЛЕНИЕ. Значение под ключом сменило
    СМЫСЛ: раньше это был ответ «есть ли сообщения на балансе», теперь — «открыт
    ли доступ». Сохранённое имя дало бы после выката минуту (TTL кэша), в которой
    закэшированные ответы о балансе читаются как вердикты авторизации — неверные
    и ничем не отличимые от верных. Старые ключи при этом истекают сами, и
