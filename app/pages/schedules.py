...
def _editor_url(form_data, ad_id: int | None, schedule_id: int | None = None) -> str:
    ...
    одна СТРОКА формы в адрес не подставляется. Из формы приходит ровно одна
    величина — целое, полученное вызывающим через `_expanded_from_form`
    (путь тумблера); остальные вызывающие передают идентификатор из пути
    запроса. Целое адрес изменить неспособно, а его принадлежность объявлению
    перепроверяет редактор (app/pages/ads.py:361-363). Довод верен дословно и
    после переезда: переехала сборка строки, а не источник её частей.
...
def _editor_redirect(form_data, ad_id: int | None, schedule_id: int | None = None):
    ...
    return RedirectResponse(
        url=_editor_url(form_data, ad_id, schedule_id), status_code=302
    )
...
async def schedules_delete(
    ...
    # СБОРКЕ АДРЕСА, И В ВЫБОРЕ ФОРМЫ ОТВЕТА (`WR-01`). Литерал значения второй
    # раз не выписывается: второй источник одного решения разошёлся бы с первым
    # при первой же правке — то самое основание, по которому адрес собирается
    # ОДНИМ кодом в `_editor_url`.
