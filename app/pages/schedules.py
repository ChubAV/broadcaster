...
# ВЕРХНЯЯ ГРАНИЦА ВЕЛИЧИНЫ ИДЕНТИФИКАТОРА — ОДНА НА ВЕСЬ ФАЙЛ. Число здесь
...
# (T-02-24, T-02-25).
#
# ОСНОВАНИЕ СНЯТО С МОДЕЛЕЙ, А НЕ ПЕРЕНЕСЕНО ЧИСЛОМ. Все три идентификатора,
# участвующие в маршрутах этого файла, объявлены ОДНИМ И ТЕМ ЖЕ отображаемым
# целым без указания расширенной разрядности:
...
# Три колонки одной разрядности, поэтому граница у них ОДНА, и держать её надо
...
# лежат (основание записано у `_ad_id_from_form` и переносится дословно).
#
...
# которого падали два из пяти воспроизведённых отказов.
ScheduleIdPath = Annotated[int, Path(ge=1, le=ID_MAX)]
...
def _ad_id_from_form(form_data) -> int | None:
    ...
    ⚠️⚠️ ОПРОВЕРГНУТО (`CR-01`) — АБЗАЦ НИЖЕ ОСТАВЛЕН НАРОЧНО, ВЕРНАЯ
    ФОРМУЛИРОВКА В КОНЦЕ ДОКСТРИНГА. Он утверждал ГРАНИЦУ, которой в коде не
    было, и именно на нём аналогия ломается.

    ⚠️ ВЕЛИЧИНА ПОДЧИНЕНА ТЕМ ЖЕ ПРАВИЛАМ, ЧТО И `_expanded_from_form`, И ПО ТОЙ
    ЖЕ ПРИЧИНЕ. Коэрция к целому делает поле структурно неспособным изменить
    адрес (T-02-23): строка с `&`, `#`, переводом строки или абсолютным URL до
    конкатенации не доходит. Владение по этому полю НЕ ПРЕЗЮМИРУЕТСЯ: и счёт, и
    предикат ветвления ограничивают выборку связью `Ad.user_id`, поэтому чужой
    номер даёт ветку перехода и ноль строк, а не доступ к чужим данным; сам
    редактор на приземлении перепроверяет владение ещё раз
    (app/pages/ads.py:361-363).
    ...
    ⚠️ АБЗАЦ ОБ АНАЛОГИИ С `_expanded_from_form` ВЫШЕ — ОПРОВЕРГНУТО, И ОН
    ОСТАВЛЕН ЗДЕСЬ НАРОЧНО, А НЕ ВЫЧЕРКНУТ. Вычеркнутая запись не оставляет
    следующему читателю причины, по которой аналогия была неверной, и он
    повторит её на следующем поле, взятом из формы. Верная формулировка —
    ниже, и она называет ЧЕТЫРЕ вещи:
    ...
    (в) ГРАНИЦА ВЗЯТА ПО ВЕРХНЕЙ ГРАНИЦЕ КОЛОНКИ (`ID_MAX`) И СТОИТ ДО
        ЗАПРОСА. Нижняя граница — ЕДИНИЦА, а не ноль: идентификаторы
        автоинкремента начинаются с единицы, и ни ноль, ни отрицательные в
        домене поля не лежат. Побочное следствие названо, а не проглочено:
        адрес приземления для `-1` и `0` собирался ИЗ НИХ (`/ads/-1/edit`,
        `/ads/0/edit`) и теперь не собирается — приземление уходит на сводный
        список.

    (г) ⚠️⚠️ ОПРОВЕРГНУТО (`CR-02`) — ФОРМУЛИРОВКА НИЖЕ ОСТАВЛЕНА НАРОЧНО, А НЕ
        ВЫЧЕРКНУТА; ВЕРНАЯ СТОИТ ПУНКТОМ (д). Она называла предметом закрытия
        МАРШРУТ ЦЕЛИКОМ, и читатель, поверивший ей, был обманут дважды: сперва
        принял маршрут за покрытый, а затем — правку одного из двух источников
        величины за закрытие блокера. Прежняя формулировка:

        «ПРЕДМЕТ НАЗВАН ПОИМЁННО: `CR-01` отчёта верификации Фазы 10,
        воспроизведённый пятисоткой на маршруте подтверждённого удаления
        (`POST /schedules/{id}/delete`, тело `return_to=editor&ad_id=` и
        двадцать пять девяток).»

    (д) ВЕРНАЯ ФОРМУЛИРОВКА: ПРЕДМЕТ ЭТОГО ПОМОЩНИКА — ПОЛЕ КОНТЕКСТА, И ТОЛЬКО
        ОНО. Маршрут подтверждённого удаления несёт ДВА источника величины, и
        второй — идентификатор ПУТИ — этим помощником не читается вовсе: он
        ограничен на границе приложения псевдонимом `ScheduleIdPath`. Тем же
        местом закрыты и остальные входы файла: идентификаторы пути маршрутов
        правки и тумблера, обязательное поле объявления и необязательное поле
        аккаунта маршрута создания. Перечень всех пяти входов стережётся
        правилом
    ...
    (е) РАСПОРЯЖЕНИЕ НЕГОДНОЙ ВЕЛИЧИНОЙ РАЗЛИЧАЕТСЯ ПО РОЛИ ЗНАЧЕНИЯ, И ЭТО
