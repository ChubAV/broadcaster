...
# ВЕРХНЯЯ ГРАНИЦА ИДЕНТИФИКАТОРА ОБЪЯВЛЕНИЯ. Число здесь ВТОРИЧНО, основание
# первично: это верхняя граница КОЛОНКИ идентификатора (знаковое
# тридцатидвухбитное целое), то есть величина, вне которой значение не может
# принадлежать НИ ОДНОЙ строке ни на одном из двух драйверов проекта — ни на
# SQLite суиты, ни на PostgreSQL боевого стенда. Поэтому значение вне диапазона
# отбрасывается ДО запроса, а не роняет запрос драйвером: на SQLite это
# `OverflowError` («Python int too large to convert to SQLite INTEGER»), на
# PostgreSQL — `DataError` вне диапазона int32, и оба заканчиваются HTTP 500 на
# форменный POST — ровно тем, что модуль правил редактора себе запретил
# (T-02-24, T-02-25).
_AD_ID_MAX = 2_147_483_647
...
def _ad_id_from_form(form_data) -> int | None:
    ...
    (в) ГРАНИЦА ВЗЯТА ПО ВЕРХНЕЙ ГРАНИЦЕ КОЛОНКИ (`_AD_ID_MAX`) И СТОИТ ДО
    ...
    (г) ПРЕДМЕТ НАЗВАН ПОИМЁННО: `CR-01` отчёта верификации Фазы 10,
    ...
    try:
        ...
        return None
    if value < 1 or value > _AD_ID_MAX:
        return None
    return value
