...
def is_same_origin(request: Request) -> bool:
    ...
    источник на проект. ЧИСЛО И ПЕРЕЧЕНЬ ПОТРЕБИТЕЛЕЙ ЗДЕСЬ НЕ ВЕДУТСЯ: полноту
    держат ДВА МАШИННЫХ ГЕЙТА, названных поимённо, —
    ...
    подтверждённого удаления пользовательских данных). Приватная копия, жившая в
    `history.py` с плана 04-10, снята планом 05-04: второй экземпляр правила
    означал бы, что правку одного гарда придётся не забыть повторить в другом, а
    забытая половина выглядела бы работающей.

    ⚠️ ПРЕЖНЕЕ ПЕРЕЧИСЛЕНИЕ ЭТОГО АБЗАЦА ОПРОВЕРГНУТО ЗАМЕРОМ И ОСТАВЛЕНО
    НАЗВАННЫМ, А НЕ СТЁРТЫМ (идиома D-30/D-32). Оно гласило: «Потребителей три,
    и все зовут эту функцию: форма покупки тарифа, форма покупки пакета
    сообщений (обе — `app/pages/billing.py`) и повтор отправки
    (`app/pages/history.py`)». ЗАМЕР (2026-09-05, `grep -rn 'is_same_origin'
    app/`) дал ЧЕТЫРЕ файла-потребителя и ДЕВЯТЬ мест вызова — то есть
    перечисление разошлось с деревом ещё ДО плана 10-18, который довёл число до
    ВОСЬМИ файлов и тринадцати мест вызова, повесив гард на четыре маршрута
    подтверждённого удаления (`WR-07`, ревизия 2026-09-04). Действующий преемник
    утверждения — два названных выше гейта, а не список в докстринге: перечень,
    который надо не забыть исправить, забывают.
