...
def is_same_origin(request: Request) -> bool:
    ...
    источник на проект: форма покупки подписки (план 05-01) и повтор отправки
    (`app/pages/history.py`) обязаны решать «свой ли источник» одним и тем же
    кодом. Перевод вызова в `history.py` на это имя — план 05-04; до него в
    `history.py` остаётся его собственная копия, и это ЕДИНСТВЕННЫЙ известный
    дубль правила.
    """
