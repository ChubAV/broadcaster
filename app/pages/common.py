...
def is_same_origin(request: Request) -> bool:
    ...
    находит их ЗАМЕРОМ, а не перечнем. Административный
    модуль из его вселенной ИЗЪЯТ ПОИМЁННО, с основанием и числом, потому что
    его маршруты уже стережёт первый гейт: два правила над одним маршрутом
    означали бы два объявленных числа, которые придётся не забыть править
    вместе. Само изъятие стережётся правилом
