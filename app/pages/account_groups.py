...
async def account_groups_partial(
    ...
    «НЕТ СЕССИИ» И «ЧУЖОЙ АККАУНТ» (WR-04, план 09-15). Ответ этого маршрута
    приезжает В СЕРЕДИНУ СУЩЕСТВУЮЩЕГО ДОКУМЕНТА: сентинел подменяет им себя.
    Запрос слоя письма следует за перенаправлением ПРОЗРАЧНО, поэтому
    собственный ответ-перенаправление означал бы, что в середину списка
    вклеивается ЦЕЛАЯ страница входа — со второй формой входа и ВТОРЫМ НАБОРОМ
    ИДЕНТИФИКАТОРОВ. Уникальность `id` документа ломается тем же способом, что и
    в CR-01, а на ней стоит каждая цель подмены и каждый внеполосный узел
    экрана. Признак отказа при этом молчаливый: ответ успешный, консоль чистая.

    Соседний вход этого же модуля отвергает РОВНО ЭТО и объясняет почему —
    смотри `account_groups_sync_status`; довод здесь не пересказывается, чтобы
    два его экземпляра не разъехались. Отличие только в форме ответа: у статуса
    синхронизации отказ есть ПУСТОЙ ответ, останавливающий опрос, а порции
    останавливать нечего — ей нужен переход, и его строит слой ответа.

    ⚠️ Имена класса ответа и статуса набраны здесь СЛОВАМИ, а не литералами, по
    тому же правилу, что записано у обработчика переключения: отсутствие
    собственного перенаправления проверяется в том числе обходом тела этого
    входа, и докстринг, набравший их дословно, удовлетворил бы обход сам.
    """
    user = await get_user_from_cookie(request, db, settings)
    if not user:
...
async def account_groups_toggle(
    ...
    # плана обработчик поля `search` не принимал ВОВСЕ — то есть довести
    # величину до ответа было нечем в принципе, и третье место отрисовки формы
    # удаления (`toggle_response.html`) печатало пустое значение.
    #
    # ⚠️ ТИП СТРОКОВЫЙ И НЕОБЯЗАТЕЛЬНЫЙ — ТОТ ЖЕ, ЧТО У `account_groups_delete`.
    # Разметка отсутствие поля объявить не может, а второй тип у одного и того
    # же поля двух обработчиков разъехался бы с первым молча.
    term = _clean_search(search)

    # АДРЕС ПРИЗЕМЛЕНИЯ СОБИРАЕТСЯ ОДИН РАЗ И ОДИН НА ОБЕ ВЕТКИ — ПОМОЩНИКОМ, А
    # НЕ f-СТРОКОЙ (WR-01, план 09-18). Докстринг `_screen_url` требует единого
    # источника прямым текстом: адрес после действия и адрес после перезагрузки
    # обязаны приходить из одного источника, иначе они разъедутся молча — и
    # человек, нажавший тумблер, увидит не тот экран, что человек, обновивший
    # страницу. Оба выхода собирали адрес литералом, и разошлись они ровно так:
    # ИЗМЕРЕНО ревизией — POST без признака htmx со страницы `?search=Альфа`
    # давал 302 на список БЕЗ фильтра.
    #
    # ⚠️ НЕОТЛИЧИМОСТЬ (D-13, T-9-02) ПЕРЕВОДОМ НЕ ОСЛАБЛЕНА, А УКРЕПЛЕНА.
    # Чужая и несуществующая группа идут ОДНОЙ веткой и получают ОДИН И ТОТ ЖЕ
    # адрес — тот же, что получила бы найденная. Различимым ответ стал бы ровно
    # тогда, когда ветки собирали бы адрес ПО-РАЗНОМУ; общий источник делает
    # обратное.
    ...
    if group is None:
        ...
        # перехода — с ним. Адрес — ТОТ ЖЕ `screen_url`, что и у найденной
        # группы: собранный отдельно, он выдал бы ветку различием заголовка.
        return await respond(request, redirect=screen_url)

    # СОСТОЯНИЕ БЕРЁТСЯ ИЗ ТЕЛА ФОРМЫ, А НЕ ВЫЧИСЛЯЕТСЯ ИЗ ХРАНИМОГО (WR-02,
    # план 09-15). Между отрисовкой строки и отправкой формы чекбокс может
    # смениться ЧЁТНОЕ число раз, и тогда хранимое значение и показанное человеку
    # совпадают — а инверсия приводила группу в ОБРАТНОЕ. Присланное значение и
    # есть то, что человек видит перед собой; применяется именно оно.
    #
    # ⚠️ МИР, В КОТОРОМ ЭТО ДОСТИЖИМО, НАЗЫВАЕТСЯ ПОИМЁННО: «Alpine жив, htmx
    # мёртв». Его завёл план 09-07, сделав снятие кнопки «Применить» УСЛОВНЫМ и
    # перенеся отправку по изменению чекбокса на слой письма: кнопка выживает,
    # автоотправки нет, и до нажатия человек переключает чекбокс сколько угодно
    # раз. По D-05 тумблер — единственный способ исключить группу из рассылки, не
    # удаляя её, поэтому цена обратного состояния — рассылка в чат, который
    # человек считает выключенным, без единого признака отказа.
    #
    # ⚠️ ПОВТОРНОЕ НАЖАТИЕ ПО-ПРЕЖНЕМУ БЕЗВРЕДНО (QUAL-01), И ЭТО СВОЙСТВО НЕ
    # ПОТЕРЯНО, А ДЕРЖИТСЯ ТЕПЕРЬ ДРУГИМ. Держит его ИДЕМПОТЕНТНОСТЬ ПРИСВОЕНИЯ:
    # второе то же тело не создаёт новой строки И не двигает состояния — в
    # отличие от инверсии, у которой второй запрос состояние МЕНЯЛ. Единица
    # записи прежняя: один `commit` одной строки под тройным `WHERE`. Клиентская
    # блокировка повторной отправки (`hx-disabled-elt`) серверной защитой НЕ
    # является и таковой не объявляется (PAY-02) — она убирает второе нажатие у
    # человека, а не второй запрос у злоумышленника.
    #
    # ⚠️ ПРЕЖНЕЕ ОБОСНОВАНИЕ НЕ ВЫЧЁРКИВАЕТСЯ, А НАЗЫВАЕТСЯ ОПИСЫВАЮЩИМ СНЯТУЮ
    # ФОРМУ (образец — записи D-30 и D-32 .planning/STATE.md). До плана 09-15
    # здесь стояло: «ИНВЕРСИЯ, а не установка: действие обратимо одним нажатием и
    # потому обходится без панели подтверждения (D-08). Обработчик, жёстко
    # ставящий False, оставил бы пользователя без способа включить группу», и
    # «ОНА ЖЕ ДЕЛАЕТ ПОВТОРНОЕ НАЖАТИЕ БЕЗВРЕДНЫМ (QUAL-01)». Оба утверждения
    # были ВЕРНЫ ДЛЯ СВОЕЙ ФОРМЫ и сняты вместе с ней: обратимость и безвредность
    # остались, но обеспечиваются присвоением, а не инверсией. Опасение «жёстко
    # ставящий False» правкой не воскрешается — значение приходит из формы и
    # принимает оба положения.
    #
    # ⚠️ D-08 ЦЕЛ И К СПОСОБУ ВЫЧИСЛЕНИЯ СОСТОЯНИЯ ОТНОШЕНИЯ НЕ ИМЕЕТ: действие
    # обратимо одним нажатием и потому обходится без панели подтверждения. Это
    # утверждение об ИНТЕРФЕЙСЕ — человек возвращает группу в прежнее состояние
    # тем же тумблером, — и правка его не касается ни одним свойством.
    #
    # ⚠️ ТИП ПАРАМЕТРА СТРОКОВЫЙ И НЕОБЯЗАТЕЛЬНЫЙ НАМЕРЕННО. Снятый чекбокс не
    # шлёт НИЧЕГО, а булев параметр с умолчанием заставил бы клиента ОБЪЯВЛЯТЬ
    # отсутствие — чего разметка формы сделать не может.
    ...
    async def _fragment() -> HTMLResponse:
        ...
        html = templates.env.get_template(
            ...
            active_groups=active_groups,
            total_groups=total_groups,
            # СТРОКА ПОИСКА ДОЕЗЖАЕТ ДО ТРЕТЬЕГО МЕСТА ОТРИСОВКИ ФОРМЫ УДАЛЕНИЯ
            # (CR-02, план 09-18) — тем же выражением, каким это делает
            # страница (`list.html`). Без неё макрос строки печатал бы своё
            # умолчание — пустую строку, — и переключённая строка уезжала бы к
            # человеку с формой удаления, потерявшей фильтр.
            filter_search=term or "",
        )
        return HTMLResponse(html)

    # `notice` НЕ передаётся (D-10): исход виден в самой строке и в счётчике,
    # а плашка на каждый успех превратила бы обратную связь в шум. Правка
    # величины `redirect=` этого не пересматривает: адрес приземления сменился,
    # уведомления как не было, так и нет.
...
async def account_groups_delete(
    ...
    # планом 10-32 (`WR-02`, ревизия 2026-09-07). Она стои́т ВЫШЕ тройного
    # `WHERE`: отказ по происхождению не имеет права стать признаком
    ...
    #
    # ⚠️ ДВА ЗАПРОСА ПОДСЧЁТА ОТСЮДА УЕХАЛИ ВНУТРЬ ОТЛОЖЕННОЙ СБОРКИ ФРАГМЕНТА
    # (IN-03, план 09-15). Обработчик следует собственному правилу-близнецу,
    # записанному в докстринге `_fragment` обработчика тумблера, — пересказывать
    # его здесь значило бы завести второй экземпляр одного довода. Ветка перехода
    # разметки не собирает вовсе, и исполнять на ней два запроса, чей результат
    # немедленно выбрасывается, было незачем. Порядок при этом ничего не теряет:
    # `_current_listing_has_a_row` исполняется первым в любом случае.

    # СТРОКА ПОИСКА ПРИХОДИТ ТЕЛОМ ФОРМЫ и проходит тот же `_clean_search`, что
    # и на странице: ветка ответа обязана считаться по той же выдаче, которую
    # человек видит перед собой.
    #
    # ⚠️ ЕЁ ШЛЮТ ОБЕ ФОРМЫ ПУТИ УДАЛЕНИЯ, А НЕ ОДНА (WR-03, план 09-15). До этого
    # плана поле несла только форма панели подтверждения, и путь без Alpine
    # приезжал сюда БЕЗ строки поиска: `_screen_url` собирал адрес без фильтра, а
    # `_current_listing_has_a_row` спрашивал про весь аккаунт вместо той выдачи,
    # которую человек видел. Равенство наборов полей двух форм держит теперь
    # правило `test_both_delete_forms_post_the_same_field_names`.
    term = _clean_search(search)
    ...
    async def _fragment() -> HTMLResponse:
        ...
        собирается вовсе, то есть ДВА ЗАПРОСА ПОДСЧЁТА НЕ ВЫПОЛНЯЮТСЯ. Правило
        это не собственное: обработчик тумблера кладёт их сюда же и по той же
        причине, названной в его докстринге (IN-03, план 09-15). До этого плана
        оба запроса стояли ВЫШЕ развилки и исполнялись на ветке перехода, где их
        результат немедленно выбрасывался.
        ...
        было. Узлы снятия уезжают И ТОГДА, когда тройной `WHERE` не нашёл
        строки, — этого ТРЕБУЕТ неотличимость (D-04-A): условная сборка сделала
        бы наличие узла признаком того, что удаление состоялось.

        ⚠️ И ЦЕНА ЭТОГО НАЗЫВАЕТСЯ ЧИСЛОМ, А НЕ СЛОВОМ «МОЛЧА». Здесь стояло:
        «по несуществующему идентификатору внеполосное снятие не находит узла и
        не делает ничего — молча и безвредно». Утверждение ЛОЖНО, и это
        проверено ПО ВЕНДОРЕННОМУ РАНТАЙМУ, а не выведено
        (`app/static/js/htmx.min.js`, version 2.0.10): внеполосный узел, чью
        цель рантайм не разрешил, СНИМАЕТСЯ из ответа и поднимает
        `htmx:oobErrorNoTarget`, а подъём события с полем признака ошибки
        заканчивается вызовом `console.error`. Холостой путь удаления (чужая,
        несуществующая, УЖЕ УДАЛЁННАЯ группа) шлёт ДВА таких узла, то есть ДВЕ
        строки в консоли на запрос, — а достижим он ровно тем повторным
        запросом, ради безвредности которого неотличимость и заведена (кнопка
        «назад», повторная отправка формы). Прежняя формулировка не
        вычёркивается, а называется здесь ошибочной (образец — записи D-30 и
        D-32 `.planning/STATE.md`).

        ⚠️ «БЕЗВРЕДНО» ОСТАЁТСЯ ВЕРНЫМ ДЛЯ ДАННЫХ И ДОКУМЕНТА, И ТОЛЬКО ДЛЯ НИХ.
        Теряется ДИАГНОСТИЧЕСКИЙ признак — «ответ 200 И ЧИСТАЯ КОНСОЛЬ», на
        который опирается приёмка вехи и который наследуют Фазы 10-15. Остаток
        назван поимённо перечнем `OOB_TARGET_EXCEPTIONS`
        (`tests/test_pages/test_account_groups.py`) с обоснованием и назначенной
        Фазой 15. Неотличимость ради тишины консоли НЕ ослабляется ни на одно
        свойство: ветви по факту удаления в шаблоне нет, узлы снятия безусловны.

        ⚠️ УТВЕРЖДЕНИЕ О БЕЗВРЕДНОСТИ СУЖЕНО ДО ПРАВДИВОГО (IN-02). Безвреден
        идентификатор, ОТСУТСТВУЮЩИЙ В ДОКУМЕНТЕ, а не «несуществующий» вообще.
        Человек, смотрящий экран групп аккаунта A и пославший
        `/accounts/B/groups/42/delete` (оба аккаунта его, группа 42 принадлежит
        A), получает ответ, снимающий со страницы A ЖИВЫЕ `#group-row-42` и
        `#group-del-42` и подставляющий линейку с числами аккаунта B, — а группа
        42 остаётся в базе. Границы привилегий это не пересекает и дефектом
        безопасности не является; дефект был в том, что проза утверждала ШИРЕ
        поведения, и читатель понёс бы дальше широкую версию.
