...
async def account_groups_delete(
    ...
    account_id: int,
    group_id: int,
    rendered_rows: int | None = Form(None),
    ...
    ⚠️ ВЕТВЛЕНИЕ «СПИСОК ОПУСТЕЛ / НЕ ОПУСТЕЛ» СЧИТАЕТСЯ ПО ЧИСЛАМ АККАУНТА, А
    НЕ ПО ФАКТУ НАХОЖДЕНИЯ СТРОКИ (D-04, T-9-10). Для чужой и несуществующей
    группы `_group_counts` даёт те же числа, что и до запроса, и ветка
    получается той же, какой она была бы у последнего успешного удаления в
    этом же аккаунте: различимого признака не появляется ни в статусе, ни в
    теле, ни в заголовках.

    ⚠️ ЧИСЛО ОТРИСОВАННЫХ СТРОК ПРИХОДИТ ОТ КЛИЕНТА, И ЭТО НЕ ЛЕНЬ, А
    ЕДИНСТВЕННАЯ ВОЗМОЖНОСТЬ. Сервер не знает, ДОКУДА дочитан список: экран мог
    остаться на первой порции, а мог быть прокручен до пятой. Вычислить это он
    не может в принципе — попытка вычислить дала бы верный ответ ровно для
    непрокрученного экрана, то есть починила бы курсор там, где он и так цел, и
    сломала бы там, где он сломан. Поэтому живой документ сообщает своё число
    сам (`rendered_rows`), а ответ строит из него смещение починенного сентинела.
    Клиент, приславший число, не совпадающее с его же экраном, смещает СВОЙ
    СОБСТВЕННЫЙ курсор; на чужие документы это не действует и правами не
    распоряжается.

    ⚠️ ВЫЧИТАЕТСЯ ЧИСЛО СНЯТЫХ С ЭКРАНА СТРОК, А НЕ ЕДИНИЦА, И НЕОТЛИЧИМОСТЬ ПРИ
    ЭТОМ ЦЕЛА (D-04 в редакции амендмента D-04-A). Вычитание единицы всегда
    задваивало бы строку на ХОЛОСТОМ пути: у чужой, несуществующей и уже
    удалённой группы внеполосное снятие не находит узла, на экране остаются все
    присланные строки, а курсор уезжает назад — тот же отказ, что и потеря
    строки, но со сменённым знаком. У всех трёх тройной `WHERE` даёт `None`,
    значит число снятых строк у всех трёх НОЛЬ и смещение одно и то же: класс
    «строка не найдена» остаётся неразличимым изнутри. Нового канала утечки
    четвёртый узел не открывает — «сколько групп стало» ТРЕТИЙ внеполосный узел
    линейки печатает открытым текстом с плана 09-02, то есть смещение выводится
    из данных, уже раскрытых этим же ответом.
    """
    user = await get_user_from_cookie(request, db, settings)
    if not user:
        return await respond(request, redirect="/login")
    ...
    # СКОЛЬКО СТРОК ЭТОТ ОТВЕТ СНИМАЕТ С ЭКРАНА. Имя говорит про ЭКРАН, а не про
    # базу, намеренно: арифметика курсора считает ВИДИМОЕ, а не удалённое.
    # Величина — функция ровно от одного вопроса: вернул ли тройной `WHERE`
    # строку. Единица — строка найдена и сейчас будет снята; ноль — не найдено
    # ничего, и с экрана не уедет ни одной строки.
    ...
    if total_groups == 0:
        # СПИСОК ОПУСТЕЛ — ЗАКРЫВАЕТСЯ ПЕРЕХОДОМ, А НЕ ВТОРОЙ ОТРИСОВКОЙ
        # ПУСТОГО СОСТОЯНИЯ (D-09). Фрагмента нет, поэтому слой отвечает 302
        # без htmx и 204 с заголовком перехода — с ним. Три различимых пустых
        # состояния экрана живут в `account_groups/list.html` в ОДНОМ
        # экземпляре: второй их экземпляр во фрагменте разошёлся бы с первым
        # молча, и человек видел бы разный экран в зависимости от того, как он
        # на него попал.
        return await respond(request, redirect=f"/accounts/{account_id}/groups")

    # СМЕЩЕНИЕ ПОЧИНЕННОГО СЕНТИНЕЛА — сколько строк документ ПРОДОЛЖИТ
    # показывать после применения этого же ответа. Собирается ТОЛЬКО когда
    # число пришло и строго положительно: величина приходит от недоверенного
    # клиента, а отрицательное смещение в адресе подгрузки — это либо отказ
    # маршрута, либо молча съехавшая выдача.
    #
    # ⚠️ УСЛОВНОСТИ ПО ФАКТУ УДАЛЕНИЯ ЗДЕСЬ НЕТ И БЫТЬ НЕ ДОЛЖНО. Узел приезжает
    # ВСЕГДА, когда приехало число: условная сборка сделала бы само его наличие
    # признаком того, что удаление состоялось, и неотличимость сломалась бы с
    # другой стороны (T-09-05-06).
    repaired_offset = (
    ...
    async def _fragment() -> HTMLResponse:
        """Четыре внеполосных узла: снятие строки, снятие панели, счётчик, курсор.
        ...
        ⚠️ ЧЕТВЁРТЫЙ УЗЕЛ ЧИНИТ КУРСОР, А НЕ СТРОКУ. Строка поиска проходит тем
        же `_clean_search`, что и на странице, и уезжает в адрес сентинела тем
        же циклом проброса: иначе после удаления в отфильтрованной выдаче
        сентинел дочитал бы НЕотфильтрованный список аккаунта.
        """
        html = templates.env.get_template(
            ...
            total_groups=total_groups,
            account_id=account_id,
            sentinel_offset=repaired_offset,
            filter_params=_filter_params(_clean_search(search)),
        )
        return HTMLResponse(html)
