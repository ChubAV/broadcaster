...
async def dashboard(
    ...
    # Три счётчика сущностей отсюда сняты по D-01 — они дублировали боковое
    # меню.
    #
    # ⚠️ D-02 ОТМЕНЁН ОСОЗНАННО: плитки считают КАЛЕНДАРНЫЕ СУТКИ читателя, а
    # не скользящие 24 часа от момента запроса. Полночь вернулась, но ЛОКАЛЬНАЯ
    # — та же, по которой режет фильтр «Сегодня» в истории. Известная цена
    # решения не изменилась (в 00:10 плитка показывает почти ноль независимо от
    # того, работала система ночью), и она принята: вопрос дашборда есть
    # «сколько ушло СЕГОДНЯ», а скользящее окно смешивало вчерашний вечер с
    # сегодняшним утром и не сходилось ни с одним календарным отчётом.
    #
    # Границы строит модуль по `user`: считать их здесь значило бы завести
    # агрегатную логику вне модуля аналитики (D-35).
    metrics = await send_metrics(db, user_id=user.id, bounds=local_day_bounds(user))
