...
async def ads_images_upload(
    ...
    #
    # ⚠️ ПРИНАДЛЕЖНОСТЬ СВЕРЯЕТСЯ ДО ЛЮБОЙ РАБОТЫ, И СВЕРЯЕТ ЕЁ ТОТ ЖЕ
    # ИНСТРУМЕНТ, ЧТО И СОХРАНЕНИЕ (D-07). Список текущих ключей приходит из
    # документа, а документ клиент вправе подделать; без сверки сервер начал бы
    # переиздавать в разметку ключи, за которые сегодня отвечает только браузер.
    # Второго инструмента расчёта потолка не заводится: этот же вызов закрывает
    # и владение, и предел числа УЖЕ прикреплённых — два места, считающие
    # потолок, разойдутся молча (Hyrum).
    raw_images = form_data.getlist("images")
    string_images = [value for value in raw_images if isinstance(value, str)]
    ...
    try:
        ...
        image_keys = own_image_keys(
            [value for value in string_images if value.strip()],
            user.id,
            settings.max_images_per_ad,
        )
    except HTTPException as exc:
        # ⚠️ ИСКЛЮЧЕНИЮ НЕЛЬЗЯ ДАТЬ ДОЕХАТЬ ДО БРАУЗЕРА, И ЭТО ОСОЗНАННАЯ ЦЕНА,
        # А НЕ ОБХОД ПРАВИЛА. Сверка поднимает отказ кодом 400, а на любом
        # четырёхсотом рантайм разметки подмены не делает ВОВСЕ: человек получил
        # бы общую плашку «Действие не выполнено» вместо слов о том, что
        # случилось с его вложениями. Форма ответа у этой ветви поэтому та же,
        # что у остальных, — фрагмент полосы с кодом 200 (D-04).
        #
        ...
        # оставлял бы за собой объекты-сироты. Список ключей уходит ПУСТЫМ —
        # переиздавать в разметку нечего, а вернуть подделанное значило бы
        # сделать ровно то, против чего стоит сверка.
        ...
        # значение читает то же множество, что проверил инструмент выше, и
        # разойтись с ним не может.
        free = settings.max_images_per_ad - len(image_keys)
