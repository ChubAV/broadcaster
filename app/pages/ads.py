...
async def ads_images_upload(
    ...
    # (WR-03), и строковой операции у объекта загруженного файла нет.
    #
    # ⚠️ СВЕРКИ ПРИНАДЛЕЖНОСТИ ПРИСЛАННЫХ КЛЮЧЕЙ ЗДЕСЬ ПОКА НЕТ, И ЭТО
    # НЕДОСТАЮЩАЯ ПОЛОВИНА, А НЕ ПРИНЯТОЕ РЕШЕНИЕ. `own_image_keys` и расчёт
    # свободных мест вводит план 12-04; до него срез несёт архитектуру целиком
    # и ровно один путь по ней. Последним рубежом владения при этом остаётся
    # СОХРАНЕНИЕ объявления, которое фаза не двигает.
    image_keys = [
        value for value in form_data.getlist("images") if isinstance(value, str)
    ]
