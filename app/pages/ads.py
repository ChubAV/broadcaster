...
# ⚠️ ОТКАЗ РАЗБОРА СОСТАВНОГО ЗАПРОСА ПОДНИМАЕТ ИМЕННО ТИП STARLETTE, И РАЗНИЦА
# С `fastapi.HTTPException` ВЫШЕ НЕ СТИЛИСТИЧЕСКАЯ (план 12-09). Тип FastAPI —
# ПОТОМОК типа Starlette, а `request.form()` поднимает РОДИТЕЛЬСКИЙ
# (`starlette/requests.py`); перехват по потомку пропустил бы отказ разбора
# насквозь, и он уехал бы человеку JSON-ом с кодом 400 — ровно тем исходом,
# который этот план убирает. Имя ввозится с приставкой, потому что короткое
# `HTTPException` в этом модуле уже занято и уже ловится — вокруг записи в
# хранилище (план 12-08).
...
async def _autosave_response(
    ...
    ad: Ad | None,
    error: str | None = None,
    media_gone: list[str] | None = None,
    media_tombstone: str | None = None,
) -> HTMLResponse:
    ...
    ⚠️ `media_gone` — ПЕРЕЧЕНЬ КЛЮЧЕЙ, ОБЯЗАННЫХ ПОКИНУТЬ ДОКУМЕНТ, И ПРЕЖНЯЯ
    РЕДАКЦИЯ ЭТОГО АБЗАЦА НАЗЫВАЕТСЯ, А НЕ СТИРАЕТСЯ (Фаза 12, план 12-11,
    решение владельца **D-19**). Здесь стоял параметр `repaint_media` —
    РАЗРЕШЕНИЕ ПЕРЕРИСОВАТЬ ПОЛОСУ, — и объяснялось, почему умолчание у него
    ложно: полоса есть единственный источник списка ключей в документе,
    внеполосная подмена заменяет узел ЦЕЛИКОМ, а рисуется полоса ИЗ БАЗЫ,
    отстающей от документа на свежую плитку и на строки отказа смешанной партии
    (гап 2, план 12-07). Всё это верно и сегодня.

    ПОЧЕМУ РАЗРЕШЕНИЯ ОКАЗАЛОСЬ МАЛО. Перерисовка узла ЦЕЛИКОМ заявляет власть
    надо ВСЕМ его содержимым — включая ключ, о котором этот запрос не знал
    вовсе. Гап 3 верификации закрыт только тогда, когда ответ адресует ровно то,
    что ВИДЕЛ. Зонд WR-07 показал обе развязки дословно:
    `deleted key A resurrected in db: True` (ПОРЯДОК I) и
    `C present in removal tray: False` при `s3 calls: 2` (ПОРЯДОК II) — ключ,
    уже записанный летящей загрузкой в хранилище, выносился из документа без
    единой строки, и объект оставался сиротой.

    Поэтому ответ несёт не узел полосы, а по одному блоку АДРЕСНОГО СНЯТИЯ на
    ключ этого перечня. Непустое значение ставит ровно один вызывающий —
    сборщик фрагмента `_save_from_editor`, и ровно тогда, когда ключ этим
    запросом ДЕЙСТВИТЕЛЬНО убран. Двум прочим вызывающим (`_attachment_refusal`,
    ветвь недоступного объявления) умолчание подходит по существу: состава
    вложений не менял ни один из них.

    ⚠️ ПОРЯДОК I ЭТИМ ПАРАМЕТРОМ НЕ ЗАКРЫТ И ЗАКРЫТ ИМ БЫТЬ НЕ МОЖЕТ: ответ
    загрузки, пришедший последним, переиздаёт снимок целиком. Грань закрывает
    план 12-12 (метка убирания и вычитание на сохранении) — названо здесь
    затем, чтобы адресное снятие не прочиталось как закрытие гапа целиком.

    ⚠️ `media_tombstone` — ВЫЧИТАЮЩАЯ МЕТКА УБИРАНИЯ, И ПОЧЕМУ ОДНОГО ПЕРЕЧНЯ
    НА СНЯТИЕ ОКАЗАЛОСЬ МАЛО, ВЫПИСАНО ЗДЕСЬ, А НЕ ОСТАВЛЕНО ЧИТАТЕЛЮ (Фаза 12,
    план 12-12, решение владельца **D-19**). Снятие правит ЭКРАН, а воскресение
    происходит в БАЗЕ, и между ними лежит запрос, который документ отправляет
    САМ: ответ загрузки несёт `HX-Trigger-After-Swap`, форма объявления слушает
    событие `from:body` и немедленно уходит автосохранением — с тем снимком,
    который тот же ответ и переиздал. Замер верификатора дословно:
    `deleted key A resurrected in db: True`.

    Помнить действие человека обязан ДОКУМЕНТ, потому что сервер различить
    «только что загруженный» и «воскресший убранный» по одному снимку НЕ МОЖЕТ:
    оба вида ключа приходят в снимке и обоих нет в базе. Метка кладётся ВНЕ цели
    подмены формы загрузки и переживает переиздание снимка; сохранение вычитает
    помеченные ключи ДО сверки владения.

    Отвергнутое владельцем названо по идентификатору: (а) общая очередь двух
    форм редактора заставила бы «×» ждать декодирования, пережатия и двух кругов
    к хранилищу, а вытесняющая очередь уронила бы средний запрос — то есть цена
    выросла бы до МОЛЧА НЕИСПОЛНЕННОГО УДАЛЕНИЯ; (б) запись допущением формы
    D-16/D-17 отвергнута прямо — задетый запрет плана 12-07 владелец закрывает
    КОДОМ.
    """
    return templates.TemplateResponse(
        request,
        "ads/includes/autosave_response.html",
        {
            ...
            "media_gone": media_gone or [],
            "media_tombstone": media_tombstone,
        },
    )
...
async def _save_from_editor(
    ...
    # ⚠️ ВЫЧИТАЮЩИЕ МЕТКИ УБИРАНИЯ (Фаза 12, план 12-12, решение владельца
    # D-19). Документ помнит действие человека дольше, чем живёт отстающий
    # снимок: ответ загрузки, пришедший ПОСЛЕДНИМ, переиздаёт снимок скрытых
    # полей, снятый ДО убирания, и без этих меток убранный ключ вернулся бы в
    # `ad.images` (замер зонда WR-07: `deleted key A resurrected in db: True`).
    #
    # Строковые значения отбираются ТОЙ ЖЕ формой, что и у поля вложений выше.
    # Отказа, в отличие от него, здесь нет и быть не должно: поле только
    # ВЫЧИТАЕТ, и власти уронить сохранение человека у него нет (T-12-12-03).
    removal_marks = {
        value
        for value in form_data.getlist("removed_images")
    ...
    try:
        ...
        snapshot = [v for v in string_images if v.strip()]
        # Воскресшие: значения снимка, которые человек уже убирал. Им предстоит
        # покинуть документ — и покинуть его ТЕМ ЖЕ ответом, который отказал им
        # в записи, иначе человек остался бы с плиткой, за которой нет ключа.
        resurrected = [v for v in dict.fromkeys(snapshot) if v in removal_marks]
        # ⚠️ ПОРЯДОК НЕСУЩИЙ: ВЫЧЕСТЬ, ПОТОМ СВЕРЯТЬ. Отказ по ДЛИНЕ стои́т в
        # `own_image_keys` ПЕРВЫМ, поэтому вычитание ПОСЛЕ сверки означало бы,
        # что воскресший чужой гонкой ключ переполняет потолок — и человек
        # получает отказ 400 за гонку, которой не устраивал: он убрал вложение,
        # освободив место, а сервер отвечает «вложений больше N».
        #
        # Вычитание не трогает ни предиката принадлежности, ни потолка:
        # `own_image_keys` остаётся ЕДИНСТВЕННЫМ местом, где решается «мой ли
        # ключ» и где потолок отказывает (D-07, Hyrum). Метка лишь исключает
        # значения, которые ТОТ ЖЕ запрос и прислал.
        subtracted = [v for v in snapshot if v not in removal_marks]
    ...
    async def _fragment() -> HTMLResponse:
        ...
        ⚠️ ОТВЕТ УНОСИТ ИЗ ДОКУМЕНТА РОВНО ТО, ЧТО УБРАЛ ЭТОТ ЗАПРОС, И НИЧЕГО
        БОЛЬШЕ (план 12-11, D-19). Перечень собирается из ОДНОГО убранного
        ключа — по ФАКТУ убирания, снятому выше, — и пуст, когда состав
        вложений не изменён. Прежде здесь передавалось РАЗРЕШЕНИЕ перерисовать
        полосу (`repaint_media=media_changed`, план 12-07), и разрешения
        оказалось мало: перерисовка узла целиком уносила и ключи, которых этот
        запрос не видел. Основание — докстринг `_autosave_response`.

        ⚠️ ПЕРЕЧЕНЬ СТАЛ ОБЪЕДИНЕНИЕМ ДВУХ ПОЛОВИН (план 12-12), И ОБЕ НАЗВАНЫ.
        Первая — ключ, убранный ЭТИМ запросом. Вторая — ВОСКРЕСШИЕ: значения
        снимка, которые человек уже убирал и которые вернул в документ ответ
        загрузки, пришедший последним. Обе половины покидают документ ОДНИМ
        путём: плитка, за которой нет ключа, уходит с экрана тем же ответом,
        который отказал ей в записи, а не ждёт перезагрузки (T-12-12-05).

        ⚠️ МЕТКА ПЕЧАТАЕТСЯ ТОЛЬКО ДЛЯ КЛЮЧА, УБРАННОГО ЭТИМ ЗАПРОСОМ, И ЭТО НЕ
        НЕДОСМОТР. У воскресшего метка в документе УЖЕ стои́т — она и есть
        причина, по которой он не попал в запись, — и вторая была бы дублем,
        растущим на каждом круге автосохранения.
        """
        response = await _autosave_response(
            ...
            saved,
            media_gone=(
                resurrected + ([removed] if media_changed else [])
            ),
            media_tombstone=removed if media_changed else None,
        )
        if created:
...
async def ads_images_upload(
    ...
    ⚠️ У МАРШРУТА ЕСТЬ СВЕРКА ИСТОЧНИКА, И НАЗВАТЬ НАДО, ЧЕМ ЕЁ ОТСУТСТВИЕ БЫЛО
    ПРЕЖДЕ (решение владельца D-15, находка ревью WR-03). Не забывчивостью:
    отсутствие было УНАСЛЕДОВАНО от снятого JSON-маршрута загрузки, где гарды не
    было ни у одного соседа, и при переезде в страничный слой (D-01) заново на
    поверхности, где идиома гарды уже существует и стоит у восьми модулей, оно
    принято НЕ БЫЛО. Владелец принял этот вопрос заново и с обратным исходом.
    Закрываемое воздействие называется прямо, а не словом «CSRF»: аутентификация
    идёт кукой, составной запрос есть «простой» запрос перекрёстного
    происхождения, браузер прикладывает куку к форме чужой страницы САМ — и
    чужая страница писала объекты в префикс жертвы, повторяемо, до потолка тела
    за запрос. Ответ 403 отдаётся БЕЗ тела: причина отказа чужому источнику не
    сообщается, как и у соседа в этом же модуле.
    """
    user = await get_user_from_cookie(request, db, settings)
    if not user:
        return await respond(request, redirect="/login")

    # СВЕРКА ИСТОЧНИКА (решение владельца D-15, находка ревью WR-03). ГДЕ ОНА
    # СТОИ́Т — сказано ОДИН РАЗ НА ПРОЕКТ, константой `ORIGIN_CHECK_BOUNDARY`
    # (`app/pages/common.py`); текст канона здесь НЕ ПОВТОРЯЕТСЯ, площадка
    # заведена сразу ССЫЛКОЙ (идиома плана 10-32, `WR-02` ревизии 2026-09-07).
    #
    # ⚠️ ПОРЯДОК ОБЯЗАТЕЛЕН ПО ДВУМ ПРИЧИНАМ СРАЗУ, И ВТОРАЯ ЗДЕСЬ СОБСТВЕННАЯ.
    # Первая — канон: сверка стои́т ПОСЛЕ прав и ДО чтения ПРЕДМЕТА. Вторая —
    # чем предмет является ЗДЕСЬ: им является ТЕЛО ЗАПРОСА. Разбор составного
    # запроса ниже (`request.form`) вычитывает поток целиком и складывает каждую
    # файловую часть во временный файл с буфером в памяти ДО первой строки
    # логики; сверка, поставленная под ним, отдавала бы межсайтовому отправителю
    # тот же 403 — оплатив ему буферизацию всей партии до потолка тела на
    # обратном прокси. Отказ выше разбора не оплачивает ничего.
    ...
    #
    # ⚠️ ЧТО ЭТОТ ПРЕДЕЛ БЕРЕЖЁТ НА САМОМ ДЕЛЕ — НАЗВАНО ВЕЛИЧИНОЙ, А НЕ СЛОВОМ
    # «ПАМЯТЬ» (план 12-10, находка ревью WR-01). Он бережёт НЕ память воркера
    # от первой же части: к моменту, когда предел мог бы сработать, разборщик
    # уже принял всё, что принял. Он ограничивает ЧИСЛО ЧАСТЕЙ, КОТОРОЕ
    # РАЗБОРЩИК СОГЛАСИТСЯ РАЗОБРАТЬ, — то есть верхнюю границу числа временных
    # файлов, заведённых одним запросом. Практическая граница на один запрос
    # есть ПРОИЗВЕДЕНИЕ предела числа частей на размер буфера временного файла
    # (`SpooledTemporaryFile`, `starlette/formparsers.py`: буфер в памяти, далее
    # сброс на диск), и верхним рубежом НАД ней стои́т потолок тела запроса на
    # обратном прокси — единственный предел, действующий ДО чтения.
    #
    # ⚠️ У ПРЕДЕЛА ПОЯВИЛСЯ ЧИТАЕМЫЙ ОТКАЗ, И НАЗВАТЬ НАДО, ЧЕМ ОН БЫЛ ПРЕЖДЕ
    # (план 12-09, находка ревью WR-06). Это ЕДИНСТВЕННАЯ защита, заведённая
    # фазой новой, и до этого плана она была единственным отказом, которого
    # человек прочесть не мог: разборщик поднимал `HTTPException(400)`, каркас
    # рисовал его JSON-ом `{"detail": …}` на HTML-поверхности, а на четырёхсотом
    # рантайм разметки подмены не делает ВОВСЕ — человек, бросивший в редактор
    # папку со снимками, читал общую плашку «Действие не выполнено». Ровно тот
    # исход, ради устранения которого D-04 заплатил лживым кодом 200.
    #
    # ⚠️ ОТВЕТ ЭТОЙ ВЕТВИ ДОПИСЫВАЕТ СТРОКУ, А НЕ ПОДМЕНЯЕТ ПОЛОСУ, И ЭТО УРОК
    # ПЕРВОГО ГАПА, ПРИМЕНЁННЫЙ К НОВОЙ ВЕТВИ, А НЕ ПОВТОРЁННЫЙ В НЕЙ (CR-02,
    # план 12-06). Разбор составного запроса НЕ СОСТОЯЛСЯ: ни одного поля
    # `images` сервер не прочитал, и текущих ключей вложений он НЕ ЗНАЕТ.
    # Ответить полосой значило бы ответить полосой ПУСТОЙ — подмена `innerHTML`
    # вынесла бы из документа все скрытые поля, и следующее автосохранение
    # записало бы объявление без вложений. Заметьте разницу с ветвью отказа
    # сверки ключей ниже: ТАМ полоса пересобирается из ПОДТВЕРЖДЁННОГО
    # подмножества, потому что разбор состоялся и подмножество известно; ЗДЕСЬ
    # подтверждать нечего, и единственный честный ответ — дописать строку.
    # Дописывание оставляет и полосу, и её скрытые поля нетронутыми, а строка
    # ложится СОСЕДОМ узла `#media-tray`, поэтому её не уносит и внеполосная
    # перерисовка полосы ответом автосохранения. Снимается строка сама:
    # следующая удачная загрузка подменяет содержимое цели целиком.
    #
    # ⚠️ ЛОВИТСЯ ОТКАЗ РАЗБОРА ЦЕЛИКОМ, А НЕ ОДИН ЕГО ВИД, И ГРАНИЦА
    # НАЗЫВАЕТСЯ ЧЕСТНО. Starlette сводит ВСЕ отказы разбора составного запроса
    # к одному `HTTPException(400)` (`starlette/requests.py`), различая их лишь
    # текстом; разбирать этот текст значило бы завести второе объявление
    # английских сообщений чужой библиотеки и молча замолчать в день, когда их
    # перепишут. Из интерфейса достижим ровно один вид — превышение числа
    # частей: браузер шлёт запрос формы, у которой границы и имена частей на
    # месте по построению. Прочие виды (потерянная граница, часть без имени)
    # означают сломанного клиента, и ему тот же ответ хуже не делает.
    try:
        form_data = await request.form(
        ...
        async def _parts_refusal_fragment() -> HTMLResponse:
            """ОДНИ строки отказа: полосы у этой ветви нет и быть не может.

            Печатает их ЕДИНСТВЕННЫЙ источник разметки строки отказа — тот же,
            что включает полоса (план 12-09). Второй копии разметки не
            появляется: она разошлась бы с первой молча, и человек читал бы
            РАЗНОЕ в зависимости от того, на какой ветви его отвергли.

            Имени файла у отказа нет: разбор не состоялся, и сервер не знает
            имени ни одной из присланных частей. Отказ относится к ЗАПРОСУ
            целиком — приписать его какому-то одному файлу было бы неправдой.
            """
            response = templates.TemplateResponse(
                request,
                "ads/includes/media_refusals.html",
                {
                    "refusals": [
                        Rejected(
                            display_name="",
                            reason=upload_parts_message(MAX_UPLOAD_PARTS),
                        )
                    ]
                },
            )

            # СПОСОБ ПОДМЕНЫ — «ДОПИСАТЬ В КОНЕЦ», И ЦЕЛЬ ПРИ ЭТОМ НЕ
            # ПЕРЕНАЗНАЧАЕТСЯ: она по-прежнему объявлена разметкой формы.
            # Меняется РОВНО способ, и основание выписано абзацем выше.
            # Применение объявлено ПОИМЁННО в перечне точечных заголовков
            # (`tests/test_pages/test_htmx_gates.py`, D-16): заголовок отменяет
            # решение шаблона из ответа, и незаписанное применение сделало бы
            # поведение формы нечитаемым по её разметке.
            #
            # Заголовка события здесь нет намеренно: не принят ни один файл, и
            # просить форму объявления сохраниться было бы кругом к серверу за
            # работу, которой не было.
            response.headers["HX-Reswap"] = "beforeend"
            return response

        # Адрес деградации — тот же `/ads/new`, что у остальных ветвей: маршрут
        # привязан к ПОЛЬЗОВАТЕЛЮ, а не к черновику (D-02), и второй адрес
        # здесь был бы вторым ответом на вопрос, у которого ответ один.
        return await respond(
            request, redirect="/ads/new", fragment=_parts_refusal_fragment
        )
    ...
    if offending or len(string_images) != len(raw_images):
        ...
        # оставлял бы за собой объекты-сироты. Запрет плана 12-06 в силе
        # ДОСЛОВНО, и ниже он не ослаблен ни на символ.
        #
        # ⚠️ ПОКОЛЕНИЕ АБЗАЦА (идиома D-30/D-32 — прежняя редакция называется, а
        # не стирается; план 12-13, находка ревью WR-08, решение владельца
        # D-20). Здесь стояло: «Строка отказа ОДНА и без имени файла: отказ
        # относится к ключу, а файла за ним нет». ПЕРВАЯ половина — отнесение
        # отказа к КЛЮЧУ — верна и сохранена работой: безымянная строка ниже
        # приходит по-прежнему и говорит именно о ключе. ВТОРАЯ снята, и вот
        # причина снятия: файлы за запросом ЕСТЬ. Человек только что выбрал их в
        # системном диалоге, диалог закрылся, запрос ушёл — а ветвь пропускала
        # цикл по файловым частям ЦЕЛИКОМ, и единственная строка говорила о
        # КЛЮЧЕ. Молчание о файлах превращало отказ по ОДНОМУ значению скрытого
        # поля в бесследное исчезновение ВСЕЙ партии: ни плиток, ни имён, ни
        # признака «принят хотя бы один файл», поэтому ни автосохранения, ни
        # события — ни единого второго сигнала.
        #
        # ⚠️ ВЕТВЬ ДОСТИЖИМА БЕЗ НАПАДАЮЩЕГО, И ИМЕННО ЭТО СДЕЛАЛО МОЛЧАНИЕ
        # НЕТЕРПИМЫМ. Речь об остатке «б», выписанном абзацем ниже: ключ СТАРОГО
        # образца, лежащий в `Ad.images` ещё с поры до нормализации имён,
        # приезжает сюда скрытым полем наравне с подделанным и сегодняшнего
        # образца не проходит. В этом состоянии у человека не подделанный
        # запрос, а ЛЕГАСИ-ВЛОЖЕНИЕ, и каждая его попытка загрузки была
        # молчаливым no-op до перезагрузки страницы.
        #
        # ЧТО ИЗ ЭТОГО СЛЕДУЕТ И ЧТО НЕ СЛЕДУЕТ. Разделяются ДВА разных
        # утверждения: «это значение не ваше» — и «поэтому остальное в запросе
        # не считается». Первое остаётся: виновное значение во фрагмент не
        # попадает, ветвление по предикату принадлежности не тронуто. Второе
        # снимается: выбор человека называется ПОИМЁННО. Рамку партии расширил
        # ВЛАДЕЛЕЦ решением D-20, а не план, и перечень остающегося вне её
        # (IN-07…IN-10) назван там же поимённо.
        refusals.append(
            Rejected(display_name="", reason=INACCESSIBLE_IMAGE_MESSAGE)
        )

        # ⚠️ ГЛАВНОЕ СВОЙСТВО ЭТОГО ЦИКЛА — ТО, ЧЕГО ОН НЕ ДЕЛАЕТ. Содержимого
        # частей он НЕ ЧИТАЕТ и сервиса записи НЕ ЗОВЁТ: число обращений к
        # хранилищу на этой ветви остаётся НУЛЁМ, и это утверждается тремя
        # правилами суиты. Громкость отказа не покупается ни одной записью —
        # иначе подделанный ключ снова оставлял бы за собой объекты-сироты.
        # Части к этой строке уже разобраны рантаймом запроса во временные файлы
        # (`starlette/formparsers.py`), поэтому перебор их ИМЁН новой работы не
        # заводит вовсе; цикл ниже читает `part.filename`, и больше ничего.
        #
        # Нового текста для человека здесь не заводится: тексты отказа — закрытый
        # набор констант, и вторая формулировка ТОЙ ЖЕ причины разошлась бы с
        # первой молча. Имя проходит `safe_filename` — та же нормализация, что на
        # прочих ветвях отказа (D-05): сырое имя подконтрольно отправителю, и
        # одного автоэкранирования шаблонизатора мало.
        #
        # Строковая часть под файловым именем пропускается ТОЙ ЖЕ формой, что и в
        # действующем цикле ниже. Находка ревью IN-08 («строковая часть под
        # файловым полем пропускается молча») остаётся ОТКРЫТОЙ ЗАПИСЬЮ и этой
        # правкой не чинится: рамка партии расширена владельцем ровно на WR-08.
        #
        # Признак «принят хотя бы один файл» здесь не трогается и остаётся
        # ложью: принятых файлов нет, и просить форму объявления сохраниться было
        # бы кругом к серверу за работу, которой не было.
        ...
        #
        # ⚠️ ПОКОЛЕНИЕ ОПИСАНИЯ ИСХОДА (идиома D-30/D-32; план 12-13, находка
        # ревью WR-08, решение владельца D-20). Здесь стояло: «Исход прямой:
        # плитка исчезает, строка отказа приходит без имени файла, следующее
        # сохранение записывает объявление без такого ключа». Первая и третья
        # трети ВЕРНЫ и сохранены. СРЕДНЯЯ теперь неправда, и неправда она
        # ровно наполовину: безымянная строка про КЛЮЧ приходит по-прежнему, но
        # РЯДОМ С НЕЙ стои́т строка на КАЖДЫЙ выбранный человеком файл — с
        # именем, прошедшим нормализацию (ветвь отказа сверки выше). Что
        # изменилось: исход перестал быть МОЛЧАЛИВЫМ. Прежде человек, у которого
        # в записи лежит ключ старого образца, выбирал файлы, закрывал диалог и
        # не видел на экране НИ ОДНОГО слова о них — каждая попытка загрузки
        # была no-op до перезагрузки страницы.
        #
        # ⚠️ САМ ОСТАТОК ПРИ ЭТОМ НЕ ПОЧИНЕН, И ЭТО ГЛАВНОЕ, ЧТО НАДО ЗНАТЬ
        # ЧИТАТЕЛЮ. Ключ старого образца по-прежнему отцепляется, состояние
        # по-прежнему достижимо, и выход из него по-прежнему один — обновить
        # страницу и добавить изображение заново (фраза текста отказа работает
        # здесь ДОСЛОВНО). Партия изменила ГРОМКОСТЬ исхода, а не достижимость
        # состояния; записано это допущением `accepted-assumption` в
        # `deferred-items.md` фазы 12. Три причины не чинить остаток, выписанные
        # ниже, остаются в силе ДОСЛОВНО.
        #
        # Чем это было ДО партии: сверка на сохранении роняла КАЖДОЕ автосохранение
        ...
        # потолку, и он избегает ДЕКОДИРОВАНИЯ, ПЕРЕЖАТИЯ и ДВУХ ЗАПИСЕЙ В
        # ХРАНИЛИЩЕ на каждую такую часть: делать работу, результат которой
        # заведомо некуда деть, не следует. Отказ партии целиком был бы и сменой
        # поведения, и лишней работой человеку — ему пришлось бы выбирать файлы
        # заново, зная, сколько мест осталось.
        #
        # ⚠️ ПОКОЛЕНИЕ АБЗАЦА (идиома D-30/D-32; план 12-10, находка ревью
        # WR-01). Здесь стояло «получают отказ по потолку БЕЗ ЧТЕНИЯ ТЕЛ».
        # Основание («работа, результат которой заведомо некуда деть, не
        # делается») ВЕРНО и сохранено; неверен был ПЕРЕЧЕНЬ сэкономленного.
        # Чтения отказ не избегает: тело к этой строке УЖЕ ПРОЧИТАНО И
        # РАЗОБРАНО — `request.form()` выше вычитал поток запроса целиком и
        # сложил каждую файловую часть во временный файл с буфером в памяти
        # (`starlette/formparsers.py`) ДО первой строки этого цикла. Цикл
        # перебирает УЖЕ РАЗОБРАННЫЕ части, а не читает их с провода.
        # ЕДИНСТВЕННЫЙ предел ДО чтения — потолок тела запроса на обратном
        # прокси (`nginx/*.conf.template`, сверяется правилом
        # `tests/test_nginx_body_limit.py`); ни `max_images_per_ad`, ни
        # `max_image_size_mb` рубежом чтения не являются и никогда им не были на
        # ЭТОМ транспорте.
        for part in form_data.getlist(UPLOAD_FILE_FIELD):
            if isinstance(part, str):
                continue
