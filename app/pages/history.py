...
RETRY_QUEUED = "queued"
RETRY_GONE = "gone"
RETRY_ACCESS_CLOSED = "access_closed"
RETRY_BUSY = "busy"

# Тексты ПОСТОЯННЫЕ и из ответа гейта не собираются. Гейт возвращает причину
# строкой, и провоз этой строки через адрес означал бы, что владелец ссылки
# печатает пользователю произвольный текст на его собственной странице.
RETRY_NOTICES = {
    RETRY_QUEUED: (
    ...
    # ⚠️ КОД И ТЕКСТ ПЕРЕЦЕЛЕНЫ ВМЕСТЕ С ПРЕДМЕТОМ ВОПРОСА, А НЕ ПОДПРАВЛЕНЫ.
    # Гейт повтора спрашивает вердикт ДОСТУПА с плана `05.1-03`, а строка
    # осталась от снятой валюты сообщений и звала пополнить величину, которой
    # больше не существует. Совет, который невозможно выполнить, хуже
    # отсутствующего: человек уходит искать несуществующий экран пополнения
    # вместо того, чтобы оплатить доступ. Само ИМЯ кода тоже сменено: прежнее
    # адресовало снятый предмет, и старая ссылка с ним теперь не печатает
    # ничего — ровно то, чего требует закрытое множество.
    RETRY_ACCESS_CLOSED: (
        "Повторить не удалось: доступ к системе закрыт. Оплатите подписку — "
        "и повтор станет доступен.",
        "warning",
    ),
    # Под этот ключ попадают ТРИ случая, а не два: первый повтор уже поставлен и
    # обработчик давно завершился; первый повтор ещё выполняется и дойдёт до
    # очереди; первый повтор ещё выполняется и будет ОТВЕРГНУТ — черновиком,
    # выключенной группой, отвалившимся аккаунтом или закрытым доступом.
    # Удержание открывается ДО этих проверок, поэтому третий случай достижим, и
    # в нём в очередь не уходит НИЧЕГО.
    #
    # Поэтому текст описывает то, что окно ДЕЙСТВИТЕЛЬНО гарантирует, — отказ
    # второму нажатию, — а обещание новой записи истории даётся УСЛОВНО.
    # Прежняя редакция утверждала постановку безусловно: пользователь третьего
    # случая ждал бы записи, которой не будет. Правдоподобная неправда хуже
    # бесполезной правды (P-04-01).
    RETRY_BUSY: (
