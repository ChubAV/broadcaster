...
RETRY_NOTICES = {
    ...
    # Под этот ключ попадают ОБА случая: и первый повтор, который ещё
    # выполняется, и второе нажатие, когда первый повтор уже поставлен, а
    # обработчик давно завершился. Текст поэтому говорит о состоянии ЗАДАЧИ, а
    # не обработчика: «уже выполняется» во втором случае было бы неправдой о
    # состоянии системы.
    RETRY_BUSY: (
        "Повтор этой отправки уже поставлен в очередь — второй раз он не "
        "уйдёт. Результат появится новой записью истории.",
        "info",
    ),
}
...
async def history_retry(
    ...
    отказных путях и при исключении оно снимается сразу — повтор, не дошедший до
    очереди, окна за собой не оставляет. На успешном пути окно ПЕРЕЖИВАЕТ ответ,
    поэтому второе нажатие в его пределах второй задачи не ставит: именно это, а
    не панель подтверждения, держит защиту при выключенном JavaScript. Обе
    границы окна (оно истекает; оно живёт в памяти одного процесса) выписаны у
    объявления реестра.
