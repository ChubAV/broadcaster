...
# ⚠️ ТРИ МАРШРУТА ЭТОГО ФАЙЛА ЗАКРЫТЫ ПОД ЧУЖОЙ ЛИЧНОСТЬЮ, ОСТАЛЬНЫЕ ОТКРЫТЫ, И
# ГРАНИЦА ЗДЕСЬ НЕ ОЧЕВИДНА — поэтому она выписана. Админка из-под имперсонации
# РАБОТАЕТ: права читаются по действующему лицу (D-20), иначе администратор
# оказался бы заперт в чужой учётной записи без пути назад. Значит все кнопки
# админки доступны человеку, находящемуся под чужой личностью, и решение по
# каждой нужно принимать отдельно.
# — `admin_delete_user`: удаление НЕОБРАТИМО (D-22), откатом кода пользователь
#   не возвращается;
# — `admin_toggle_free_access`: это ДЕНЬГИ, а весь денежный путь под чужой
#   личностью запрещён (D-22); администратору достаточно сперва вернуться к
#   себе — одно нажатие;
# — `admin_impersonate`: вложенный вход завёл бы цепочку личностей, которой
#   продукт не предусматривает.
# Открытыми осознанно оставлены перезапуск воркера, снятие задачи из очереди и
# блокировка: все три — административные операции ДЕЙСТВУЮЩЕГО ЛИЦА, обратимые
# и записываемые в журнал под его идентификатором, а не под чужим.
...
# ⚠️ ЧАСТНОГО МНОЖЕСТВА ПРИЧИН ОТКАЗА ПЕРЕЗАПУСКА ЗДЕСЬ БОЛЬШЕ НЕТ. Правило,
# ради которого оно заводилось, в силе целиком: кнопку жмут В АВАРИИ, и по её
# результату решают, идти ли на сервер руками, — молчащий отказ здесь дороже,
# чем где-либо ещё. Изменился только владелец слов: обе строки переехали
# посимвольно в закрытый реестр `app/pages/notices.py`, общий на весь продукт.
# Вместе с ними переехал и запрет на текст стороннего исключения — он ничего не
# сообщает администратору и способен вынести наружу внутренние пути и адреса.
...
async def admin_dashboard(
    ...
    ⚠️ ОКНО ОШИБОК — СУТКИ, А НЕ ЧАС ИЗ МАКЕТА (D-40). Дельта приезжает тем же
    обращением к базе. Острые всплески ловит блок инцидентов — у него на это
    свой признак с окном в час (D-51).

    ⚠️ ОКНО ЗДЕСЬ СКОЛЬЗЯЩЕЕ, И ЭТО РАСХОДИТСЯ С ПОЛЬЗОВАТЕЛЬСКИМ ДАШБОРДОМ
    НАМЕРЕННО (Q3). Дашборд считает КАЛЕНДАРНЫЕ сутки своего читателя, от его
    локальной полуночи; «Обзор» сохраняет скользящие 24 часа от момента
    запроса. Совпадения цифр между двумя экранами больше нет и не должно быть:
    они отвечают на РАЗНЫЕ вопросы — операционный «что происходит по сервису
    последние сутки» против личного «сколько ушло сегодня». Календарные сутки у
    общесистемного счёта не определены вовсе: у сервиса нет одного читателя, по
    чьей зоне резать полночь.
    """
    now = datetime.now(timezone.utc)

    users = await user_totals(db, now=now)
    paying = await paying_total(db, now=now)
    # `now` берётся снятый строкой выше: второй снимок часов развёл бы окно
    # плиток с окном инцидентов на время исполнения запросов между ними.
    metrics = await send_metrics(
        db, user_id=None, bounds=sliding_window_bounds(now=now)
    )
    ...
    return templates.TemplateResponse(
        "admin/workers.html",
        {
            ...
            **await _workers_view(db),
            # ⚠️ ИСХОД ПЕРЕЗАПУСКА ОТСЮДА НЕ УХОДИТ ВОВСЕ, И ЭТО НЕ ПОТЕРЯ.
            # Обработчик перезапуска пишет КОД в адрес, а рисует его общая
            # область шелла по закрытому реестру. Второй ключ контекста был бы
            # вторым местом отрисовки одного и того же — то есть двумя плашками
            # об одной беде, расходящимися при первой правке.
        },
    )
...
async def admin_toggle_free_access(
    ...
    # МАРШРУТОВ АДМИНКИ (перезапуск воркера, снятие задачи, вход под
    # пользователем): после проверки прав, ДО любого чтения. Аутентификация
    # проекта идёт cookie, поэтому браузер приложит её к межсайтовой форме сам,
    # и без сверки источника единственным, что стоит между сторонней страницей
    # и этим действием, остаётся умолчание `samesite="lax"` — одна политика
    # браузера там, где проект в трёх соседних местах требует явной серверной
    # проверки. Чужому источнику причина отказа не сообщается.
    if not is_same_origin(request):
        return Response(status_code=403)
...
async def admin_impersonate(
    ...
    ⚠️ ВЛОЖЕННЫЙ ВХОД ОТВЕРГАЕТСЯ ЗАВИСИМОСТЬЮ ЗАПРЕТА, СТОЯЩЕЙ НА ЭТОМ ЖЕ
    ОБРАБОТЧИКЕ (D-22). Цепочку личностей формат токена допускает — claim `act`
    просто перезаписался бы, — а продукт не поддерживает: полоса возврата
    назвала бы одного пользователя, возврат привёл бы к другому, а журнал
    записал бы пару, которой не было. Разрешить это молча значило бы завести
    состояние, о котором не думал никто. Отказ ничего не отнимает:
    администратор возвращается к себе одним нажатием и входит заново.
...
async def admin_toggle_block(
    ...
    """Закрыть или открыть пользователю доступ в продукт (критерий 2 фазы).

    ⚠️ ГАРД ПРОИСХОЖДЕНИЯ ОБЯЗАТЕЛЕН И ПОСТАВЛЕН ПОЗЖЕ САМОГО МАРШРУТА
    (CR-02 ревизии фазы 6). Блокировка запирает ПЛАТЯЩЕГО клиента снаружи
    продукта, то есть по последствию стоит рядом с удалением, а не рядом с
    тумблером показа. Пока гарда не было, три самых разрушительных действия
    раздела оказывались единственными изменяющими входами админки БЕЗ сверки
    источника — при том что три соседних, написанных этой же фазой, её несут.
    """
    # ГАРД ПРОИСХОЖДЕНИЯ — ТОТ ЖЕ И В ТОЙ ЖЕ ПОЗИЦИИ, ЧТО У СОСЕДНИХ ИЗМЕНЯЮЩИХ
    # МАРШРУТОВ АДМИНКИ (перезапуск воркера, снятие задачи, вход под
    # пользователем): после проверки прав, ДО любого чтения. Аутентификация
...
async def admin_delete_user(
    ...
    """УДАЛИТЬ учётную запись пользователя — необратимо.

    ⚠️ САМОЕ РАЗРУШИТЕЛЬНОЕ ДЕЙСТВИЕ РАЗДЕЛА, И ГАРД ПРОИСХОЖДЕНИЯ ЗДЕСЬ НЕ
    ПЕРЕСТРАХОВКА (CR-02 ревизии фазы 6). Соседний маршрут выписывает токен
    имперсонации и гард несёт — с доводом «без гарда сторонняя страница выписала
    бы себе токен от имени вошедшего администратора»; этот маршрут УДАЛЯЕТ того
    же пользователя целиком и до сих пор не нёс ничего. Асимметрия была
    самопротиворечивой, а не осознанной: обработчик правился этой же фазой
    (добавлена зависимость запрета под чужой личностью), и гард не был добавлен
    заодно.
    """
    # ГАРД ПРОИСХОЖДЕНИЯ — ТОТ ЖЕ И В ТОЙ ЖЕ ПОЗИЦИИ, ЧТО У СОСЕДНИХ ИЗМЕНЯЮЩИХ
    # МАРШРУТОВ АДМИНКИ (перезапуск воркера, снятие задачи, вход под
