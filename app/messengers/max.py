...
def get_max_endpoint(account_id: int) -> str | None:
    """Get the HTTP endpoint for a max-worker container from Redis."""
    import redis as redis_lib
    from app.config import get_settings
    settings = get_settings()
    r = redis_lib.from_url(settings.redis_url)
    try:
        endpoint = r.get(f"max:endpoint:{account_id}")
        return endpoint.decode() if endpoint else None
    finally:
        r.close()


def ensure_max_container(account_id: int) -> str | None:
    """Start max-worker container if not running, wait for it to be ready, return endpoint."""
    from app.services.max_container_manager import start_container, wait_for_container_ready
    endpoint = get_max_endpoint(account_id)
    if endpoint:
        return endpoint
...
class MaxMessenger(BaseMessenger):
    def __init__(self, bridge_url: str | None = None, session_id: str = ""):
        ...
        self.log = logger.bind(messenger="max", session_id=session_id)

    @property
    def bridge_url(self) -> str:
        if self._bridge_url:
            return self._bridge_url
        endpoint = ensure_max_container(int(self.session_id))
        if not endpoint:
            raise RuntimeError(f"Cannot start max-worker for account {self.session_id}")
