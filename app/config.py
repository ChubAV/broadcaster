...
class Settings(BaseSettings):
    ...
    # ИСТОЧНИК ЛОГОВ ДЛЯ ПОДРАЗДЕЛА «ЛОГИ» АДМИН-ПАНЕЛИ (D-27, D-28).
    #
    # ⚠️ УМОЛЧАНИЕ ОБЯЗАТЕЛЬНО, И ОНО ЖЕ — ГЛАВНОЕ, ЧТО ЗДЕСЬ ЕСТЬ. Мониторинг
    # проекта ОПЦИОНАЛЕН: боевые команды запуска и выката его не поднимают, а
    # его сеть подключена как внешняя — то есть этот адрес разрешается только
    # при поднятом мониторинге. Поле без умолчания требовало бы переменной
    # окружения ради службы, которой в бою может не быть вовсе, и её отсутствие
    # роняло бы сборку настроек — то есть ВЕСЬ веб-процесс, а не подраздел.
    #
    # Умолчание указывает на службу в общей сети `broadcaster`; переопределение
    # приходит из окружения, без выката кода — форма, применённая в проекте ко
    # всем внешним адресам.
    #
    # Недостижимость этого адреса — штатная ветка, а не отказ: она возвращается
    # отдельным полем результата и рисуется плашкой с командой подъёма
    # мониторинга (`app/services/loki_client.py`).
