...
class Settings(BaseSettings):
    ...
    #
    # ЭТО МАШИННАЯ СТРОКА ФОРМАТА ЮKASSA, А НЕ ПОДПИСЬ МАКЕТА. Правило то же,
    # что у `plan_limits` выше, и повторено здесь намеренно: значение уезжает
    ...
    # знак рубля и запятая запрещены здесь так же, как в соседнем прейскуранте.
    #
