...
> Все пять вопросов закрыты при планировании фазы. Решение по каждому живёт в разделе
> «Decisions resolved in this plan» названного плана — там же обоснование и цена альтернативы.
> Рекомендация исследования не всегда совпала с решением: вопрос 5 решён ПРОТИВ рекомендации, и
> это отмечено в строке ниже, а не сглажено.

| # | Вопрос | Решение | Где решено |
|---|--------|---------|------------|
| 1 | `[data-longtext]` против D-32 | не трогать примитив, ввести соседний модификатор — как и рекомендовало исследование | план 04-07 |
| 2 | Разделитель полей файла выгрузки | точка с запятой — как и рекомендовало исследование | план 04-08 |
| 3 | Второй запрос на флагах ближайших отправок | принят как названное отступление от D-38 — как и рекомендовало исследование | план 04-04 |
| 4 | Куда включить маршрут паршала ленты | мимо страничного роутера, отдельным роутером — как и рекомендовало исследование | план 04-05 |
| 5 | Судьба JSON-API `app/routes/history.py` | **выровнять по модулю аналитики, не сносить** — ПРОТИВ рекомендации исследования: допущение A5 недоказуемо, а снос публичной JSON-поверхности необратим | решено в плане 04-01, исполняется планом 04-10 |

1. **(RESOLVED — план 04-07)** **`[data-longtext]` документирован как «ни усечения, ни многоточия, ни скрытия за раскрытием» — а D-32 требует «длинный обрезается по высоте с раскрытием».**
...
2. **(RESOLVED — план 04-08)** **Разделитель CSV: `,` (RFC 4182) или `;` (русский Excel)?**
...
3. **(RESOLVED — план 04-04)** **Второй запрос в блоке «Ближайшие отправки» (флаги групп) — приемлемое отступление от D-38?**
...
4. **(RESOLVED — план 04-05)** **Куда включить маршрут паршала ленты — в `pages_router` (с `load_shell_context`) или мимо него?**
...
5. **(RESOLVED — решено в плане 04-01, исполняется планом 04-10; решение ПРОТИВ рекомендации ниже)** **Судьба `app/routes/history.py` (`GET /stats`, `GET ""`) — выровнять или снести?**
   ...
   - **Принято: выровнять, не сносить.** `grep` подтвердил отсутствие потребителей в репозитории, но A5 остаётся недоказуемым для внешних клиентов, а снос публичной JSON-поверхности — дверь в одну сторону. Выравнивание обратимо и чинит тот же дефект.
