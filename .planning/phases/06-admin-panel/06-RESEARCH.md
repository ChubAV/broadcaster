...
> Все пять вопросов сняты планами фазы 6 (спланирована 2026-08-21). У каждого
> ниже — строка **Разрешено** с планом, который несёт решение. Раздел сохранён
> целиком: рекомендация исследования и её основание — вход чекпойнтов, а не
> история.
...
1. **Куда именно вешать зависимость блокировки (Ф-4, Pattern 2)?**
   ...
   - **Разрешено — план 06-06.** Рекомендация принята дословно: множество `BLOCK_CHECKED_API_ROUTERS` заводится третьим объявлением в `test_access_gate.py`, а судьба `billing_router` вынесена владельцу задачей `checkpoint:decision gate="blocking"` (план перестаёт быть автономным: `autonomous: false`).
...
2. **Форма гейта запретов имперсонации: роутеры или маршруты (Pattern 4)?**
   ...
   - **Разрешено — план 06-13.** Гибрид принят и вынесен отдельным планом, как и рекомендовано: пер-роутерная зависимость плюс машинный гейт «роутер + маршрут» с доказанными зубами.
...
3. **Порог «всплеска отказов» и его окно (A9)?**
   ...
   - **Разрешено — план 06-04.** Форма принята (доля за окно при нижней границе числа отправок), а сами два числа — единственная дискреционная величина фазы без готового объявления в проекте — вынесены владельцу задачей `checkpoint:decision gate="blocking"`.
...
4. **Переносить ли `filter_chips` в `components/` (Ф-15)?**
   ...
   - **Разрешено — план 06-03.** Рекомендация принята и усилена: переезд идёт отдельным планом (не задачей), базовый адрес становится ОБЯЗАТЕЛЬНЫМ параметром вместо умолчания, и обе константы инвентаризации библиотеки компонентов правятся тем же коммитом.
...
5. **Что показывать в колонке «Воркер» для Telegram (D-09)?**
   ...
   - **Разрешено — план 06-05.** Выбор формы измерения живости telegram-строки вынесен владельцу задачей `checkpoint:decision gate="blocking"`: планировщик не назначает подпись сам, потому что оба варианта проверяемы, а цена ошибки — колонка, о которой администратор судит в аварии.
