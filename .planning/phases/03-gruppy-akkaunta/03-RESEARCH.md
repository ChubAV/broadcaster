...
Все три вопроса разрешены исполненной фазой. Исходные формулировки сохранены дословно —
ценность записи в том, что вопрос СТОЯЛ; разрешение дописано отдельной строкой к каждому и
сверено с живым деревом при закрытии фазы (план 03-12).
...
1. **Судьба `GET/DELETE/PATCH /api/groups` (D-14, дискреция).**
   ...
   - **РАЗРЕШЕНО сносом.** Рекомендация принята: `app/routes/groups.py` удалён целиком планом 03-07 — каталог `app/routes/` файла групп не содержит (`ls app/routes/`). Решение закреплено угрозами T-03-29 и T-03-30 в `03-SECURITY.md`.
2. **Куда положить новый роутер: секция в `app/pages/accounts.py` (уже 819 строк) или новый `app/pages/account_groups.py`.**
   ...
   - **РАЗРЕШЕНО выбором отдельного модуля.** Рекомендация принята: `app/pages/account_groups.py` существует, шаблоны лежат в `app/templates/account_groups/`.
3. **Показ «в N расписаниях» в подписи (D-08, дискреция).** Данные считаются как в старом `_get_group_stats` (обход JSON `group_ids` в Python — `app/pages/groups.py:50-56`). Дёшево для страницы в 30 строк; рекомендация — показывать: это объясняет пользователю последствия удаления.
   - **РАЗРЕШЕНО показом.** Рекомендация принята: подпись рендерится в `app/templates/account_groups/includes/group_row.html:40`, ветка нуля — `:42` («не в расписаниях», словами, а не «в 0 расписаниях»).
