...
| T-03-17 | Information Disclosure | текст ошибки в `last_sync_result` | medium | accept | В узкую ветку `accounts.py:832` долетает только `MessengerFetchError`. Свой подконтрольный текст дают ТОЛЬКО `whatsapp.py:133` и `max.py:123` («мост вернул HTTP N»); `whatsapp.py:130`, `max.py:120` и `telegram_user.py:265` строят сообщение из СТОРОННЕГО исключения — `f"{type(e).__name__}: {e}"`, и часть httpx-исключений встраивает в него внутренний адрес wa-worker'а, который доезжает до плашки (`account_groups/list.html:116`). Прежняя формулировка строки утверждала «все конструкции — свой текст» и была неверна — исправлено по CR-01 (`03-REVIEW.md`). Раскрытие ПРИНЯТО владельцем (R-03-09): текст видит только владелец аккаунта на своей странице, severity ниже порога блокировки `high`. `IntegrityError` попасть не может: коммит в своём `try` `:864-866`, обработчик пишет фиксированную строку `:889-891`; широкая ветка пишет `UNEXPECTED_FAILURE_MESSAGE` `:856` | closed |
...
| R-03-09 | T-03-17 | Текст стороннего httpx-исключения (вместе с внутренним адресом wa-worker'а) доезжает до плашки ошибки синхронизации из `whatsapp.py:130`, `max.py:120`, `telegram_user.py:265`. Принято как риск severity medium: текст видит только владелец аккаунта на собственной странице, severity ниже порога блокировки `high`, полный текст и так пишется в лог через `log.error`. Решение принято в UAT раунда 2 (`03-UAT.md`, проверка 4, вариант «а»); сквозной проброс закреплён тестом `tests/test_routes/test_sync_groups.py:695` | chubav | 2026-08-13 |
