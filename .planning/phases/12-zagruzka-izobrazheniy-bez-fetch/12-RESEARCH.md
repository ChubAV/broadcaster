...
**Confidence:** HIGH (почти всё измерено чтением исходников этого дерева; три открытых вопроса помечены)
...
1. **Как удачная загрузка снова начнёт создавать черновик (G-5)?**
   ...
   - Что неясно: какой механизм владелец считает приемлемым — `HX-Trigger` с событием, которое
     `#ad-form` слушает через `from:body`; либо сознательное принятие смены поведения
     («черновик создаётся первым набранным символом, а не первой картинкой»).
   - Рекомендация: **спросить владельца ДО планирования**. Это смена наблюдаемого поведения, и
     рамка вехи требует, чтобы существующее осталось работающим. Первый вариант заводит один
     заголовок и ноль JS; второй — ноль кода и названную цену.
...
2. **Каким становится `client_max_body_size` и где записывается его связь с настройками (G-1)?**
   - Что известно: `20M` в HTTPS-шаблоне при потолке `10 × 5 = 50 МБ`; в HTTP-шаблоне директивы
     нет вовсе.
   - Что неясно: поднимать предел до ~55–64 МБ, или сузить потолок партии (например,
     принимать не более K файлов за раз при неизменном `max_images_per_ad`).
   - Рекомендация: поднять предел **и** записать формулу прозой рядом со значением; сужение
     партии — вторая смена поведения сверх тех, что фаза уже несёт.
...
3. **Что делать с двумя изъятиями, которые Фаза 9 адресовала Фазе 12 (G-11-bis)?**
   ...
   - Что неясно: снимать их (значит переводить сам `#ad-form` на `form_wrapper` — работа вне
     §Phase Boundary) или переадресовать Фазе 15 записью, как D-14 переадресовал R-08-02.
   - Рекомендация: **переадресовать с записью**, а не снимать молча. Перевод `#ad-form` на макрос
     задевает `hx-trigger`, `hx-sync` и `hx-swap="none"` — свойства, ради которых форма и написана
     руками; делать это заодно с FETCH-01 значит смешать две несвязанные правки в одной фазе.

4. **Имя файлового поля.**
   - Что известно: `images` **запрещено** — обработчик сохранения отвергает файловые части под
     этим именем осознанно (WR-03).
   - Рекомендация: любое другое (`files`, `upload`); значение — только в том, чтобы имя не
     совпало с `images` и было объявлено в одном месте.
