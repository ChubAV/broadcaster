...
годится.** Измерено: он не уложился в десятиминутный предел замера при 3410 собранных тестах, то
есть идёт в сотни раз дольше быстрого. Это инструмент слияния волны и приёмки фазы.
...
полная закрывает задачу. Оценивая время исполнения этих трёх задач, закладывать **+10–20 минут**
на каждый полный прогон, а не считать его бесплатным.
...
- [x] Feedback latency < 15s на задачу (измерено: ~4 с на модуль) — первая команда `<automated>`
      ...
      ⚠️ полный прогон > 600 с и единицей обратной связи не служит; у задач 12-03-01, 12-05-01 и
      12-05-02 он стои́т ПОСЛЕДНИМ как гейт завершения, и время на него закладывается отдельно
