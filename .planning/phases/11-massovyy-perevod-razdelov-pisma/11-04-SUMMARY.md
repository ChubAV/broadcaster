...
- `commits: 5` is MEASURED, not narrated: `git rev-list --count 3c68187..HEAD` over the persisted plan ledger at the moment this file was written, before any docs commit existed. It counts this plan's TASK commits and matches the `## Task Commits` list above, on the same convention as the sibling plan 11-03 (`commits: 3`, with its own docs commits outside that figure).
- ⚠️ THE LEDGER RANGE IS WIDER THAN `actuals.commits`, AND NO TOTAL IS QUOTED HERE ON PURPOSE. The range `3c68187..HEAD` also contains the close-out commits — the SUMMARY commit (`e466585`), the metadata commit (`e052d99`) and any correction to this file — so it GROWS while the close-out is still being written. An earlier revision of this bullet named a specific total and was falsified by the very commit that recorded it; quoting the invariant instead of a snapshot ends that regress. A reader comparing the two figures should expect `ledger range = task commits + close-out commits`, not equality.
