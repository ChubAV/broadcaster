...
Задача 1 объявлена `tdd="true"` и прошла последовательность гейтов полностью:

| Гейт | Коммит | Наблюдение |
|------|--------|------------|
| RED | `c13227d` (`test`) | `test_disabled_group_chosen_in_the_schedule_stays_visible` падает: выбранная выключенная группа отсутствует в разметке |
| GREEN | `eb87bda` (`feat`) | тот же тест зелёный, 36 тестов файла проходят |
| REFACTOR | — | не потребовался: правка легла в существующие блоки без дублирования |

Пять остальных утверждений RED-коммита зелёные с самого начала намеренно — это парные сторожа скоупа и нулевого дифа, чьё поведение правка менять НЕ должна. Красным был ровно тот тест, который описывает новое поведение.

## Self-Check: PASSED

- Все заявленные файлы существуют на диске (5 из 5).
- Все заявленные коммиты присутствуют в истории ветки (4 из 4).
- Удалённых файлов в диапазоне `a06c83d..HEAD` нет.
- Рабочее дерево чистое, неотслеживаемых артефактов не осталось.
- Заглушек, пропущенных тестов и невыполненных `<verify>` в плане нет — раздел «Known Stubs» не заводится.
