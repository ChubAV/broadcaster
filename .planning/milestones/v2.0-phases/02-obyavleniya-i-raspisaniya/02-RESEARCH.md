...
Все пять вопросов закрыты на планировании. У каждого назван план-владелец, в котором решение принято и записано; ниже сохранены исходные формулировки вопросов и рекомендации исследования, чтобы основание решения оставалось читаемым.
...
1. **Каким должен быть порог счётчика длины текста?**
   ...
   - **RESOLVED: `02-04`** §`<planner_decisions>` — предел 4096, предупреждение с 90% предела, при непустом списке вложений предупреждение с 1024; счётчик не блокирует сохранение. Основание — внешний факт MEDIUM-достоверности, поэтому решение продублировано в `02-04` §`<flagged_assumptions>` вместе с риском, если допущение неверно.
...
2. **Показывать ли расписания объявлений-черновиков в сводном списке?**
   ...
   - **RESOLVED: `02-07`** §`must_haves.truths` — расписания объявлений-черновиков показываются в сводном списке с пометкой «Объявление в черновике», а в ячейке ближайшего запуска стоит «отправок не будет».
...
3. **Что показывать в поле «группы» карточки сводного списка — количество или имена?**
   ...
   - **RESOLVED: `02-07`** §`must_haves.truths` — имена первых нескольких групп плюс остаток числом («и ещё K»), имена обрезаются многоточием с полным значением в подсказке; имена берутся одним запросом на всю страницу, ограниченным владельцем.
...
4. **Нужна ли разовая чистка уже сохранённых чужих/внешних значений в `Ad.images`?**
   ...
   - **RESOLVED: `02-02`** §`<threat_model>` T-02-10 — диспозиция `accept`: фикс WR-01 действует только на записи, разовая чистка уже сохранённых значений сознательно вынесена за границу фазы (интерфейс, не миграция данных). Диагностический запрос и остаточный риск записываются в SUMMARY как задел бэклога.
...
5. **Сохранять ли `/schedules/new` и `/schedules/{id}/edit` как 301-редиректы вместо полного удаления?**
   ...
   - **RESOLVED: `02-06`** — D-14 не переоткрывается: адреса удаляются целиком, мёртвого кода не остаётся. Мягкая посадка постоянным редиректом рассмотрена и отклонена; остаточный риск зафиксирован как T-02-31 с диспозицией `accept` и как возможный последующий шаг в SUMMARY, если после выката в логах появятся обращения на эти адреса.
