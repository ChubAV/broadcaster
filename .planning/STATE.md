...
- ⚠️ [Phase 5] Перед выкатом обязателен `YOOKASSA_WEBHOOK_CLIENT_IP_HEADER=X-Real-IP` в прод-`.env`. Без него гард источника вебхука читает `request.client.host` — адрес nginx-контейнера — и отклоняет каждое настоящее уведомление, тихо останавливая приём платежей. Аварийный выключатель: `YOOKASSA_WEBHOOK_VERIFY_IP=false`. Правки nginx и docker-compose не нужны.
