...
- [ ] 04-01-PLAN.md — Tracer: модуль аналитики отправок и плитки метрик за сутки сквозным срезом; переезд фильтров истории и счётчика (DASH-01, HIST-01)
- [ ] 04-02-PLAN.md — Ревизия `0016`: составной индекс `(user_id, sent_at)` на `send_logs` и её миграционный тест (D-36)
- [ ] 04-03-PLAN.md — Воркерская половина повтора: вынос `build_dispatch_task` и таск `retry_send` на все три канала (HIST-04)
...
| 4. Дашборд и история | 0/10 | Not started | - |
