...
``tests/test_services/test_image_upload.py::test_supported_formats_and_refusal_text_stay_in_step``.

⚠️ АДРЕС ПРИЁМА ПЕРЕНАЦЕЛЕН, И ПРЕЖНИЙ НАЗВАН СЛОВАМИ, А НЕ ПУТЁМ (находка
IN-01, план 12-10). Здесь стоял путь в модуль тестов ПРЕЖНЕГО JSON-маршрута
загрузки — модуль снят планом 12-05 ЦЕЛИКОМ, и путь указывал в пустоту. Дословно
он не набирается и летописью: приёмочный обход находки ищет снятое имя по дереву
``tests/``, и упоминание удовлетворило бы обход само — правило стало бы
самоопровергающимся. Правило ТОГО ЖЕ ИМЕНИ пережило снятие модуля и живёт по
новому адресу выше; проверено чтением действующего модуля, а не предположено.
