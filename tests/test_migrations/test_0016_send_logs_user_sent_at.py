...
def test_revision_0016_continues_0015():
    ...
    Ветвление ломает `alembic upgrade head` сразу для всех, поэтому проверяются
    обе половины сцепки: голова ОДНА и она — 0016, а её `down_revision` — 0015.
    Очередь невыкаченных ревизий 0013-0015 при этом не переставляется.
    """
