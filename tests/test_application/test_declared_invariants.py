...
# ⚠️ 21 → 20 ПЛАНОМ 08-05, И ПОНИЖЕНИЕ ПРИЕХАЛО ТЕМ ЖЕ КОММИТОМ, ЧТО ЗАКРЫЛ ДОЛГ
# РАБОТОЙ. Из реестра ушла запись абзаца `create_payment` о НЕ построенном
# частичном индексе: индекс построен (ревизия `0021` плюс `__table_args__`
# модели), абзац переписан на сделанное и назвал свидетелей. Понижать потолок
# отдельным планом значило бы оставить между ним и реестром запас — место для
# нового ложного абзаца.
WITHOUT_WITNESS_CEILING = 20
