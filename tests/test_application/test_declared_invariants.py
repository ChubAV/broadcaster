...
# Порог заглавного прогона. Рабочее значение — два, и причина названа прогоном:
# абзац `WR-03` в `switch_is_refused` (`plan_switch.py`) несёт прогон ровно в два
# слова, и при пороге три выпадает из отбора вовсе.
...
# Четыре места находок раунда 6 — тройками «модуль, владелец, фрагмент».
...
ROUND_SIX_FINDINGS = (
    (
        ...
        "converted_remainder",
        "ДЛИНА КАЛЕНДАРНОГО МЕСЯЦА В ОТВЕТ НЕ ВХОДИТ",
    ),
    (
    ...
    (
        ...
        "switch_is_refused",
        "СОХРАНЕНИЕ действующего тарифа",
    ),
)
...
def paragraph_fingerprint(paragraph: str) -> str:
    ...
    """
    return " ".join(paragraph.split())[:FINGERPRINT_LENGTH]
...
def test_the_selection_covers_the_round_six_findings():
    """Все четыре места находок раунда 6 ОТОБРАНЫ и в реестре долга ОТСУТСТВУЮТ.
