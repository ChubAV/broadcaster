...
⚠️ ВТОРОЕ УТВЕРЖДЕНИЕ — ПРО ОТБОР НЕЗАКРЫТЫХ ПЛАТЕЖЕЙ ПО ОБЪЯВЛЕННОМУ
МНОЖЕСТВУ НАБЛЮДАЕМЫХ СТАТУСОВ, а не по дополнению терминальных (D-01, план
08-11). Улика была в третьем, никогда не встречавшемся статусе, и посылка её
умерла: собственные статусы платёжного провайдера в колонку `payments.status`
не попадают ни одним путём — её пишет только платёжный сервис объявленными
константами. Зато четвёртый статус в неё добавили МЫ САМИ (`expired`, снятое
по сроку давности намерение), и дополнение поглотило его молча: строка
приходила под чипсом «В обработке», хотя та же таблица печатала её словом
«просрочен». Поэтому улика перевёрнута — просроченная строка обязана НЕ
приходить под чипсом.
...
@pytest.mark.asyncio
async def test_the_ledger_selects_unclosed_payments_by_the_declared_awaiting_set(
    db_session: AsyncSession,
):
    """Под чипсом «В обработке» приходят ТОЛЬКО наблюдаемые строки.

    ⚠️ ПРЕЖНЯЯ УЛИКА БЫЛА В ТРЕТЬЕМ СТАТУСЕ, И ЕЁ ПОСЫЛКА УМЕРЛА. Тест сеял
    выдуманный статус платёжного провайдера и требовал, чтобы отбор по
    дополнению терминальных подхватил его сам. Но собственные статусы ЮKassa в
    колонку `payments.status` не попадают ни одним путём: её пишет только
    платёжный сервис объявленными константами, и закрытость словаря колонки
    утверждается машинно (`tests/test_services/test_payment_status_vocabulary.py`).
    Зато четвёртый статус в неё добавили МЫ САМИ — `expired`, снятое по сроку
    давности намерение (D-01), — и дополнение поглотило его молча.

    Поэтому улика ПЕРЕВЁРНУТА: просроченная строка обязана НЕ приходить под
    чипсом, чья подпись «В обработке», ведь та же таблица печатает её словом
    «просрочен». Отбор идёт по объявленному `AWAITING_STATUSES`, а счёт над
    журналом считается тем же условием, что и содержимое.
    """
    user = await _seed_user(db_session, "unclosed@test.com")
    await _seed_payment(db_session, user, status=STATUS_PENDING)
    await _seed_payment(db_session, user, status=STATUS_EXPIRED)
    await _seed_payment(db_session, user, status=STATUS_SUCCEEDED)
    await _seed_payment(db_session, user, status=STATUS_CANCELED)
    await db_session.commit()
    ...
    assert unclosed.total == 1 == len(unclosed.rows)
    assert {row.status for row in unclosed.rows} == {STATUS_PENDING}
