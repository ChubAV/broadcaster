...
def test_the_only_remaining_stage_reads_the_declaration_of_the_rule():
    """Правило читает ОДИН объект функции, а не копия.

    ⚠️ СТАДИЙ ОСТАЛОСЬ ОДНА ИЗ ДВУХ, И ЭТО НЕ ОСЛАБЛЕНИЕ, А СНЯТИЕ ПРЕДМЕТА.
    Стадия НАМЕРЕНИЯ (`app.pages.billing` решала, продавать ли) правила больше
    не спрашивает: тарифов Free/Basic/Pro не существует (D-F), переходить между
    ними некуда, и гард смены тарифа снят вместе с витриной (планы 05.1-02 и
    05.1-05). Утверждать, что снятая стадия ссылается на то же объявление, —
    значит требовать вызова, который обязан отсутствовать.

    Отсутствие проверяется ЯВНО: молча ослабленное утверждение не отличило бы
    «гарда нет» от «гард завёл вторую копию правила».

    Файл целиком умирает планом 05.1-07 вместе с самим правилом; до тех пор он
    обязан говорить правду о дереве.
    """
    import app.pages.billing as intent_stage
    import app.services.payment_service as apply_stage

    assert not hasattr(intent_stage, "switch_is_refused"), (
        "стадия намерения снова спрашивает правило перехода между тарифами, "
