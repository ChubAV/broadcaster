...
@pytest.mark.parametrize(
    ...
    [
        ...
        # нечего. Образец ЦИФРОВОЙ намеренно, по той же причине, что латинские
        # имена у экрана групп аккаунта выше: поиск уходит в адрес сентинела
        # урлкодированным, и кириллический образец пришлось бы сверять с
        # процентными последовательностями — тест краснел бы на кодировке, а не
        # на потерянном отборе. Цифра 1 встречается в посеве
        # («Объявление 1», «Объявление 10»), то есть отбор непустой.
        ("ads", "search=1", "search=1"),
    ],
)
