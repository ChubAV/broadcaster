...
async def _seed_section(db: AsyncSession, section: str) -> str:
    ...
    user = await _user(db)
    if section == "ads":
        # ЛАТИНСКИЙ образец в тексте — намеренно и по той же причине, что
        # латинские имена групп ниже: поиск раздела уходит в адрес сентинела
        # урлкодированным, и кириллический образец пришлось бы сверять с
        # процентными последовательностями — тест краснел бы на кодировке, а не
        # на потерянном отборе. Образец стоит у ВСЕХ посеянных объявлений: отбор,
        # оставляющий меньше страницы, второго сентинела не породил бы вовсе, и
        # проверка «отбор доехал до второй страницы» зеленела бы на пустоте.
        db.add_all(
            [
                Ad(
                    user_id=user.id,
                    title=f"Объявление {i}",
                    text=f"Текст {i} promo",
                    images=[],
                )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Объявления получили панель поиска задачей 260825-m0b — до неё
        # единственной осью раздела была прокрутка, и терять сентинелу было
        # нечего. Образец латинский и стоит у ВСЕХ посеянных объявлений: почему
        # именно так — сказано в ветке посева раздела.
        ("ads", "search=promo", "search=promo"),
    ],
)
