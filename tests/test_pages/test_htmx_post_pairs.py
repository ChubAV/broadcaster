...
# =============================================================================
# Посев: загрузка вложения объявления (Фаза 12, план 12-01)
# =============================================================================

ADS_IMAGES_UPLOAD = "app/pages/ads.py::ads_images_upload"


async def _arrange_image_upload_without_parts(
    client, db, settings, identity
) -> _Arranged:
    """Загрузка БЕЗ файловых частей — сеять нечего, форма самодостаточна.

    Маршрут привязан к ПОЛЬЗОВАТЕЛЮ, а не к черновику (D-02 Фазы 12), поэтому ни
    объявления, ни аккаунта посев не заводит: отвечает обработчик полосой
    итоговых ключей, а их при пустом запросе ноль.
    """
    await _current_user(db, identity, settings)
    return _Arranged(url="/ads/images")


async def _arrange_image_upload_without_session(
    client, db, settings, identity
) -> _Arranged:
    client.cookies.clear()
    return _Arranged(url="/ads/images")
...
POST_PAIR_CASES: tuple[_PairCase, ...] = (
    ...
    # Фаза 12, план 12-01. Загрузка вложения объявления: экран редактора
    # ОСТАЁТСЯ, подменяется содержимое полосы, и ответ фрагментом приезжает на
    # ВСЕ исходы работы (D-04). Метка — узел содержимого полосы: без него
    # подменять цель нечем, и рантайм промолчал бы об этом.
    #
    # ⚠️ У СЛУЧАЯ НЕТ ФАЙЛОВОЙ ЧАСТИ, И ЭТО ГРАНИЦА ОБХОДА, А НЕ ВЫРОЖДЕННЫЙ
    # ПОСЕВ. Обе половины обхода шлют тело формы (`data=`) и составного запроса
    # послать не умеют — предмет ЭТОГО реестра транспорт, а не приём файла.
    # Половина «файл доехал» утверждена поимённо в
    # `tests/test_pages/test_ads_image_upload.py`, где обе половины пары сделаны
    # с настоящим PNG: с признаком htmx — 200 и плитка со скрытым полем ключа,
    # без признака — 302 на `/ads/new`. Запись здесь замыкает правило «у
    # переведённого обработчика есть пара», а не подменяет тот модуль.
    _PairCase(
        key=ADS_IMAGES_UPLOAD,
        name="загрузка вложения — запрос без файловых частей",
        identity="user",
        arrange=_arrange_image_upload_without_parts,
        landing="/ads/new",
        transport=FRAGMENT,
        fragment_mark='id="media-tray"',
    ),
    _PairCase(
        key=ADS_IMAGES_UPLOAD,
        name="загрузка вложения — нет сессии",
        identity="user",
        arrange=_arrange_image_upload_without_session,
        landing="/login",
        transport=LOCATION,
    ),
)
...
#   33 → 37, Фаза 11, план 11-16: четыре исхода ПОВТОРНОЙ СИНХРОНИЗАЦИИ ГРУПП
#   АККАУНТА — все переходом (действие навигационное, D-02): успех на экран
#   групп, аккаунта нет и аккаунт чужой (одна ветка, T-11-29) на `/accounts`,
#   нет сессии на `/login`.
#   ПОСТАВЛЕНО ПРОГОНОМ: `случаев пар в реестре 37, объявлено 33`.
#   37 → 40, Фаза 11, план 11-17: три исхода СИНХРОНИЗАЦИИ ГРУПП АККАУНТА — все
#   переходом (действие навигационное, D-02): успех и «синхронизация уже идёт»
#   на экран групп, аккаунта нет на `/accounts`. Освобождение внутрипроцессной
#   заявки на каждом выходе утверждено поведением в
#   `tests/test_routes/test_sync_groups.py`, а не здесь.
#   ПОСТАВЛЕНО ПРОГОНОМ: `случаев пар в реестре 40, объявлено 37`.
#   40 → 41, Фаза 11, план 11-17, задача 3: ПОВТОРНАЯ СИНХРОНИЗАЦИЯ с
#   идентификатором вне колонки (D-07) — ветка «аккаунта нет», переход на
#   `/accounts` на обоих транспортах, а не отказ валидации фреймворка.
#   ПОСТАВЛЕНО ПРОГОНОМ: `случаев пар в реестре 41, объявлено 40`.
#   41 → 43, Фаза 11, план 11-18: два исхода СТАРТА ПОДКЛЮЧЕНИЯ MAX — успех
#   (фрагмент контейнера шага мастера, метка — узел опроса статуса) и «нет
#   сессии» (переход на экран входа). Ветка пустого телефона отвечает 422 на
#   обоих транспортах и утверждена поимённо в
...
#   48 → 50, Фаза 12, план 12-01: два исхода ЗАГРУЗКИ ВЛОЖЕНИЯ объявления —
#   запрос без файловых частей (фрагмент полосы: ответ фрагментом приезжает на
#   ВСЕ исходы работы, D-04) и «нет сессии» (переход на экран входа).
#   ⚠️ ПОЛОВИНА «ФАЙЛ ДОЕХАЛ» В ЭТОТ РЕЕСТР НЕ ВХОДИТ, И ЭТО ГРАНИЦА ОБХОДА, А
#   НЕ ПРОПУСК: обе половины шлют тело формы и составного запроса послать не
#   умеют. Обе её стороны утверждены поимённо в
...
#
# 158 → 159, Фаза 12, план 12-01: утверждение 302 о загрузке вложения в
