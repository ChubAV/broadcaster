...
async def test_autosave_response_carries_the_media_tray(
    ...
    ⚠️ ПРАВИЛО ПЕРЕНАЦЕЛЕНО НА ПУТЬ УБИРАНИЯ, А НЕ ОСЛАБЛЕНО (Фаза 12, план
    12-07), И ПРИЧИНА У ПЕРЕНАЦЕЛИВАНИЯ ПРЕДМЕТНАЯ. Прежде запрос шёл БЕЗ
    ``remove_image``, то есть измерял «полоса приезжает КАЖДЫМ автосохранением»
    — а ровно это и есть гап 2 верификации: ответ, перерисовывающий полосу из
    базы, отстающей от документа на незаписанный ключ, уносит с экрана и свежую
    плитку, и строки отказа, за которые D-04 заплатил лживым кодом 200. Предмет
    правила — покупка плана 12-03 («убранное кнопкой «×» исчезает с экрана»), и
    она держится ИМЕННО на пути убирания. Все три прежних утверждения
    сохранены дословно; сменился только запрос, который их поднимает, и ключ, о
    скрытом поле которого спрашивает третье, — теперь это ОСТАВШИЙСЯ ключ.
    Соседнее правило ``test_a_text_only_autosave_leaves_the_tray_alone``
    сторожит вторую половину: автосохранение, состава вложений не менявшее,
    полосы не приносит вовсе.
    """
    owner_id = (await _user(db_session)).id
    ...
    assert 'name="images"' in html and f'value="{kept}"' in html, (
        "внеполосная полоса пришла без скрытого поля ключа: следующая отправка "
        "формы ушла бы БЕЗ вложений и отцепила бы их молча"
    )
    assert f'value="{dropped}"' not in html, (
        "убранный ключ остался в полосе: человек нажал «×», ключ ушёл из базы, "
        "а плитка осталась на экране — он нажмёт ещё раз"
    )
