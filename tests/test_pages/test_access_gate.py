...
#
# ⚠️ 5 → 4 (ФАЗА 12, ПЛАН 12-05): РОУТЕР ЗАГРУЗКИ ИЗОБРАЖЕНИЙ СНЯТ ИЗ ЭТОГО
# МНОЖЕСТВА, И ТРЕБОВАНИЕ МНОЖЕСТВА ВЫПОЛНЯЕТСЯ ТЕПЕРЬ ДРУГИМ МЕХАНИЗМОМ, А НЕ
# ОТМЕНЯЕТСЯ. Обработчик загрузки переехал на СТРАНИЧНУЮ поверхность и заведён
# внутри `ads_router` (D-01 `12-CONTEXT.md`); там на нём уже висит пер-роутерная
# зависимость `require_access` (`app/pages/__init__.py`), а её провал едет
# отказом, который человек ВИДИТ (204 + заголовок перехода, FOUND-07), а не
# JSON-ответом, которого страница не покажет. Роутера с прежним именем в сборке
# больше нет вовсе, и его собственный модуль снят тем же коммитом.
#
# ⚠️ РЕШЕНИЕ О ПОВЕРХНОСТИ ПРИНЯТО ПО КОНВЕНЦИИ ПРОЕКТА, А НЕ ПОДОГНАНО РАДИ
# ЗЕЛЕНИ: D-07 Фазы 11 назвал `app/routes/` «другим транспортом», а слой ответа
# и его отказ живут в `app/pages/`. Правка этого множества есть следствие
# решения, и обратный порядок — «снять запись, чтобы перестало краснеть» — был бы
# ровно той формой обхода, которую весь этот файл закрывает.
#
# ⚠️ ОСНОВАНИЕ ВЫПИСАНО ЗДЕСЬ СВОИМИ СЛОВАМИ ДЛЯ ЭТОГО МНОЖЕСТВА, А НЕ ССЫЛКОЙ
# НА СОСЕДНЕЕ: множества объявлены НЕЗАВИСИМЫМИ, и выведение одного из другого
# запрещено этим файлом прямо (см. абзац над `BLOCK_CHECKED_API_ROUTERS`).
GATED_API_ROUTERS = {
    "ads_router",
    "accounts_router",
    "schedules_router",
...
#
# ⚠️ 5 → 4 (ФАЗА 12, ПЛАН 12-05): РОУТЕР ЗАГРУЗКИ ИЗОБРАЖЕНИЙ СНЯТ ИЗ ЭТОГО
# МНОЖЕСТВА ОТДЕЛЬНОЙ ПРАВКОЙ, А НЕ ЗАОДНО С ГЕЙТОМ ДОСТУПА, И ОСНОВАНИЕ
# НАПИСАНО СВОИМИ СЛОВАМИ ДЛЯ ЭТОГО ВОПРОСА. Вопрос здесь — «действует ли учётная
# запись», и ответ на него для загрузки не изменился: обработчик переехал на
# СТРАНИЧНУЮ поверхность внутрь `ads_router` (D-01), а страничная сборка
# спрашивает то же самое своей пер-роутерной зависимостью
# (`app/pages/__init__.py`) — заблокированного она не пускает. То есть
# требование множества выполняется ДРУГИМ механизмом, а не отменяется; роутера с
# прежним именем в сборке больше нет, его модуль снят тем же коммитом.
#
# ⚠️ ССЫЛКИ НА СОСЕДНЕЕ МНОЖЕСТВО ЗДЕСЬ НЕТ НАМЕРЕННО. Абзац выше объявляет
# множества независимыми и выведение одного из другого запрещает; правка «заодно»
# и была бы таким выведением, только сделанным руками.
