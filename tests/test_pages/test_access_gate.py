...
#
# ⚠️ `billing_money_router` СТОИТ ЗДЕСЬ ПО ТОЙ ЖЕ ПРИЧИНЕ, ЧТО И `billing_router`,
# И ЕГО ПОЯВЛЕНИЕ — НЕ ДРОБЛЕНИЕ РАДИ ДРОБЛЕНИЯ. Денежные ИЗМЕНЯЮЩИЕ входы
# раздела вынесены в свой роутер планом 06-13, чтобы запрет действий под чужой
# личностью (D-22) висел на РОУТЕРЕ и новый денежный маршрут оказывался закрыт
# ПО УМОЛЧАНИЮ (D-23); чтение раздела осталось в общем роутере, потому что под
# чужой личностью оно разрешено. К вопросу ЭТОГО файла — «закрывает ли истёкший
# доступ» — разделение отношения не имеет: обе половины раздела оплаты обязаны
# оставаться открытыми, иначе человек с истёкшим сроком не сможет заплатить
# (T-05.1-16). Молчание здесь было бы ровно той формой обхода, которую этот файл
# закрывает.
OPEN_ROUTERS = {
    "auth_router",
    "dashboard_router",
    "billing_router",
    "billing_money_router",
    "admin_router",
    "profile_router",
}
...
# может даже войти, чтобы заплатить (T-05.1-16); денежный роутер — у него
# остался ровно один вход, вебхук ЮKassa, и отказ на нём остановил бы приём
# денег (читающие входы сняты вместе с валютой сообщений);
...
# закрывает.
#
# ⚠️ `admin_partials_router` СТОИТ ЗДЕСЬ ПО ОТДЕЛЬНОМУ РЕШЕНИЮ, а не заодно с
# соседями, и обоснование записано абзацем в `app/main.py` рядом с его
# включением. Коротко, тремя независимыми доводами: админка целиком объявлена
# НЕ ЗАКРЫВАЕМОЙ в страничной сборке (`admin_router` в `OPEN_ROUTERS`) —
# администратор без оплаченного доступа обязан входить в админку, иначе чинить
# аварию некому; права администратора висят на самом обработчике паршала,
# поэтому открытым для постороннего он не становится и отвечает 403; опрос
# бессрочен (D-12), и запрос за вердиктом доступа умножался бы на число
# открытых вкладок каждые двадцать секунд ради ответа, заранее известного.
# Молчание здесь было бы ровно той формой обхода, которую этот файл закрывает.
OPEN_API_ROUTERS = {
    "auth_router",
    "billing_router",
    "dashboard_feed_router",
    "admin_partials_router",
    "pages_router",
}


# РОУТЕРЫ JSON-API, КОТОРЫЕ ЗАКРЫВАЕТ БЛОКИРОВКА УЧЁТНОЙ ЗАПИСИ (CR-01, D-30).
# ТРЕТЬЕ множество этого файла, и оно ОТДЕЛЬНОЕ от перечня гейта доступа, хотя
# сегодня совпадает с ним по составу. Совпадение есть факт, а не правило: гейт
# доступа отвечает на вопрос «оплачено ли», гейт блокировки — на вопрос
# «действует ли учётная запись», и первая же фаза, ответившая на них по-разному
# (например, закрыв блокировкой раздел, который истёкшему доступу оставлен
# открытым), развела бы множества. Выведение одного из другого запретило бы это
# молча.
#
# ⚠️ `billing_router` СТОИТ ВНЕ ЭТОГО МНОЖЕСТВА ПО РЕШЕНИЮ ВЛАДЕЛЬЦА (D-53,
# вариант A чекпойнта плана 06-06), а не заодно с гейтом доступа. У роутера
# остался ровно один вход — уведомление ЮKassa о СОСТОЯВШЕМСЯ платеже,
# приходящее ИЗВНЕ; отказ на нём остановил бы приём денег по уже совершённым
# платежам, и потерянное уведомление откатом кода не возвращается. Страничная
# форма покупки живёт не здесь, а в `app/pages/billing.py`, за гардом
...
def _blocking_included_routers() -> dict[str, bool]:
    """Та же сборка: кто включён и на ком висит гейт БЛОКИРОВКИ (CR-01)."""
    return _routers_with_dependency(MAIN_PY, "get_current_user_id_active")
...
def test_api_gate_covers_exactly_the_declared_routers():
    ...
    assert set(included) == (
        GATED_API_ROUTERS | OPEN_API_ROUTERS | BLOCK_CHECKED_API_ROUTERS
    ), (
...
def test_the_blocking_gate_covers_exactly_the_declared_routers():
    """Множество роутеров, закрываемых БЛОКИРОВКОЙ, равно объявленному.

    ⚠️ ТРЕТИЙ ПЕРЕЧЕНЬ ЧИТАЕТ ТОТ ЖЕ ИСХОДНИК ТЕМ ЖЕ ПОМОЩНИКОМ, И ЭТО СУТЬ
    ПРИЁМА. Лобовая починка блокировки — проверка внутри общей зависимости
    аутентификации — закрыла бы денежный роутер молча; здесь навеска ПЕР-
    РОУТЕРНАЯ, поэтому «кого закрывает блокировка» есть выписанное решение, а
    не следствие места, куда легла правка.

    Роутер, добавленный будущей фазой, роняет этот тест В ЛЮБОМ СЛУЧАЕ: он не
    назван ни в одном из трёх множеств, и последнее утверждение красит его
    независимо от того, повесили на него зависимость или забыли.
    """
    included = _blocking_included_routers()

    blocked = {name for name, is_gated in included.items() if is_gated}

    assert blocked == BLOCK_CHECKED_API_ROUTERS, (
        f"перечень закрытых блокировкой роутеров разошёлся с объявленным: "
        f"лишние {blocked - BLOCK_CHECKED_API_ROUTERS}, "
        f"недостающие {BLOCK_CHECKED_API_ROUTERS - blocked}"
    )
    assert "billing_router" not in blocked, (
        "блокировка закрыла денежный роутер — отказ уходит в ответ на "
        "уведомление ЮKassa о СОСТОЯВШЕМСЯ платеже (D-53, отвергнутый "
        "вариант C)"
    )
    assert set(included) == (
        GATED_API_ROUTERS | OPEN_API_ROUTERS | BLOCK_CHECKED_API_ROUTERS
        ...
        "в сборку приложения включён роутер, о котором этот тест не знает — "
        "решение «закрывает ли его блокировка» не принято"
    )
...
def test_the_api_authentication_dependency_is_left_untouched():
    ...
    маршруты, которые обязаны оставаться открытыми (денежный роутер целиком), и
...
@pytest.mark.asyncio
async def test_the_money_path_stays_open_when_access_expired(
    expired_client: AsyncClient,
):
    """Денежный путь истёкшим доступом НЕ закрывается (T-05.1-16).

    Закрыть его значило бы запереть человека в продукте, где единственное
    действие, открывающее доступ, само требует доступа.

    ⚠️ ПРЕДМЕТ ПЕРЕЦЕЛЕН С JSON-ЧТЕНИЯ НА ВХОД ОПЛАТЫ, ПОТОМУ ЧТО ЧИТАТЬ БОЛЬШЕ
    НЕЧЕГО. Три читающих JSON-входа денежного роутера (остаток сообщений,
    прейскурант пакетов и журнал операций по остатку) сняты вместе со всей
    валютой сообщений, и у роутера остался один вход — уведомление ЮKassa,
    приходящее ИЗВНЕ и не имеющее отношения к тому, оплачен ли доступ у
    отправителя. Граница осталась той же и не ослабла: она про то, что человеку
    с истёкшим сроком дают ЗАПЛАТИТЬ, и настоящий её носитель — именно вход
    оплаты, а не чтение чисел рядом с ним.

    ПРОВЕРЯЕТСЯ ОТДЕЛЬНЫЙ МАРШРУТ, А НЕ ТОЛЬКО СТРАНИЦА. Страницу держит
    `test_pages_of_recovery_stay_open_when_access_expired`; форма оплаты —
    самостоятельный маршрут с самостоятельной зависимостью, и закрыться она
    может независимо от страницы, на которой нарисована.

    СОЗДАНИЕ ПЛАТЕЖА ПОДМЕНЕНО: предмет вопроса — ДОШЁЛ ЛИ запрос до
    обработчика, а не что ответила ЮKassa. Настоящий вызов SDK ушёл бы в сеть
    прямо из суиты.
    """
    confirmation_url = "https://yookassa.ru/checkout/payments/expired-may-pay"
    with patch(
        "app.pages.billing.create_payment",
        new=AsyncMock(
            return_value={
                "confirmation_url": confirmation_url,
                "payment_id": "yoo_expired_may_pay",
            }
        ),
    ):
        response = await expired_client.post(
    ...
    assert response.status_code not in (402, 403), (
        "вход оплаты закрыт гейтом доступа: человек не может заплатить"
    )
    assert response.headers.get("location") != EXPIRED_LOCATION, (
        "вход оплаты увёл человека оплачивать доступ вместо того, чтобы принять "
        "оплату, — гейт замкнулся сам на себя"
    )
    assert response.headers["location"] == confirmation_url, (
        f"истёкший доступ не довёл до страницы оплаты: "
