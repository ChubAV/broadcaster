...
def make_webp_bytes() -> bytes:
    ...
    Скопировано из `tests/test_routes/test_uploads.py` по тому же основанию, по
    которому скопированы построители выше: тестовый модуль не библиотека.
    Настоящим изображением байты быть не обязаны — распознавание идёт по первым
    байтам и отвергает формат ДО того, как до содержимого дойдёт декодер.
    """
