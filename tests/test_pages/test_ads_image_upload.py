...
async def test_a_storage_failure_becomes_a_row_and_keeps_the_accepted_keys(
    ...
    assert "хранилище" in reason, (
        f"причина отказа {reason!r} не говорит о хранилище: человек прочтёт "
        "аварию инфраструктуры как отказ СВОЕМУ файлу и пойдёт искать другой"
    )
