...
#
#   10 → 9, Фаза 13, план 13-01. ИСТОЧНИК ДВИЖЕНИЯ: запись ключа сессии опроса
#   мастера Telegram СНЯТА. Опрос стал POST (D-01 Фазы 13), и ключ сессии
#   читается из тела формы в обработчике, а не параметром сигнатуры, — каталог
#   его больше не наблюдает. ⚠️ ПОПРАВКА ПОСЫЛКИ, ИЗМЕРЕННАЯ ПРОГОНОМ: D-12 и
#   RESEARCH §Инвентарь (строка 15) ждали, что запись ПЕРЕЕДЕТ ключом на POST при
#   неподвижном числе 10; правило равенства найденного и заявленного показало
#   иное. ⚠️ ЧИСЛО ПОСТАВЛЕНО ПРОГОНОМ ПОКРАСНЕВШЕГО ПРАВИЛА.
#   `test_the_catalogue_universe_is_declared_by_a_number` сказал дословно:
#   «ПРИЛОЖЕНИЕ РАЗОШЛОСЬ С ЗАМЕРОМ. Найдено и не заявлено: []; заявлено и не
#   найдено: ['app/pages/accounts.py::GET /accounts/connect/tg_user/qr-status →
#   session_id']». Записи под новым ключом прогон не нашёл.
CATALOGUE_APPENDIX_DECLARED = 9
...
CATALOGUE_APPENDIX: tuple[_AppendixEntry, ...] = (
    ...
    # ⚠️ ЗАПИСЬ КЛЮЧА СЕССИИ ОПРОСА МАСТЕРА TELEGRAM СНЯТА, А НЕ ПЕРЕКЛЮЧЕНА
    # НА POST (Фаза 13, план 13-01; поправка посылки D-12 и RESEARCH §Инвентарь,
    # строка 15, — они ждали переезда ключа при неподвижном числе). Опрос сменил
    # метод GET → POST, и ключ сессии теперь читается из ТЕЛА ФОРМЫ внутри
    # обработчика (RESEARCH §Pitfall 8, образец — телефон мастера MAX), а
    # параметром сигнатуры больше не является. Вход не исчез: каталог наблюдает
    # только параметры сигнатуры, и поля формы, читаемые в теле, он не видит ПО
    # ПОСТРОЕНИЮ — это граница каталога, а не пропажа величины. Сама величина
    # по-прежнему строка, а не целое колонки идентификатора, и операндом
    # сравнения по ней не уезжает. Летопись у `CATALOGUE_APPENDIX_DECLARED`.
    _AppendixEntry(
