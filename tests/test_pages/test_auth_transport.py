...
async def test_a_new_password_leaves_for_the_login_by_a_full_load_with_the_notice(
    ...
    ⚠️ ОДИН ТОКЕН НА ОБА ТРАНСПОРТА — ЭТО СЕГОДНЯШНЕЕ СВОЙСТВО ОБРАБОТЧИКА,
    А НЕ ТРЕБОВАНИЕ. Правило — пара транспортов (идиома суиты: одно поведение,
    два пути), и ему нужен один и тот же вход. Но обработчик
    `forgot_password_reset` проверенный токен НЕ ПОГАШАЕТ: `verified_at`
    он не читает вовсе, а на коротком пароле выпускает СВЕЖИЙ проверенный
    токен — значит держатель продлевает возможность бесконечно. Находка
    ревизии кода Фазы 14 (CR-02, `14-REVIEW.md`), датирована ДО фазы
    (`git show fd69a26a` — те же четыре проверки), принята риском решением
    владельца 2026-09-23 (`14-SECURITY.md`, T-14-21, accept).

    ⚠️ КОГДА CR-02 БУДУТ ЧИНИТЬ, ЭТО ПРАВИЛО ОБЯЗАНО ПОКРАСНЕТЬ — на втором
    POST. Красное здесь означает, что погашение заработало, а НЕ что перевод
    на htmx сломался. Чинить надо ПРАВИЛО (развести два транспорта на два
    токена), а не возвращать переигрывание ради зелени. Эта летопись стоит
    здесь именно затем, чтобы починка встретила объяснённый отказ.
    """
