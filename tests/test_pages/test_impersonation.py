...
async def _enter(client: AsyncClient, user_id: int):
    ...
    ⚠️ ФОРМА ОТВЕТА ЗДЕСЬ ЗАВИСИТ ОТ ТРАНСПОРТА КЛИЕНТА, И ЭТО НЕ ПОСЛАБЛЕНИЕ, А
    ПРЕДМЕТ (план 10-03). С фазы 10 вход отвечает через слой ответа: человеку без
    JavaScript — прежнее перенаправление на прежний адрес, слою письма — переход
    заголовком со статусом 204. Помощник зовут и те тесты, что берут фикстуру
    признака htmx рядом с админской (она ставит признак на ОБЩИЙ объект клиента),
    поэтому жёстко зашитая одна форма означала бы падение по транспорту, которым
    тест даже не интересуется.

    ⚠️ ЧТО НЕ ЗАВИСИТ ОТ ТРАНСПОРТА, УТВЕРЖДАЕТСЯ ПОСЛЕ РАЗВИЛКИ И НА ОБОИХ:
    адрес приземления и признак действующего лица в выданной cookie. Второе — и
    есть то, ради чего помощник вообще удостоверяется: cookie, потерянная на
    ответе 204, оставила бы администратора собой при полностью правильном
    заголовке перехода.
    """
    response = await _impersonate(client, user_id)

    if client.headers.get("HX-Request"):
        assert response.status_code == 204, (
            f"вход под пользователем ответил слою письма {response.status_code} "
            "вместо 204 — утверждение ниже проверяло бы обычную админскую сессию"
        )
    ...
    assert landing == "/dashboard", (
        f"вход под пользователем привёл не в продукт, а на {landing!r}"
    )
