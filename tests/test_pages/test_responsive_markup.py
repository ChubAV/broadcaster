...
def test_billing_component_library_did_not_grow():
    ...
    ⚠️ ЧИСЛО ФАЙЛОВ КАТАЛОГА КОМПОНЕНТОВ ЗДЕСЬ БОЛЬШЕ НЕ УТВЕРЖДАЕТСЯ (IN-04
    ревизии Фазы 9). Тест назван по разделу биллинга, а число, которое он
    сторожил, к биллингу отношения не имело: подниматься ему приходилось на
    файлы, которых ни один экран биллинга не зовёт (filter_chips, thumb,
    form_wrapper), и поднимать приходилось ДВЕ константы вместо одной. Второй
    подъём и был дефектом: он существовал только потому, что число стояло не
    там. Теперь оно утверждается ОДИН раз — в общем инвентаре шаблонов
    (test_template_inventory), — а этому тесту остался тот набор частей
    биллинга, который он называет своим именем.
    """
...
def test_template_inventory():
    ...
    # issue #40, единственный способ показать вложение.
    #
    # ⚠️ ЧИСЛО ФАЙЛОВ КАТАЛОГА КОМПОНЕНТОВ ОХРАНЯЕТСЯ ЗДЕСЬ И ТОЛЬКО ЗДЕСЬ
    # (IN-04 ревизии Фазы 9). Второе утверждение того же числа стояло в
    # test_billing_component_library_did_not_grow — тесте, названном по разделу,
    # которому это число не принадлежит, — и заставляло поднимать ДВЕ константы
    # на каждый новый файл каталога. Двойной подъём снят вместе с причиной,
    # его породившей: пополнение библиотеки краснит ровно один тест, и тест
    # этот назван по своему предмету.
    #
...
@pytest.mark.asyncio
async def test_the_messenger_icon_in_the_schedule_summary_is_not_squeezed(
    authed_client: AsyncClient, db_session: AsyncSession
):
    """Иконка мессенджера в шапке свёрнутой карточки не сжимается и не режется.

    Дефект issue #44: общее правило `.sched-card__sum > *` писалось под ТЕКСТ —
    `min-width: 0` разрешает flex-элементу сжаться ниже своего содержимого, а
    `overflow: hidden` превращает переполнение в ОБРЕЗКУ. Первым же ребёнком
    сводки стоит не текст, а графика — 30-пиксельный круглый глиф из
    includes/messenger_icon.html, — и её срезало по левому и правому краю всюду,
    где сводка переставала помещаться в строку.

    Тест держит ОБА конца контракта. Починить иконку СНЯТИЕМ общего правила
    значило бы обменять один дефект на другой: сводка перестала бы обрезаться
    многоточием и вытолкнула бы тумблер из шапки (UI-SPEC E7 overflow). Поэтому
    рядом с утверждением об исключении стоит утверждение о сохранности
    многоточия у текстового ребёнка.

    Утверждения о стилях идут по тексту БЕЗ комментариев (_css_without_comments):
    проверка по сырому тексту краснела бы от объяснения, которое несёт сама
    правка, и от запрета, называющего отвергнутый вариант.
    """
    ad, _schedule = await _seed_editor_schedule(db_session)

    # Карточка запрошена СВЁРНУТОЙ (без ?sched=) — ровно то её состояние, что на
    # скриншоте issue #44.
    html = (await authed_client.get(f"/ads/{ad.id}/edit")).text

    # Невакуумность: правило, адресованное несуществующей структуре, — тот же
    # мусор, что и стиль без разметки.
    sum_at = html.find('class="sched-card__sum"')
    assert sum_at != -1, "сводки в шапке карточки нет — правилу нечего адресовать"
    after_sum = html[sum_at:]
    icon_at = after_sum.find('class="msg"')
    text_at = after_sum.find("<span title=")
    assert icon_at != -1, "иконки мессенджера в сводке нет: посев не привязал аккаунт"
    assert text_at != -1, "текстового ребёнка сводки нет"
    assert icon_at < text_at, (
        "иконка перестала быть ПЕРВЫМ ребёнком сводки — исключение по классу "
        ".msg адресовано уже не тому месту"
    )

    css = _css_without_comments()

    # Исключение заведено. Разбор по ТЕЛУ блока, а не поиск строки целиком:
    # перестановка свойств внутри правила дефектом не является.
    exception = re.search(r"\.sched-card__sum\s*>\s*\.msg\s*\{([^}]*)\}", css)
    assert exception is not None, (
        "правила `.sched-card__sum > .msg` в таблице стилей нет: графика "
        "по-прежнему сжимается и обрезается общим правилом сводки"
    )
    exception_body = exception.group(1)
    assert re.search(r"flex:\s*none", exception_body), (
        "иконка осталась сжимаемым flex-элементом сводки (flex: none нет)"
    )
    assert re.search(r"overflow:\s*visible", exception_body), (
        "обрезка с иконки не снята (overflow: visible нет)"
    )

    # Многоточие сводки НЕ принесено в жертву: обрезается по-прежнему ТЕКСТ.
    general = re.search(r"\.sched-card__sum\s*>\s*\*\s*\{([^}]*)\}", css)
    assert general is not None, "общее правило сводки исчезло"
    general_body = general.group(1)
    for prop in ("min-width: 0", "overflow: hidden", "text-overflow: ellipsis"):
        assert prop in general_body, (
            f"общее правило сводки потеряло `{prop}`: сводка перестанет "
            "обрезаться многоточием и вытолкнет тумблер из шапки"
        )

    # Перелома под этот дефект не заведено: сжатие flex-элемента происходит на
    # ЛЮБОЙ ширине, где сводка перестаёт помещаться, а названная в issue ширина —
    # та, на которой её заметил сообщивший, а не порог правила.
    assert "1600" not in css, "под дефект заведён медиазапрос вместо правила каскада"


@pytest.mark.asyncio
async def test_the_schedule_save_form_does_not_swallow_the_rhythm_of_the_card_body(
    authed_client: AsyncClient, db_session: AsyncSession
):
    """Форма сохранения не гасит вертикальный ритм тела карточки (issue #45).

    Дефект: в развёрнутой карточке расписания подпись «ГРУППЫ · выбрано N из M»
    стоит вплотную к полосе чипсов аккаунтов, «ДНИ И ВРЕМЯ» — к кнопкам
    «ВЫБРАТЬ ВСЕ»/«СНЯТЬ ВСЕ», а строка «Время по …» ПЕРЕКРЫТА сверху залитой
    кнопкой «СОХРАНИТЬ РАСПИСАНИЕ», которая срезает нижние выносные элементы
    глифов. Причина у всех трёх одна и она структурная: `.sched-card__body`
    объявляет `gap: 16px`, но промежуток flex-контейнера действует между ЕГО
    ДЕТЬМИ, а прямые дети тела — подсказка и ДВЕ формы. Все `.sched-card__block`
    и завершающий `.sched-card__actions` лежат ВНУТРИ формы сохранения, которая
    сама промежутка не раздаёт. Между подблоками остаётся ровно 0.

    Контрольное наблюдение того же скриншота, подтверждающее диагноз: зазор
    между «СОХРАНИТЬ РАСПИСАНИЕ» и «УДАЛИТЬ РАСПИСАНИЕ» НОРМАЛЬНЫЙ — эти кнопки
    лежат в РАЗНЫХ формах, то есть являются прямыми детьми тела, и промежуток
    родителя до них доходит.

    Тест держит ОБА конца контракта. Правило, адресованное любой форме тела
    (`.sched-card__body > form` без `:has()`), захватило бы и форму удаления,
    чей единственный видимый ребёнок — кнопка «УДАЛИТЬ РАСПИСАНИЕ»: раскладка
    `stretch` растянула бы опасное действие во всю ширину карточки, и один
    дефект обменялся бы на другой. Поэтому форма регулярного выражения
    совпадения такому правилу НЕ даёт.
    ...
    # Карточка запрошена РАЗВЁРНУТОЙ: тело рендерится только под `expanded`, и
    # без параметра `sched` проверять было бы нечего.
    html = (await authed_client.get(f"/ads/{ad.id}/edit?sched={schedule.id}")).text

    # --- Группа A: невакуумность разметки -----------------------------------
    # Каждое утверждение ловит структурную перестановку, после которой правило
    # стилей адресовало бы пустоту.
    body_at = html.find('class="sched-card__body"')
    assert body_at != -1, "тела карточки в ответе нет — правилу нечего адресовать"

    save_at = html.find(f'action="/schedules/{schedule.id}/edit"')
    assert save_at != -1, "формы сохранения в ответе нет"
    assert body_at < save_at, (
        "форма сохранения перестала стоять внутри тела карточки — селектор "
        "`.sched-card__body > form` адресует уже не её"
    )

    # Машинная запись диагноза: ни один подблок не является прямым ребёнком
    # тела, и промежутку родителя нечего разделять.
    assert html.count('class="sched-card__block"', body_at, save_at) == 0, (
        "подблок оказался прямым ребёнком тела карточки — диагноз issue #45 "
        "устарел, и промежуток родителя до него теперь доходит сам"
    )

    # Вложенных форм в карточке нет (докстринг sched_card.html:13-19), поэтому
    # первый `</form>` закрывает именно форму сохранения.
    save_end = html.find("</form>", save_at)
    assert save_end != -1, "форма сохранения не закрыта"
    assert html.count('class="sched-card__block"', save_at, save_end) >= 2, (
        "подблоки перестали лежать внутри формы сохранения — обёртке нечего "
        "разделять, и правило адресовано пустоте"
    )

    # Форма удаления — СОСЕД формы сохранения, а не её потомок. Это контрольное
    # наблюдение скриншота и одновременно основание для группы B: селектор
    ...
    delete_at = html.find(f'action="/schedules/{schedule.id}/delete"')
    assert delete_at != -1, "формы удаления в ответе нет"
    assert save_end < delete_at, (
        "форма удаления перестала быть соседом формы сохранения — различать "
        "две формы тела больше нечем"
    )

    # --- Группа B: правило в таблице стилей ---------------------------------
    css = _css_without_comments()

    # Разбор по ТЕЛУ блока, а не поиск строки целиком: перестановка свойств
    # внутри правила дефектом не является. Дочерний комбинатор внутри `:has()`
    # необязателен, а вот само `:has()` — обязательно.
    wrapper = re.search(
        r"\.sched-card__body\s*>\s*form:has\(\s*>?\s*\.sched-card__block\s*\)\s*\{([^}]*)\}",
        css,
    )
    assert wrapper is not None, (
        "правила для формы С ПОДБЛОКАМИ внутри тела карточки в таблице стилей "
        "нет: либо правки нет вовсе и подблоки по-прежнему слиплись, либо она "
        "адресована ЛЮБОЙ форме тела и растянет кнопку «УДАЛИТЬ РАСПИСАНИЕ» "
        "во всю ширину карточки"
    )
    wrapper_body = wrapper.group(1)
    for prop in ("display: flex", "flex-direction: column", "min-width: 0"):
        assert prop in wrapper_body, (
            f"обёртка-форма не раздаёт ритм своим детям (`{prop}` нет)"
        )

    # Величина ритма объявлена ОДНИМ значением. Дочерний комбинатор в селекторе
    # правки не даёт этому выражению совпасть с ней.
    parent = re.search(r"\.sched-card__body\s*\{([^}]*)\}", css)
    assert parent is not None, "правило `.sched-card__body` исчезло"
    parent_body = parent.group(1)

    def _gap(rule_body: str) -> str | None:
        found = re.search(r"(?<![-\w])gap:\s*([^;}]+)", rule_body)
        return found.group(1).strip() if found else None

    assert _gap(parent_body) == "16px", (
        "ритм тела карточки перестал быть 16px — шагом UI-SPEC §Spacing Scale"
    )
    assert _gap(wrapper_body) == _gap(parent_body), (
        "обёртка завела СВОЮ величину промежутка вместо ритма родителя: "
        "нового токена расстояния под этот дефект не заводится"
    )

    # Ловит «починку» переносом ритма из родителя в обёртку: тело карточки
    # обязано остаться колонкой — между ним и формой удаления промежуток
    # работает и сегодня.
    for prop in ("display: flex", "flex-direction: column"):
        assert prop in parent_body, (
            f"тело карточки перестало быть flex-колонкой (`{prop}` нет): "
            "промежуток между двумя формами тела пропал вместе с ним"
        )
