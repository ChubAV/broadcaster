...
def image_key(user_id: int, name: str = "photo.jpg") -> str:
    """Ключ вложения ровно в той форме, которую выдаёт приём загрузки.

    Источник формы — сервис приёма загрузки в `app/services`:
    `{user_id}/{uuid4().hex}_{имя}`.

    ⚠️ ССЫЛКА ОПИСАНА СЛОВАМИ, А НЕ НАБРАНА ПУТЁМ (Фаза 12, план 12-05): прежний
    путь модуля маршрута и его адрес исчезли вместе с модулем, а критерий приёмки
    считает вхождения пути в ТЕКСТЕ файлов суиты.
    """
