...
⚠️ СЛЕПАЯ ЗОНА ОБХОДА НЕГОДНЫХ ВЕЛИЧИН — НАЗВАНА ЗДЕСЬ, А НЕ ОБНАРУЖЕНА
СЛЕДУЮЩИМ КРУГОМ. До третьего круга верификации Фазы 10 матрица негодных величин
(`UNUSABLE_AD_ID_VALUES`) прогонялась ТОЛЬКО ПО ОДНОЙ ОСИ — по телу формы, по
полю контекста экрана. Вторая ось, АДРЕС ЗАПРОСА, не обходилась вовсе: адрес был
зашит одной константой (`MISSING_SCHEDULE_URL`) с величиной ВНУТРИ диапазона
колонки, то есть на заведомо годном пути запроса. ⚠️ ПОЧЕМУ ЭТОГО НЕ БЫЛО ВИДНО:
прогон был ЗЕЛЁН, и зелёным он оставался бы одинаково и на починенном дереве, и
на сломанном — правило, чья константа лежит внутри диапазона, за которым живёт
дефект, до дефекта попросту НЕ ДОСТАЁТ. Предъявлять такой прогон доводом
закрытия нельзя; это записанное предупреждение таблицы Anti-Patterns отчёта
верификации третьего круга, строка `:2272` этого файла.

ЧЕМ ЗАКРЫТО (план 10-15): вторая константа адреса с величиной ВНЕ диапазона
(`OUT_OF_RANGE_SCHEDULE_URL`), прогон той же матрицы по оси адреса
(`UNUSABLE_PATH_ID_OUTCOMES` и её правило) с основанием у каждой записи
ожидаемого исхода, и ОБЪЯВЛЕННОЕ ЧИСЛО ОСЕЙ (`UNUSABLE_VALUE_AXES_DECLARED`) —
теперь ось, выпавшая из обхода, краснит прогон. Способность правила покраснеть
показана МУТАЦИЕЙ границы, а не заявлена прозой. ЧЕМ НЕ ЗАКРЫТО: границу самой
величины ставит не этот модуль, а приложение (план 10-12, `ID_MAX` и псевдонимы
в `app/pages/schedules.py`); идентификаторы того же класса в СОСЕДНИХ страничных
модулях ни одним правилом здесь не покрыты, и предмет этот шире файла.

⚠️ ИЗМЕРЕННАЯ ГРАНИЦА ТОГО, ЧТО ЭТОТ МОДУЛЬ ВООБЩЕ СПОСОБЕН УТВЕРЖДАТЬ. Он идёт
через транспорт БЕЗ ПРОЦЕССА БРАУЗЕРА, на встроенной базе, и не исполняет НИ
СТРОЧКИ разметочного рантайма. Он доказывает ДОСТАВКУ: что разметка и заголовки
пришли, что узел присутствует в теле ответа, что код и адрес приземления именно
те. Он НЕ доказывает ЭФФЕКТА: что снятие со страницы произошло, что осиротевшая
панель уехала, что ловушки фокуса в документе не осталось, что число на линейке
счётчика сдвинулось у человека на экране. Присутствие узла в ответе доказуемо
здесь; ПРИМЕНЕНИЕ узла рантаймом — нет. Эта половина принадлежит обходу
`10-UAT.md` и передана плану 10-16. ⚠️ ЗАПИСЬ НАЗЫВАЕТ ГРАНИЦУ, А НЕ СНИМАЕТ ЕЁ:
она не является утверждением, что вторая половина цели наблюдалась.

⚠️ ЧЕГО ЗДЕСЬ НЕ ЗАВОДИТСЯ НИ В КАКОМ ВИДЕ — МАШИННОЙ ПОДМЕНЫ РАЗМЕТОЧНОГО
РАНТАЙМА. Правило, объявившее, что внеполосный узел ПРИМЕНЁН, утверждало бы
ШИРЕ измеренного и вернуло бы фазе ровно тот дефект, которым она провалена
первым кругом: запись шире поведения. Планы 10-05…10-07 и 10-10 отказались от
такой подмены сознательно, и отказ продолжен здесь.
"""
import contextlib
import itertools
...
def _expected_oob_ids(schedule_id: int) -> set[str]:
    ...
    писалась. ИСТОЧНИК РАСХОЖДЕНИЯ НАЗЫВАЕТСЯ: план 10-50 (гэп `G-10-6`, обход
    `walkthrough_3` от 2026-09-11) добавил ЧЕТВЁРТЫЙ узел — сводку объявления
    `ad-summary`, — потому что линейка списка и сводка предпросмотра
    противоречили друг другу до следующей полной загрузки страницы. Четвёртый
    узел адресует цель СОБСТВЕННЫМ идентификатором (`id="ad-summary"` при
    `hx-swap-oob="true"`), поэтому в множество он входит БЕЗ решётки — в
    отличие от линейки счётчика, названной селектором внутри признака.
...
UNUSABLE_PATH_ID_OUTCOMES: dict[str, tuple[int, str]] = {
    ...
    #
    # ⚠️⚠️ ПОКОЛЕНИЕ ТРЁХ СТРОК НИЖЕ (`9…9`, `-1`, `0`; Фаза 11, план 11-02,
    # решение D-07). Абзацы о границе `le=`/`ge=` псевдонима `ScheduleIdPath`
    # оставлены ДОСЛОВНО (идиома D-30/D-32): они верны для дерева, на котором
    # писались. ОПРОВЕРГНУТО ожидание `422`: граница величины уехала ВНУТРЬ
    # обработчика (`id_in_column`, `app/pages/identifiers.py`) и стоит там ДО
    # выборки, а величина вне колонки идёт ТОЙ ЖЕ веткой, что несуществующий
    # идентификатор, — ровно исход строки `1_0` ниже (фрагмент, 200). Предмет
    # «не 500 и не четвёртая форма» не ослаблен; `422` остаётся только у ФОРМЫ
    # ЗАПИСИ (`1e3`), которую отвергает разбор ТИПА фреймворка вне предмета D-07.
    "9" * 25: (200, ""),
    ...
    # БЕЗ границы.
    #
    # ⚠️ ПОКОЛЕНИЕ ИСХОДА (Фаза 11, план 11-07, T-07-13). Абзац выше оставлен
    # ДОСЛОВНО (идиома D-30/D-32): он верен — отвергает по-прежнему коэрция типа,
    # и на дереве без границы величины исход был бы тем же. ОПРОВЕРГНУТ РОВНО
    # КОД: запрос идёт ПОД ПРИЗНАКОМ htmx (`_post_with_path_identifier` шлёт
    # `HTMX_HEADERS`) и адресован СТРАНИЧНОМУ слою, а с настоящего плана такой
    # отказ валидации отвечает ПУСТЫМ 400 вместо тела фреймворка с эхо ввода.
    # Тело здесь было СТОКОМ: правило `422` с плана 11-09 свопает его в DOM.
    # Прежнее ожидание `422` держится на ЭТОМ ЖЕ маршруте БЕЗ признака htmx и
    # у JSON-API на обоих транспортах — оба закрыты обходами
...
async def test_an_unusable_path_identifier_never_reaches_the_database(
    ...
    for value in UNUSABLE_AD_ID_VALUES:
        ...
        # ⚠️ 400 ДОПУЩЕН ЗДЕСЬ ПЛАНОМ 11-07, И ЭТО НЕ ПОСЛАБЛЕНИЕ ПЕРЕЧНЯ.
        # Отказ валидации на пути htmx у страничного слоя отвечает ПУСТЫМ 400
        # (T-07-13): тело фреймворка с эхо ввода было бы стоком свопа. Это ТА ЖЕ
        # форма «отказ валидации», что и 422 соседом, — сменился её код, а не
        # состав перечня, и по величине она по-прежнему НЕРАЗЛИЧИМА, то есть
        # карты занятых идентификаторов не выдаёт (T-10-07).
        elif response.status_code not in (200, 204, 400, 422):
            complaints.append(
...
async def test_the_counter_node_is_characterised_as_belonging_to_the_ad_named_by_the_request(
    client: AsyncClient, db_session: AsyncSession, test_settings
):
    """ХАРАКТЕРИЗАЦИЯ: линейка несёт число объявления, ВЛАДЕЮЩЕГО удалённой строкой.

    ⚠️ ЗЕЛЁНЫЙ ЦВЕТ ЭТОГО ПРАВИЛА НЕ ЕСТЬ УТВЕРЖДЕНИЕ О ПРАВИЛЬНОСТИ (WR-05,
    ревизия 2026-09-11). Оно ОПИСЫВАЕТ сегодняшнее поведение, признанное
    ДЕФЕКТНЫМ и записанное открытым окном 49 журнала `.planning/WINDOWS.md`:
    цели `#sched-count` и `#ad-summary` адресованы СТАТИЧЕСКИ, а числа считаются
    по `ad_id` НАЙДЕННОЙ строки, — запрос, называющий адресом расписание
    объявления B, а полем контекста экран объявления A, ставит в документ A
    числа объявления B, и они переживают запрос до перезагрузки.

    ПРЕЖНЕЕ ИМЯ (`..._belongs_to_the_ad_named_by_the_request`) УТВЕРЖДАЛО ЭТО
    ПОВЕДЕНИЕ НОРМОЙ. Цена такого имени отложенная и вполне конкретная: при
    следующей попытке ПОЧИНИТЬ предмет правило покраснеет — и красный цвет будет
    прочитан как регресс, то есть починку откатят, сославшись на суиту. Имя и
    отметка вместе говорят читателю: это слепок, а не норма; чинить предмет
    правило не запрещает, а сопровождать починку обязано ПЕРЕПИСЫВАНИЕМ, а не
    откатом.

    ПОЧЕМУ ПРЕДМЕТ НЕ ПОЧИНЕН ЗДЕСЬ. Решение отложить правку записано владельцем
    (окно 49: правка меняет цель узла у ВСЕХ его потребителей, а наблюдается она
    шагом 1.8 обхода, не пройденным ни разу — опоры для суждения о правке нет).
    Ревизия предъявила претензию не к этому решению, а к ИМЕНИ правила; имя и
    исправлено.
