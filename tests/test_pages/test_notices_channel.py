...
3. ЧИСЛО МЕСТ ЗАПИСИ ВЫПИСАНО ЗДЕСЬ РУКАМИ И БУДЕТ МЕНЯТЬСЯ. Собственное число
   ...
   ⚠️ С ФАЗЫ 10 ЭТО ЧИСЛО УБЫВАЮЩЕЕ, И СЧИТАЕТ ОНО АДРЕСА, СОБРАННЫЕ РУКАМИ.
   Обработчик, переведённый на слой ответа, передаёт код ПАРАМЕТРОМ, и адрес с
   кодом ему собирает единственная на проект сборка — то есть места записи
   ЗДЕСЬ у него не остаётся, а исход человеку доезжает по-прежнему. Что исход
   не пропал, утверждает НЕ это число, а второй обход (граница 1): код
   упоминается константой реестра в своём обработчике, и правило осиротевших
   кодов краснеет ровно тогда, когда исход исчезает на самом деле.
...
#
# ЛЕТОПИСЬ ЧИСЛА:
#   12 → 6, Фаза 10, план 10-03. ИСТОЧНИК ДВИЖЕНИЯ: шесть адресов перестали
#   собираться РУКАМИ. Повтор отправки из журнала (четыре кода) и перезапуск
#   контейнера воркера (два кода) переведены на слой ответа, где код едет
#   ПАРАМЕТРОМ, а адрес с кодом собирает единственная на проект сборка.
#   ⚠️ ЭТО НЕ ПРОПАЖА ИСХОДОВ, И ОТЛИЧИЕ ПРОВЕРЯЕТСЯ, А НЕ ДЕКЛАРИРУЕТСЯ. Все
#   шесть кодов по-прежнему УПОМИНАЮТСЯ константами реестра в своих
#   обработчиках, поэтому второй обход этого файла (граница 1) видит их
#   по-прежнему, и правило осиротевших кодов осталось зелёным. Пропади исход
#   на самом деле — покраснело бы именно оно, а не это число.
#   ⚠️ ЧИСЛО СТАЛО УБЫВАЮЩИМ. Оно считает адреса, собранные РУКАМИ, а веха
#   переводит разделы на слой ответа один за другим: каждая следующая фаза
#   опускает его дальше, и ноль означал бы, что ни один адрес с кодом больше не
#   склеивается вне единственной сборки.
NOTICE_WRITE_PLACES = 6
