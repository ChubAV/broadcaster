...
1. КОД, СОБРАННЫЙ ПОДСТАНОВКОЙ ВО ВРЕМЯ ИСПОЛНЕНИЯ, из адресной строки
   исходника не читается. ⚠️ ПРЕЖНЯЯ РЕДАКЦИЯ ЭТОЙ ГРАНИЦЫ НЕ ВЫЧЁРКНУТА, А
   ПОМЕЧЕНА (идиома D-30/D-32): она гласила, что такое место в продукте РОВНО
   ОДНО — помощник страничного слоя расписаний, возвращавший человека в редактор
   с кодом исхода, где адрес был общим на два исхода. Она была верна для дерева,
   на котором писалась.
   ЧЕМ СНЯТА: планом 11-05 у того помощника не осталось вызывающих (правка,
   тумблер и создание расписания ушли на слой ответа), и он удалён вместе со
   своей подстановкой. ДЕЙСТВУЮЩЕЕ УТВЕРЖДЕНИЕ: мест, собирающих код
   подстановкой во время исполнения, в продукте НЕ ОСТАЛОСЬ НИ ОДНОГО — все
   оставшиеся места записи выписывают код константой реестра прямо в адресной
   строке. Граница при этом закрыта по-прежнему не молчанием, а вторым обходом:
   гейт собирает все УПОМИНАНИЯ констант реестра в исходниках приложения, и код,
   уехавший ПАРАМЕТРОМ в слой ответа, попадает в счёт через свой обработчик.
   Обход мимо реестра поэтому невозможен и здесь.
...
# СНЯТЫЕ НАПИСАНИЯ, выписанные литералами. Каждое было артефактом редиректа:
# порождалось сервером, потреблялось один раз и никем не закладывалось —
# поэтому снятие шло без периода совместимости (D-09).
#
# ЛЕТОПИСЬ ПЕРЕЧНЯ:
#   5, Фаза 8, план 08-06: пять написаний свода.
#   5 → 6, Фаза 11, план 11-14, D-10: шестой частный микро-контракт сведён в
#   `?notice=`. Ключ исхода снятия задачи из очереди в свод Фазы 8 не входил:
#   у подраздела очереди жили свой закрытый словарь исходов и своё место
#   отрисовки, и решение D-07 Фазы 10 отдало свод Фазе 11. Снят сразу, без
#   периода совместимости, по тому же доводу D-09.
#   ⚠️ ФОРМА ЗАПИСИ ТА ЖЕ, ЧТО У ПЯТИ (`?<ключ>=`), И ОТ НЕЁ ЗАВИСИТ ОХВАТ.
#   Правило ищет ПОДСТРОКУ: у всех трёх снятых мест записи ключ открывал строку
#   запроса, поэтому именно эта форма их и называет. Вариант с присоединением
#   (`&<ключ>=`) формой не покрыт — так же, как у пяти соседей.
RETIRED_QUERY_KEYS = (
    "?error=",
    "?saved=",
    "?reset=",
...
#
#   6 → 5, Фаза 11, план 11-05. ИСТОЧНИК ДВИЖЕНИЯ: снят помощник страничного
#   слоя расписаний, возвращавший человека в редактор своего объявления с кодом
#   исхода. Вызывающих у него не осталось: правку, тумблер и СОЗДАНИЕ расписания
#   веха перевела на слой ответа, и код исхода едет туда ПАРАМЕТРОМ, а адрес с
#   кодом склеивает единственная на проект сборка.
#   ⚠️ ЭТО НЕ ПРОПАЖА ИСХОДА, И ОТЛИЧИЕ ПРОВЕРЯЕТСЯ, А НЕ ДЕКЛАРИРУЕТСЯ. Оба
#   кода снятого места по-прежнему УПОМИНАЮТСЯ константами реестра в своих
#   обработчиках, поэтому второй обход этого файла (граница 1) видит их
#   по-прежнему, и правило осиротевших кодов осталось зелёным.
#   ⚠️ ЧИСЛО ПОСТАВЛЕНО ПРОГОНОМ ПОКРАСНЕВШЕГО ПРАВИЛА. Вывод
#   `test_the_number_of_notice_writers_is_the_declared_one`, дословно: «мест
#   записи в страничном слое 5, а объявлено 6».
#
#   5 → 4, Фаза 11, план 11-09. ИСТОЧНИК ДВИЖЕНИЯ: СОХРАНЕНИЕ ЧАСОВОГО ПОЯСА В
#   ПРОФИЛЕ переведено на слой ответа. Адрес с кодом исхода перестал склеиваться
#   РУКАМИ в обработчике: код едет слою ответа ПАРАМЕТРОМ, а адрес с ним
#   собирает единственная на проект сборка.
#   ⚠️ ЭТО НЕ ПРОПАЖА ИСХОДА, И ОТЛИЧИЕ ПРОВЕРЯЕТСЯ, А НЕ ДЕКЛАРИРУЕТСЯ. Код
#   сохранённого профиля по-прежнему УПОМИНАЕТСЯ константой реестра в своём
#   обработчике, поэтому второй обход этого файла (граница 1) видит его
#   по-прежнему, и правило осиротевших кодов осталось зелёным. Пропади исход на
#   самом деле — покраснело бы именно оно, а не это число. Вдобавок этот исход
#   ВПЕРВЫЕ в вехе едет и на ветке фрагмента — внеполосным блоком уведомления.
#   ⚠️ ЧИСЛО ПОСТАВЛЕНО ПРОГОНОМ ПОКРАСНЕВШЕГО ПРАВИЛА, А НЕ ВЫЧИТАНИЕМ. Вывод
#   `test_the_number_of_notice_writers_is_the_declared_one`, дословно: «мест
#   записи в страничном слое 4, а объявлено 5».
#   ⚠️ ЭТО ПРАВИЛО НЕ ВХОДИЛО В ПРОВЕРОЧНЫЙ НАБОР ПЛАНА 11-09 и было найдено
#   прогоном соседних модулей — тот же класс, что и стареющий счётчик плана
#   11-06: счётчик, которого план не называет, двигается его правкой молча.
#
#   4 → 1, Фаза 11, план 11-15. ИСТОЧНИК ДВИЖЕНИЯ: ОФОРМЛЕНИЕ ДОСТУПА С УХОДОМ
#   НА СТРАНИЦУ ЮKassa переведено на слой ответа. Три адреса страницы подписки
#   с кодом причины (платежи выключены, незакрытое намерение, сбой создания)
#   перестали склеиваться РУКАМИ: коды едут слою ответа ПАРАМЕТРОМ, а адрес с
#   ними собирает единственная на проект сборка. Оставшееся место записи в
#   страничном слое — в разделе входа.
#   ⚠️ ЭТО НЕ ПРОПАЖА ИСХОДОВ, И ОТЛИЧИЕ ПРОВЕРЯЕТСЯ, А НЕ ДЕКЛАРИРУЕТСЯ. Все
#   три кода по-прежнему УПОМИНАЮТСЯ константами реестра в обработчике, и
#   правило осиротевших кодов на том же прогоне зелёное.
#   ⚠️ ЧИСЛО ПОСТАВЛЕНО ПРОГОНОМ ПОКРАСНЕВШЕГО ПРАВИЛА. Вывод
#   `test_the_number_of_notice_writers_is_the_declared_one`, дословно: «мест
#   записи в страничном слое 1, а объявлено 4: {'app/dependencies.py': 1,
#   'app/pages/auth.py': 1}».
#   ⚠️ ПРАВИЛО НЕ ВХОДИЛО В ПРОВЕРОЧНЫЙ НАБОР ПЛАНА 11-15 И ПОЙМАНО ПОЛНЫМ
#   ПРОГОНОМ ИСПОЛНИТЕЛЯ ДО МЕТАДАННЫХ — тот же класс, что у планов 11-06 и
#   11-09.
#
#   1 → 0, Фаза 14, план 14-05. ИСТОЧНИК ДВИЖЕНИЯ: НОВЫЙ ПАРОЛЬ (последнее место
#   записи страничного слоя, раздел входа) переведён на слой ответа. Адрес
#   экрана входа с кодом смены пароля перестал склеиваться РУКАМИ в
#   обработчике: код едет выходу полной перезагрузки ПАРАМЕТРОМ, а адрес с ним
#   собирает единственная на проект сборка (D-10).
#   ⚠️ ЭТО НЕ ПРОПАЖА ИСХОДА, И ОТЛИЧИЕ ПРОВЕРЯЕТСЯ, А НЕ ДЕКЛАРИРУЕТСЯ. Код
#   смены пароля по-прежнему УПОМИНАЕТСЯ константой реестра в обработчике, и
#   правило осиротевших кодов на том же прогоне зелёное; рисует его область
#   уведомления шелла — это утверждает правило
#   `test_a_new_password_leaves_for_the_login_by_a_full_load_with_the_notice`.
#   ⚠️ НОЛЬ ЗДЕСЬ НЕ ВАКУУМ СЛОМАННОГО ОБХОДА: тем же прогоном обход находит
#   место записи вне страничного слоя (`app/dependencies.py`, число ниже), то
#   есть выражение по-прежнему видит запись там, где она есть.
#   ⚠️ ЧИСЛО ПОСТАВЛЕНО ПРОГОНОМ ПОКРАСНЕВШЕГО ПРАВИЛА. Вывод
#   `test_the_number_of_notice_writers_is_the_declared_one`, дословно: «мест
#   записи в страничном слое 0, а объявлено 1: {'app/dependencies.py': 1}».
#   ⚠️ ПРАВИЛО НЕ ВХОДИЛО В ПРОВЕРОЧНЫЙ НАБОР ПЛАНА 14-05 И ПОЙМАНО ПРОГОНОМ
#   СОСЕДНИХ МОДУЛЕЙ ДО МЕТАДАННЫХ — тот же класс, что у планов 11-06, 11-09 и
#   11-15.
NOTICE_WRITE_PLACES = 0
...
def test_control_negative_a_returned_queue_outcome_key_reddens_the_gate():
    """ЧТО ДОКАЗЫВАЕТ: гейт ловит вернувшийся ШЕСТОЙ снятый ключ (план 11-14).

    ⚠️ КОНТРОЛЬ ЗАВЕДЁН ИМЕННО НА НОВЫЙ КЛЮЧ, А НЕ ПОЛОЖЕН НА СОСЕДНИЙ. Контроль
    выше доказывает зубы на пятёрке Фазы 8; ноль шестого ключа на настоящем
    дереве согласуется и с перечнем, до которого ключ не дописан или дописан с
    опечаткой, — и тогда зелёный гейт утверждал бы то, чего не проверяет.
    Подмена идёт в ПАМЯТИ и в тот модуль, где ключ жил: старое написание на
    диск не пишется ни в какой момент.
    """
    module = "app/pages/admin.py"
    original = _scanned_sources()[module]
    assert "?result=" not in original, (
        "старое написание уже есть в настоящем модуле — контроль не отличил бы "
        "подмену от дерева"
    )
    returned = original + '\n_A_FUTURE_PHASE_MIGHT_WRITE = "/admin/queue?result=removed"\n'

    hits = _retired_key_hits(_sources_with(module, returned))

    assert "?result=" in hits and hits["?result="] == [module], (
        "ГЕЙТ НЕ ЗАМЕТИЛ ВЕРНУВШИЙСЯ КЛЮЧ ИСХОДА ОЧЕРЕДИ — у исхода снятия "
        f"задачи снова заведётся второй владелец мимо реестра: {hits}"
    )
