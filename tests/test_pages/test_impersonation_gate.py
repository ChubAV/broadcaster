...
# прежний JSON-вход загрузки пока стоял на месте — его снял план 12-05 вместе с
# переездом его суиты. Окно сосуществования двух входов было названо здесь, а не
# оставлено на догадку: число обещало вернуться к 49 тем же планом, которым уйдёт
# старый модуль. Значение поставлено ПРОГОНОМ покрасневшего правила, а не
# вычитанием в уме.
#
# 50 → 49 (Фаза 12, план 12-05): ОБЕЩАНИЕ ПРЕДЫДУЩЕЙ СТРОКИ ИСПОЛНЕНО — модуль
# маршрута загрузки слоя JSON-API снят ЦЕЛИКОМ (D-03), и его изменяющее
# объявление исчезло из обхода вместе с ним. ⚠️ ЭТО ДВИЖЕНИЕ ВНИЗ, И ОНО НЕ
# «УБРАЛИ ЛИШНЕЕ»: вход не исчез, а ПЕРЕЕХАЛ на страничную поверхность, где и был
# посчитан прошлой строкой. Маршрутов стало меньше на один, а входов загрузки
# по-прежнему один. Значение поставлено ПРОГОНОМ покрасневшего правила; вывод
# дословно: `изменяющих маршрутов стало 49, а выписано 50` / `assert 49 == 50`.
#
# 49 → 49 (ДВИЖЕНИЯ НЕТ), Фаза 13, план 13-01: завершение QR-входа мастера
# Telegram СНЯТО, а опрос статуса того же мастера стал POST (D-01 Фазы 13) —
# минус один изменяющий маршрут, плюс один. Модуль тот же, поэтому
# `MUTATING_MODULE_COUNT` не двигается тоже. ⚠️ Неподвижность ИЗМЕРЕНА прогоном:
# покраснело только правило классификации, дословно `в проекте есть изменяющие
# маршруты, о которых этот гейт не знает:
# app/pages/accounts.py::accounts_connect_tg_user_qr_status`, а правило числа
# осталось зелёным; запись завершения в `ALLOWED_ROUTES` заменена записью опроса.
MUTATING_ROUTE_COUNT = 49

# Столько модулей обоих слоёв объявляют изменяющие маршруты.
#
# 15 → 14 (Фаза 12, план 12-05): модуль маршрута загрузки слоя JSON-API снят
# целиком, и объявлявших изменяющие маршруты модулей стало на один меньше.
# ⚠️ ЧИСЛО ДВИНУЛОСЬ ЗДЕСЬ, А НЕ ПЛАНОМ 12-01, И РАЗНИЦА СОДЕРЖАТЕЛЬНА: новый
# маршрут завёлся в `app/pages/ads.py`, который изменяющие маршруты объявлял и
# ДО него, поэтому число модулей тогда не шевельнулось, хотя число маршрутов
# выросло. Значение поставлено ПРОГОНОМ покрасневшего правила — второе
# утверждение того же теста, и увидеть его удалось лишь после правки первого:
# первое утверждение до него не допускало. Вывод дословно: `модулей с
# изменяющими маршрутами стало 14, а выписано 15` / `assert 14 == 15`.
MUTATING_MODULE_COUNT = 14
...
ALLOWED_ROUTES = {
    ...
    # Фаза 13, план 13-01: запись завершения входа заменена записью опроса —
    # маршрут завершения снят, и аккаунт сохраняет сам опрос (D-01 Фазы 13).
    "app/pages/accounts.py::accounts_connect_tg_user_qr_status": (
        "подключение аккаунта — опрос сохраняет аккаунт (D-01 Фазы 13)"
    ),
    ...
    # ⚠️ ЗАПИСЬ ПЕРЕНАЦЕЛЕНА НА НОВЫЙ ОБРАБОТЧИК С СОХРАНЁННОЙ ПРИЧИНОЙ (Фаза 12,
    # план 12-05), А НЕ ЗАВЕДЕНА ЗАНОВО. Прежде рядом стояла вторая запись — на
    # обработчик снятого модуля маршрута загрузки слоя JSON-API, — и окно
    # сосуществования двух входов было названо здесь планом 12-01. Окно закрыто:
    # старый модуль снят целиком вместе со своей суитой, и запись о нём ушла тем
    # же коммитом. Причина осталась ДОСЛОВНО той же, и это существенно: решение
    # о загрузке картинки под чужой личностью от транспорта не зависит, а
    # переписанная заодно с переездом причина выглядела бы новым решением.
    #
    # ⚠️ ЗАПИСЬ ЗДЕСЬ ОБЯЗАТЕЛЬНА, И ЭТО ЗАЯВЛЕННОЕ СВОЙСТВО ФОРМЫ, А НЕ
    # ПЕДАНТИЗМ: обход читает ОБА каталога маршрутов, и изменяющее объявление, не
    # попавшее ни в одно множество, роняет гейт (см. абзац о маршруте «отправить
    # сейчас» ниже).
    #
    # ⚠️ ПЕРЕЧНЕЙ, УТВЕРЖДАЮЩИХ РЕШЕНИЕ О ПОВЕРХНОСТИ ЭТОГО МАРШРУТА, ТРИ, А НЕ
    # ДВА. Критерий 3 ROADMAP-а называет только два — `GATED_API_ROUTERS` и
    # `BLOCK_CHECKED_API_ROUTERS` в `tests/test_pages/test_access_gate.py`, — а
    # третий вот этот. Записано затем, чтобы следующий читатель не искал третий
    # заново: разведка Фазы 12 нашла его находкой G-7, и найден он был потому,
    # что гейт покраснел, а не потому, что критерий на него указал.
