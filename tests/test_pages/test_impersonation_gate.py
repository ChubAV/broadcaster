...
# именно произошло.
#
# 49 → 50 (Фаза 12, план 12-01): страничный маршрут загрузки вложений
# `app/pages/ads.py::ads_images_upload` заведён ВНУТРИ `ads_router` (D-01), а
# JSON-вход `app/routes/uploads.py::upload_image` пока стои́т на месте — его
# снимает план 12-05 вместе с переездом его суиты. Окно сосуществования двух
# входов названо здесь, а не оставлено на догадку: число вернётся к 49 тем же
# планом, которым уйдёт старый модуль. Значение поставлено ПРОГОНОМ
# покрасневшего правила, а не вычитанием в уме.
MUTATING_ROUTE_COUNT = 50
...
ALLOWED_ROUTES = {
    ...
    "app/pages/ads.py::ads_images_upload": "загрузка картинки объявления",
    # ⚠️ ДВА ВХОДА ЗАГРУЗКИ ОДНОВРЕМЕННО — ОКНО ПЕРЕЕЗДА, А НЕ ДУБЛЬ РЕШЕНИЯ
    # (Фаза 12). Страничный маршрут выше заведён планом 12-01; JSON-вход ниже
    # снимает план 12-05 ВМЕСТЕ со своей суитой в 1099 строк. Снять его раньше
    # значило бы покраснить эти утверждения, которым ещё нечем заменить предмет.
    # Причина у обеих записей ОДНА и та же дословно: решение о загрузке картинки
    # под чужой личностью от транспорта не зависит.
