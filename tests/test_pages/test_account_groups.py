...
async def test_header_carries_the_sync_form(
    ...
    # Фаза 11, план 11-17: тег формы печатает макрос-обёртка htmx-формы, и метод
    # у него набран строчными (`method="post"`); регистр значения метода HTML не
    # различает, поэтому сличение без учёта регистра утверждает то же свойство.
    assert re.search(
        rf'<form method="post" action="/accounts/{account.id}/sync-groups"',
        html,
        re.IGNORECASE,
...
#
# 60 → 61, Фаза 11, план 11-17: форма ЗАПУСКА СИНХРОНИЗАЦИИ в шапке экрана групп
# пошла через макрос-обёртку htmx-формы и стала ещё одним местом, отправляющим
# POST атрибутом запроса, — подстановка попадает и в неё. Строки списка не
# двигались: прибавка ровно одна, по числу форм шапки.
# ПОСТАВЛЕНО ПРОГОНОМ покрасневшего
# `test_no_declared_include_selector_names_an_id_the_document_lacks`, дословно:
# `подстановка вернула 61 объявлений вместо измеренных 60 — механизм подстановки
# перестал попадать в места подтверждения, и утверждения ниже говорили бы не о
# том документе`.
SUBSTITUTED_INCLUDE_DECLARATIONS_MEASURED = 61
