...
#
# Число ИЗМЕРЕНО на живом дереве, а не выведено ожиданием: посев из трёх групп
# даёт по одному объявлению на строку списка (панель подтверждения строки —
# единственное место экрана, объявляющее включаемые данные), и других
# объявлений отрисованный документ экрана не несёт.
INCLUDE_DECLARATIONS_MEASURED = 3
...
async def test_no_declared_include_selector_names_an_id_the_document_lacks(
    ...
    * у аккаунта крупнее сентинел на первой странице есть, но после
    ...
    assert len(declarations) == INCLUDE_DECLARATIONS_MEASURED, (
        f"объявлений включаемых данных на странице {len(declarations)}, а "
        f"измерено было {INCLUDE_DECLARATIONS_MEASURED}: либо разборщик "
        f"перестал их находить (и правило ниже зеленеет на пустом множестве), "
        f"либо объявлений стало больше, и решения об этом никто не принимал"
    )
    assert declarations, (
        "объявлений включаемых данных не найдено ни одного — правило ниже "
        "зелено ПО ПОСТРОЕНИЮ: разность пустого множества с любым пуста"
    )
    ...
    assert not unreachable_after, (
        f"панель подтверждения объявляет источник включаемых данных, которого "
        f"документ не несёт: {sorted(unreachable_after)} — положение экрана "
        ...
        f"на первичной отрисовке цель ещё была"
    )
