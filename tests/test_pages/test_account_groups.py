...
# подстроиться молча.
INCLUDE_DECLARATIONS_MEASURED = 30
...
INCLUDE_TARGET_EXCEPTIONS_DECLARED = 1
...
async def test_no_declared_include_selector_names_an_id_the_document_lacks(
    ...
    * у аккаунта крупнее сентинел на первой странице есть, но после
    ...
    # --- Антивакуум живёт ЗДЕСЬ: это единственное из трёх положений, где
    #     объявления есть по замыслу. Без этого утверждения правило зеленело бы
    #     от того, что разборщик перестал находить объявления ВОВСЕ.
    ...
    assert len(big_declarations) == INCLUDE_DECLARATIONS_MEASURED, (
        ...
        f"{INCLUDE_DECLARATIONS_MEASURED}: либо разборщик перестал их находить "
        f"(и утверждения ниже зеленеют на пустом множестве), либо объявлений "
        f"стало больше или меньше, и решения об этом никто не принимал"
    )
    assert big_declarations, (
        "объявлений включаемых данных не найдено ни одного — утверждения ниже "
        "зелены ПО ПОСТРОЕНИЮ: разность пустого множества с любым пуста"
    )

    # --- Пока следующая порция есть, цель объявления в документе СТОИТ, и
    #     объявление обязано быть достижимым без всяких исключений.
    unreachable_on_a_full_page = _include_selectors(big_page) - _document_ids(
        big_page
    )
    assert not unreachable_on_a_full_page, (
        f"панель подтверждения объявляет источник включаемых данных, которого "
        f"документ не несёт: {sorted(unreachable_on_a_full_page)} — положение "
        f"экрана «следующая порция ЕСТЬ, узел курсора в документе стоит». Это "
        f"положение исключений не имеет и иметь не может: цель здесь есть, и "
        f"недостижимой она может стать только от потерянного идентификатора"
    )
    ...
    glued_declarations = INCLUDE_ATTR_RE.findall(glued)
    assert glued_declarations, (
        "в склеенном документе объявлений включаемых данных нет вовсе — "
        "утверждение ниже зелено по построению"
    )

    unreachable_after = _include_selectors(glued) - _document_ids(glued)
    undeclared = unreachable_after - set(INCLUDE_TARGET_EXCEPTIONS)
    assert not undeclared, (
        f"панель подтверждения объявляет источник включаемых данных, которого "
        f"документ не несёт: {sorted(undeclared)} — положение экрана "
        f"«долистали до конца, сентинел ушёл из документа насовсем». Строка в "
        f"консоли вернётся на КАЖДОЕ удаление и здесь, на строках, у которых "
        f"на первичной отрисовке цель ещё была. Записи в "
        f"INCLUDE_TARGET_EXCEPTIONS у этой цели нет — то есть это НОВОЕ "
        f"расхождение, а не принятое владельцем отступление"
    )
...
#
# --- ЧИСЛО КОНТРОЛЕЙ ФАЙЛА: 1 ------------------------------------------------
#
# Один положительный и ни одного отрицательного. Число записано ЗДЕСЬ затем,
...
# взята целиком.
#
# ⚠️ ЧИСЛО ДВИНЕТСЯ ЕЩЁ ДВАЖДЫ В ЭТОМ ЖЕ КРУГЕ, И КАЖДОЕ ДВИЖЕНИЕ ОБЯЗАНО
# ПОЛУЧИТЬ СВОЮ СТРОКУ ЛЕТОПИСИ, а не пройти молча: задача 4 плана 09-13
# (`test_control_negative_an_include_declaration_without_a_target_reddens_the_gate`
# — только при ветви, обнулившей `INCLUDE_DECLARATIONS_MEASURED`) и задача 1
# плана 09-14
