...
async def test_a_blocked_user_gets_no_cookie_from_the_page_login(
    ...
    # Фаза 14, план 14-01, D-03: отказ во входе — 422, а не 200. Человек
    # остаётся на экране входа, и код один на оба транспорта; прежнее 200 было
    # верно для дерева, где отказ отвечал готовой страницей.
    assert response.status_code == 422, (
        f"отказ во входе ответил {response.status_code} вместо страницы входа "
        f"со словами и кодом 422"
    )
