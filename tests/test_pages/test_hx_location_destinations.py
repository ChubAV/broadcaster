...
#   — 77 → 79, Фаза 14, план 14-06: ВОЗВРАТ ИЗ-ПОД ЧУЖОЙ ЛИЧНОСТИ переведён на
#     слой ответа. Это ПЕРВОЕ движение числа на ДВА от ОДНОГО обработчика, и
#     причина предметная: у возврата ДВЕ ветки уходят главным выходом БЕЗ
#     фрагмента — успех на `/admin` и «действующего лица нет» на `/dashboard`
#     (D-12). Подавать фрагмент здесь нечему: у формы возврата нет цели подмены,
#     любой её исход есть переход. Третья ветка (закрытое либо исчезнувшее
#     действующее лицо) уходит выходом ПОЛНОЙ ПЕРЕЗАГРУЗКИ на `/login` и в этот
#     счёт не входит — он считает заголовок ЧАСТИЧНОГО перехода; отказ по
#     источнику отвечает голым 403 (D-01) и вызовом слоя ответа не является.
#     Адрес `/dashboard` уже был в карте; `/admin` пришёл В КАРТУ ВПЕРВЫЕ —
#     первое приземление перехода на админский обзор за всю веху.
#     ПОСТАВЛЕНО ПРОГОНОМ: `вызовов слоя ответа БЕЗ фрагмента найдено 79, а
#     объявлено 77` (покрасневшее правило, до записи числа; Фаза 14, план 14-06).
#     Тем же прогоном `test_every_transition_destination_has_a_declared_template`
#     сказал дословно: `адреса приземления без сопоставленного шаблона:
#     ['/admin']`.
HX_LOCATION_DESTINATION_CALLS_DECLARED = 79
...
HX_LOCATION_DESTINATION_TEMPLATES = {
    ...
    # Фаза 14, план 14-06: успех возврата из-под чужой личности (D-12).
    # Страница наследует `base.html` и инлайн-скриптов не несёт, поэтому правило
    # двойного исполнения на её цепи зелено: подменённое содержимое `body`
    # второго набора слушателей не поднимает.
