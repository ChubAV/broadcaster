...
async def test_retry_confirmation_panel_names_the_current_ad_content(
    ...
    ⚠️ ОКНО ПОИСКА ОГРАНИЧЕНО СОБСТВЕННОЙ ФОРМОЙ ПАНЕЛИ, А НЕ ДЛИНОЙ В
    СИМВОЛАХ (Фаза 10, план 10-02). Прежде отсчитывалось 2000 символов от корня
    панели, и правило краснело от ЛЮБОЙ правки компонента, которая раздвигала
    разметку перед текстом вопроса: выражение клиентского состояния выросло на
    ветвь приземления фокуса — текст уехал за границу окна и «пропал», хотя в
    документе стои́т на месте. Отказ при этом выглядел как потеря текста, то
    есть указывал не на ту причину. Текст вопроса лежит ВНУТРИ формы панели
    (компонент печатает его первым узлом формы), поэтому границу задаёт сама
    форма — тем же разборщиком, что и у соседних правил файла.
    """
    user = await _current_user(db_session)
    log, *_ = await _seed_retryable_with_ids(db_session, user.id)

    html = (await authed_client.get("/history")).text
    panel = _retry_panel_form(html, log.id).lower()
