...
def _retry_panel_form(html: str, log_id: int) -> str:
    """Окно, на котором проверяется утверждение «панель несёт настоящую форму».
    ...
    по префиксу обрезал бы окно на СОБСТВЕННОМ заголовке панели, и форма снова
    оказалась бы за границей. Корень — цифры и закрывающая кавычка сразу за
    ними.
    """
    ...
    return html[start : following.start() if following else len(html)]
