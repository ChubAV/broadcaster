...
вход и выход, и каждый класс входов проверяется отдельно. Сквозной путь после
переезда загрузки в страничный слой (Фаза 12, D-01) РАЗДЕЛЁН НАДВОЕ, и обе
половины называются здесь, потому что ни одна не закрывает путь целиком:

* сервисная — ``tests/test_services/test_image_upload.py``: там и живёт теперь
  утверждение, что объектов в хранилище становится ДВА
  (``test_store_upload_stores_exactly_the_delivery_image_and_its_thumbnail``);
* страничная — ``tests/test_pages/test_ads_image_upload.py``: партия, фрагмент
  полосы, строки отказа и судьба уже прикреплённых ключей.

Здесь же — что именно лежит в объектах.

⚠️ ПРЕЖНИЙ АДРЕС НАЗВАН СЛОВАМИ, А НЕ ПУТЁМ (находка IN-01, план 12-10). Здесь
стоял путь в модуль тестов ПРЕЖНЕГО JSON-маршрута загрузки, снятый планом 12-05
ЦЕЛИКОМ. Это ИМЕНОВАННОЕ исключение из идиомы «прежняя редакция называется, а не
стирается»: приёмочный обход находки ищет снятое имя по дереву ``tests/``, и
дословно набранный путь удовлетворил бы обход сам — правило стало бы
самоопровергающимся.
