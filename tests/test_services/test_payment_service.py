...
async def test_a_second_subscription_intent_is_refused_before_the_money_moves(
    ...
    ⚠️ ИСТОЧНИК ОТКАЗА СМЕНИЛСЯ, ПРЕДМЕТ — НЕТ (план 08-05, D-06). Отказ теперь
    принимает СУБД: вставку второго незакрытого намерения отвергает частичный
    уникальный индекс `uq_payments_open_subscription_intent`, а прикладной
    проверки за этим тестом больше не стоит ни одной строкой. Тип отказа и число
    оставшихся строк от этого не изменились — точка сохранения откатывается, и
    мусорной строки вставка не оставляет.
    """
    user = await _user(db_session)
...
async def test_the_refusal_never_reaches_yookassa(db_session):
    ...
    ⚠️ УТВЕРЖДЕНИЕ СОХРАНЕНО ДОСЛОВНО И СТАЛО СИЛЬНЕЕ (план 08-05). Прежде оно
    держало порядок ПРИКЛАДНОЙ проверки; теперь отказ приходит от ограничения
    СХЕМЫ — и по-прежнему не доходит до SDK ни одним вызовом, потому что строка
    резервируется ДО обращения к ЮKassa (D-05). Ровно этого требовала
    формулировка PAY-01: второе намерение отвергается до того, как двинулись
    деньги.
    """
    user = await _user(db_session)
    await _open_intent(db_session, user, payment_id="yoo_first")
...
async def test_a_stale_intent_does_not_block_a_new_one(db_session):
    ...
    ⚠️ ПРЕДМЕТ ПРЕЖНИЙ, МЕХАНИЗМ НОВЫЙ, И РАЗНИЦА НЕ СЛОВЕСНАЯ (план 08-05, D-02).
    Раньше просроченное намерение просто НЕ СЧИТАЛОСЬ прикладной проверкой:
    строка оставалась `pending` навсегда, и у человека накапливались два
    оплачиваемых счёта на 3000 ₽ за один и тот же месяц доступа. Теперь
    просроченное намерение УБИРАЕТСЯ — ленивая уборка в начале создания платежа
    переводит его в `expired`, выводя из-под предиката индекса.

    ПОЭТОМУ УТВЕРЖДЕНИЕ О ДВУХ СТРОКАХ ОСТАЛОСЬ ВЕРНЫМ: уборка ГАСИТ строку, а не
    удаляет её. Обе строки по-прежнему оплачиваемы (`expired` не входит в
    `TERMINAL_STATUSES`), и заплативший по старой ссылке получит свой месяц.
    Изменилось то, что живым намерением старая строка больше не считается НИ ДЛЯ
    КОГО — ни для потолка, ни для экрана.

    ОСТАТОЧНОЕ ОКНО НАЗВАНО, А НЕ ЗАМОЛЧАНО, И ОНО ОДНО. Строка, просроченная и
    не тронутая уборкой — потому что её владелец за оплатой больше не приходил, —
    до следующей его попытки читается на экранах биллинга и админки как «в
    обработке». Это ЦЕНА РАЗМЕНА, принятая решением D-02: периодическая задача
    отвергнута (асинхронна и вводит второго писателя на денежный путь), отмена у
    ЮKassa — тоже (второй сетевой вызов, чей отказ снова запирает человека).
    Выход из состояния теперь ЕСТЬ и назван: следующая же попытка оплаты.
    """
...
async def test_a_canceled_intent_does_not_block_a_new_one(db_session):
    ...
    Отбор этот теперь ведёт не код, а ПРЕДИКАТ ИНДЕКСА (`status = 'pending'`), и
    утверждение проверяет ровно его частичность.
    """
    user = await _user(db_session)
    await _open_intent(
...
async def test_a_user_without_an_open_intent_pays_without_obstruction(db_session):
    ...
    и потолок, отвергающий ВСЁ, прошёл бы их все до одного. Под ограничением
    схемы граница снизу стала ещё нужнее: уникальный индекс, объявленный по
    пользователю ЦЕЛИКОМ вместо частичного, отверг бы и первую оплату тоже.
    """
    user = await _user(db_session)
...
async def test_an_intent_of_another_user_does_not_reach_over(db_session):
    ...
    Считает теперь СУБД: индекс объявлен по колонке `user_id`, и утверждение
    проверяет, что колонка в нём именно эта.
    """
...
async def test_a_terminal_payment_never_blocks_a_new_one(db_session):
    ...
    Источник отказа новый (предикат индекса `status = 'pending'`), предмет
    прежний: проведённый и отменённый платежи под предикат не попадают, и
    прикладной проверки за этим утверждением больше не стоит.
    """
    user = await _user(db_session)
    await _open_intent(
...
async def test_the_refusal_leaves_its_own_trace(db_session):
    ...
    такой величиной. Само же наличие следа стало ВАЖНЕЕ прежнего: отказ теперь
    приходит из недр СУБД, и без этой записи он был бы неотличим ни от отказа
    ЮKassa, ни от чужой поломки ограничения.
    """
    user = await _user(db_session)
    await _open_intent(db_session, user, payment_id="yoo_first")
