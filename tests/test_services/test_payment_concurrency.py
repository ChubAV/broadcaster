...
# --- Остаточная щель: ДВА ОПЛАЧИВАЕМЫХ платежа одного пользователя ------------
#
...
#
# ⚠️ ОТКУДА ТЕПЕРЬ БЕРЁТСЯ САМА ПАРА — ПЕРЕПИСАНО ПЛАНОМ 08-05 ВМЕСТЕ С
# МЕХАНИЗМОМ. Двух НЕЗАКРЫТЫХ подписочных намерений у одного человека больше не
# бывает: их запрещает второй частичный индекс,
# `uq_payments_open_subscription_intent`. Пара оплачиваемых строк возникает
# теперь ЕДИНСТВЕННЫМ способом — старшая строка уже ПОГАШЕНА (`expired`) ленивой
# уборкой либо отказом SDK, а младшая осталась `pending`. Оплачиваемости это не
# отнимает: `expired` не входит в `TERMINAL_STATUSES`, поэтому заявку выигрывают
# ОБЕ доставки, и щель остаётся ровно той же ширины, что была.


async def _seed_two_subscription_payments(factory) -> int:
    """Пользователь БЕЗ подписки и две его ОПЛАЧИВАЕМЫЕ строки платежа.

    ⚠️ СТАРШАЯ СТРОКА САЖАЕТСЯ `expired`, И ЭТО НЕ ПОДГОНКА ПОД ОГРАНИЧЕНИЕ, А
    ЕДИНСТВЕННАЯ ФОРМА, В КОТОРОЙ ПАРА ТЕПЕРЬ ВОЗНИКАЕТ В ПРОДУКТЕ. Прежде обе
    сажались `pending`; под потолком плана 08-05
    (`uq_payments_open_subscription_intent`) такая пара НЕВСТАВЛЯЕМА — второе
    незакрытое намерение отвергает СУБД. В `expired` строку переводит ленивая
    уборка просроченных намерений либо отказ платёжного SDK после резерва.

    ПРЕДМЕТ ПОТРЕБИТЕЛЯ ЭТИМ НЕ ОСЛАБЛЕН НИ НА БУКВУ: обе строки остаются
    ОПЛАЧИВАЕМЫМИ, потому что `expired` НЕ ВХОДИТ в `TERMINAL_STATUSES`, и
    `_claim_payment` выигрывает заявку на каждой. Два человеческих платежа —
    два сдвига срока, ровно как и раньше.
    """
    async with factory() as session:
