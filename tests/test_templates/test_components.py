...
# НЕЗАЯВЛЕННОЕ. Какое ОСНОВАНИЕ стои́т за числом три у заготовок — расширенное
# отступление владельца либо признанное нарушение критерия 3 — здесь НЕ
# записано и не предполагается: решение принимает владелец, и вносит его план
# 10-53 после останова на нём.
...
def _declared_script_handler_registrations() -> dict[str, int]:
    ...
    """
    # ⚠️ КРАСНАЯ ФАЗА: значение заготовок стои́т ЛИТЕРАЛОМ, равным ЗАМЕРУ БАЗЫ
    # ФАЗЫ (`7c1041d`): два обработчика, заведённых Фазой 8 (коммит `6d2eaf2`).
    # Это НЕ «неверное число, потом верное»: это единственный честный способ
    # ИЗМЕРИТЬ, что правило различает дерево ДО коммита `4e24d61` и дерево ПОСЛЕ
    # него. Обычная форма RED этой фазы — «правило красно на нетронутом дереве» —
    # здесь недостижима честно: покраснеть на настоящем дереве правило могло бы
    # только ОТКАТОМ третьего обработчика, а откат запрещён (правка закрывает
    # настоящий дефект `G-10-5`). Литерал снимается шагом GREEN.
    failure_banner_handlers = 2
    return {
        **SCRIPT_HANDLER_REGISTRATIONS_DECLARED_HERE,
        FAILURE_BANNER_TEMPLATE: failure_banner_handlers,
    }
...
def test_criterion_three_holds_by_the_numbers():
    ...
    ⚠️ ПЯТОЕ УТВЕРЖДЕНИЕ НЕ РЕШАЕТ, ПРИНЯТ ЛИ КРИТЕРИЙ 3 НАРУШЕННЫМ, И НЕ
    ПРЕДПОЛАГАЕТ НИ ОДНОГО ИЗ ДВУХ ОТВЕТОВ. Оно объявляет ЧИСЛО СЕГОДНЯШНЕГО
    ДЕРЕВА и ловит НЕЗАЯВЛЕННОЕ. Какое ОСНОВАНИЕ стои́т за числом три у
    заготовок — расширенное отступление владельца, называющее третий файл и
    третий обработчик поимённо, либо признанное нарушение критерия — вносит
    план 10-53 ПОСЛЕ останова на владельце. Машина не вправе выбрать за него ни
    одну из двух ветвей, ровно как верификатор не вправе толковать отступление
    шире, чем оно себя объявляет.
    """
