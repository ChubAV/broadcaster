...
# ⚠️ ФОРМА ПРОФИЛЯ БЕРЁТСЯ СБОРЩИКОМ САМОГО ОБРАБОТЧИКА, А НЕ ВТОРЫМ РЕНДЕРОМ
# ШАБЛОНА ПО ПУТИ (план 11-21). Второй экземпляр сборки разошёлся бы с первым
# молча: тест зеленел бы на разметке, которой обработчик не отдаёт. Та же
# доктрина единственного источника, по которой три числа критерия 3 ниже берутся
# импортом.
from app.constants import TIMEZONE_CHOICES
from app.pages.profile import _settings_markup
...
# --- ОБЛАСТЬ ПРАВИЛА «ИНДИКАТОР ВНЕ ПОТОКА» НА РЕНДЕРЕ (G-11-6, план 11-21) ---

WRAPPED_FORM_CLASS_MARKUP = 'class="form-wrapper"'
INDICATOR_NODE_MARKUP = '<span class="form-busy" aria-hidden="true"></span>'


def test_the_reported_profile_form_carries_the_indicator_scope():
    """Отчётная форма профиля рендерится В ОБЛАСТИ правила, а не только рядом.

    ⚠️ ПРЕДМЕТ — РЕНДЕР, А НЕ ИСХОДНИК МАКРОСА, И ЭТО НЕ ДУБЛИРОВАНИЕ СОСЕДНЕГО
    ГЕЙТА. Правило разметки читает ТЕКСТ шаблона обёртки; оно осталось бы зелёным
    и в том случае, если бы вызывающий печатал свою форму мимо макроса. Здесь
    форма собирается СБОРЩИКОМ САМОГО ОБРАБОТЧИКА (`_settings_markup`) — то есть
    ровно тем путём, которым разметка уезжает человеку.

    Форма профиля выбрана из трёх отчётных (профиль 21 px, мастер MAX 18,5 px,
    правка расписания 24 px) потому, что её замер снят диагнозом на живой
    странице, а не на впрыснутой разметке.

    Утверждаются три вещи: адрес формы тот же, класс области на месте, и узел
    индикатора лежит ВНУТРИ формы — вне её он не был бы потомком области, и
    правило стилей не нашло бы его ни при каком классе.
    """
    out = _settings_markup(selected=TIMEZONE_CHOICES[1][0])
    form = _form_tag(out)

    assert 'action="/profile"' in form, (
        f"форма профиля перестала быть настоящей формой POST на прежний адрес: "
        f"{form!r}"
    )
    assert WRAPPED_FORM_CLASS_MARKUP in form, (
        f"отрендеренная форма профиля не несёт класса области: правилу "
        f"«индикатор вне потока» нечего адресовать, и под кнопкой «Сохранить» "
        f"возвращается полоса 21 px — {form!r}"
    )

    at = out.index(INDICATOR_NODE_MARKUP)
    assert out.index(form) < at < out.index("</form>"), (
        "узел индикатора лежит вне формы: потомком области он не является, и "
        "правило стилей не найдёт его ни при каком классе формы"
    )


def test_the_panel_form_stays_outside_the_indicator_scope():
    """Форма панели подтверждения класса области НЕ несёт — и это на рендере.

    ⚠️ ПАРА К ПРАВИЛУ РАЗМЕТКИ, А НЕ ЕГО ПОВТОР. Гейт разметки читает ИСХОДНИК
    компонента; здесь панель РЕНДЕРИТСЯ, то есть утверждение снимается с той
    самой разметки, которая уезжает во все 18 мест подтверждения.

    Высота панели — ПРИНЯТОЕ состояние (отметка 3.5 обхода Фазы 10), выведенное
    из объёма исправления G-11-6 решением владельца от 2026-09-17. Узел
    индикатора при этом ОСТАЁТСЯ на месте: панель не лишается порога видимости,
    она лишается только чужой правки.
    """
    out = render("components/modal.html", "modal", **MODAL_ARGS)
    form = _form_tag(out)

    assert 'class="modal__form"' in form, (
        f"форма панели потеряла класс своей колонки — опору принятой вёрстки: "
        f"{form!r}"
    )
    assert WRAPPED_FORM_CLASS_MARKUP not in form, (
        f"форма панели получила класс области правила «индикатор вне потока»: "
        f"её индикатор уехал бы из потока, и высота изменилась бы разом во всех "
        f"18 местах подтверждения вопреки принятому состоянию — {form!r}"
    )
    assert "form-wrapper" not in form, (
        f"имя класса области встретилось в теге формы панели в другом написании: "
        f"{form!r}"
    )

    assert INDICATOR_NODE_MARKUP in out, (
        "панель перестала печатать узел индикатора: место подтверждения "
        "лишилось порога видимости"
    )
...
# ⚠️ ПЕРЕЧЕНЬ ПУСТ С ПЛАНА 11-18, И ПРАВИЛО ОТ ЭТОГО СТРОЖЕ, А НЕ БЕССМЫСЛЕННЕЕ.
# Абзац выше описывает дерево, на котором писался, и не стёрт. Задача 1 плана
# 11-18 перенесла обработчик дословно со страницы accounts/connect_max.html во
# включаемый шаг мастера MAX (перечень назвал новый файл), задача 2 сняла его
# вовсе: форма телефона перешла на макрос-обёртку, и защиту от двойной отправки
# несёт цель блокировки обёртки. Обход по-прежнему идёт по ВСЕМ шаблонам, и
# первый же встроенный обработчик отправки, где бы он ни появился, краснеет как
# незаявленный.
KNOWN_SUBMIT_HANDLER_FILES: frozenset[str] = frozenset()
...
# Коды отказа ВАРИАНТА ОШИБКИ, которые панель подтверждения СПОСОБНА выдать на
# пути перехода.
#
# ⚠️ ПРЕЖНЯЯ ЗАПИСЬ ПОЛУЧАЕТ ПОКОЛЕНИЕ, А НЕ ВЫЧЁРКИВАЕТСЯ. Здесь стояло: «Два
# кода… отказ перезапуска воркера в админке — единственное подтверждение фазы,
# чей обработчик уводит человека с кодом исхода». Верно для дерева Фазы 10.
# С плана 11-14 (D-10) кодом реестра уводит и снятие задачи из очереди: его
# отказ недоступности хранилища — третий код этого перечня, а два его отказа
# ВАРИАНТА ПРЕДУПРЕЖДЕНИЯ выписаны отдельным перечнем ниже — они едут В
# ПЛОЩАДКУ, и запись шапки области уведомлений называет это прямо.
PANEL_REFUSAL_CODES = (
    app_notices.WORKER_NO_CONTAINER,
    app_notices.WORKER_RESTART_FAILED,
    app_notices.QUEUE_DROP_UNAVAILABLE,
)

# Отказы панели снятия задачи, которые НЕ несут варианта ошибки и потому едут в
# вежливую область — то есть в площадку приземления (план 11-14). Перечень
# утверждается, а не заявляется: смени кто-то их вариант на ошибку, запись
# шапки снова стала бы шире поведения — в обратную сторону.
PANEL_POLITE_REFUSAL_CODES = (
    app_notices.QUEUE_DROP_MISSING,
    app_notices.QUEUE_DROP_NO_QUEUE,
)
...
def test_the_landing_region_never_receives_the_text_of_a_refusal():
    ...
    # Обратная сторона сужения плана 11-14: отказы-предупреждения панели снятия
    # задачи действительно НЕ несут варианта ошибки, как и говорит шапка.
    assert PANEL_POLITE_REFUSAL_CODES, "перечень вежливых отказов панели ПУСТ"
    for code in PANEL_POLITE_REFUSAL_CODES:
        record = app_notices.notice_for(code)
        assert record is not None, f"код {code!r} реестру НЕИЗВЕСТЕН"
        assert record.variant != "error", (
            f"код {code!r} несёт вариант 'error' — он поедет в соседнюю "
            "область, а шапка области уведомлений говорит, что он едет в "
...
def _strip_js_comments(source: str) -> str:
    ...
    ЛЕТОПИСЬ (Фаза 13, план 13-05): адрес мастера Telegram, приведённый выше
    предметом измерения, снят из дерева Фазой 13 вместе со сценарием мастера
    (план 13-01). Замер 2026-09-21 обходом `_script_bodies` над
    `app/templates/**/*.html`: блоков сценария 7, строк с двойной косой в их
    телах 80, и ВСЕ 80 — комментарии; после этой функции строк с двойной косой
    внутри строкового литерала НОЛЬ (синтетика `'https://example.org/p'` тот же
    обход переживает). Строкознающий разбор поэтому остаётся ЗАЩИТОЙ от первого
    адреса со схемой в теле сценария, а не отражением сегодняшнего замера;
    поведение функции не менялось.
...
#
# ЛЕТОПИСЬ ЧИСЛА `ads/form.html`: 8 → 5, Фаза 12, план 12-03. ДВИЖЕНИЕ ВНИЗ, и
# оно ПЕРВОГО рода, а не второго: регистрации УШЛИ ИЗ ДЕРЕВА вместе со своим
# предметом, разборщик их не терял. Ушли ровно три, и каждая названа:
#   1. слушатель `change` файлового поля — загрузка уехала на сервер формой
#      загрузки (`ads/includes/media_upload_form.html`), поля в этом файле нет;
#   2. слушатель `error` миниатюры в клиентски собранной плитке — плитки печатает
#      сервер, а откат на полноразмерный адрес несёт макрос `components/thumb.html`;
#   3. перехват `click` кнопки «×» — снят решением D-10: кнопка стала обычной
#      именованной кнопкой отправки формы объявления.
# ОСТАЛОСЬ ПЯТЬ, и они тоже названы: ввод в поле текста (счётчик символов) и
# четыре подписки формы на состояние запроса автосохранения.
# ⚠️ ЧИСЛО ПОСТАВЛЕНО ПРОГОНОМ ПОКРАСНЕВШЕГО ПРАВИЛА, А НЕ ВЫЧИТАНИЕМ В УМЕ.
# Вывод `test_criterion_three_holds_by_the_numbers` на дереве после задачи 1,
# дословно: «УТВЕРЖДЕНИЕ 5 ИЗ 5 (регистрации обработчиков в телах блоков
# `<script>`) НЕ СОШЛОСЬ. ads/form.html: найдено 5, объявлено 8».
# ⚠️ РЕШЕНИЯ ВЛАДЕЛЬЦА ЭТО ДВИЖЕНИЕ НЕ ТРЕБУЕТ, И ГРАНИЦА НАЗЫВАЕТСЯ: решение
# требуется на РОСТ (новая сущность в смысле D-13). Здесь сущностей стало
# МЕНЬШЕ, то есть критерий 3 соблюдён с запасом, а не отступление расширено.
