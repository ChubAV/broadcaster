...
def image_key(user_id: int, name: str = "photo.jpg") -> str:
    """Ключ вложения в форме, которую выдаёт приём загрузки изображения.

    Источник формы — сервис приёма загрузки в `app/services`:
    `{user_id}/{uuid4().hex}_{имя}`. Ключи в тестах строятся этим помощником, а
    не пишутся от руки: правило владения проверяет форму ключа, и произвольная
    строка ему не удовлетворяет.

    ⚠️ ССЫЛКА ОПИСАНА СЛОВАМИ, А НЕ НАБРАНА ПУТЁМ (Фаза 12, план 12-05). Прежде
    здесь стояли путь снятого модуля маршрута и его адрес; оба исчезли вместе с
    ним, а критерий приёмки считает вхождения пути в ТЕКСТЕ файлов суиты и
    комментарий от кода не отличает.
    """
