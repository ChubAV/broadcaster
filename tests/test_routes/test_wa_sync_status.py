...
async def test_sync_status_active_shows_groups(sync_setup):
    ...
    # «Групп» больше не стоит рядом с числом отдельным элементом — она
    # принадлежит шапке колонок и приходит в ячейку атрибутом. Утверждение
    # по-прежнему связывает подпись и число в одном элементе, то есть
    # покрытие не ослаблено: число «3» без своей подписи тест не пройдёт.
    assert re.search(r'title="Групп"[^>]*>\s*3\s*</span>', resp.text), resp.text
