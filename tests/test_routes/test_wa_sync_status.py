...
async def test_sync_status_active_shows_groups(sync_setup):
    ...
    # План 06: ответ опроса заменяет СТРОКУ списка целиком, поэтому подпись
    # «Групп» приходит в ячейку атрибутом подсказки.
    # План 11: и ОТДЕЛЬНЫМ элементом тоже — на 860px шапка колонок скрывается,
    # подсказка на касании недоступна, и число осталось бы без смысла. Оба
    # носителя подписи проверяются в одном утверждении вместе с числом: покрытие
    # не ослаблено, а усилено — число «3» без своей подписи тест не пройдёт ни
    # на широкой ширине, ни на узкой.
    assert re.search(
        r'title="Групп"[^>]*>\s*<span data-cell-label>Групп</span>\s*3\s*</span>',
        resp.text,
    ), resp.text
