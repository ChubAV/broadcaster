...
# Импортируется как `from tests.conftest import a_future_run_moment`.


def a_future_run_moment(days: int = 1) -> datetime:
    """Момент запуска, ОСТАЮЩИЙСЯ будущим при любом дне прогона (WR-08).

    ЗАЧЕМ ИМЯ И ЗАЧЕМ ОТНОСИТЕЛЬНОСТЬ. До этого помощника один и тот же литерал
    `datetime(2026, 9, 12, 6, 0, tzinfo=timezone.utc)` был вписан больше двадцати
    раз по дюжине модулей — без имени, то есть без единого места, где сказано,
    ЧТО он означает. А означал он «будущий слот»: части продукта относительны к
    «сейчас» — `app/application/admin/incidents.py` считает ПРОСРОЧЕННЫМИ строки
    с `next_run_at < beat_cutoff`, `upcoming_sends` сортирует по тому же полю и
    отбирает по `is_active`.

    ⚠️ ЦЕНА ЛИТЕРАЛА — ОТЛОЖЕННАЯ, А НЕ СЕГОДНЯШНЯЯ, И ИМЕННО ПОЭТОМУ ОПАСНАЯ.
    Дата стояла на сутки позже дня правки: пока прогон шёл раньше неё, фикстуры
    означали «будущий слот», а начиная со следующих суток — «просроченный». Ни
    один модуль на этом не краснел БЫ СРАЗУ, но смысл фикстуры менялся МОЛЧА, а
    молча меняющаяся фикстура есть отложенный ложный зелёный или ложный красный,
    который разбирать будут уже не как дефект фикстуры.

    `days` называется там, где нужны РАЗНЫЕ моменты у соседних строк: правило о
    «ближайшем запуске» на совпадающих моментах не отличило бы одну строку от
    другой.
    """
...
# --- Объявление маркеров прогона ----------------------------------------------
#
# ⚠️ МАРКЕР ОБЪЯВЛЯЕТСЯ ХУКОМ ЗДЕСЬ, А НЕ РАЗДЕЛОМ НАСТРОЕК В `pyproject.toml`, И
# ОСНОВАНИЕ ЗАПИСАНО РЯДОМ С ОБЪЯВЛЕНИЕМ. Раздела настроек прогона у проекта НЕТ
# ВОВСЕ: ни `[tool.pytest.ini_options]`, ни `pytest.ini`, ни `setup.cfg`, ни
# `tox.ini`. Его появление сместило бы КОРЕНЬ ПРОГОНА (пустой раздел в
# `pyproject.toml` делает файл якорем rootdir), то есть изменило бы поведение ВСЕЙ
# суиты — относительные пути в выводе, разрешение `conftest`, поиск плагинов — ради
# регистрации ОДНОГО имени. Цена несоразмерна предмету, поэтому имя регистрируется
# средствами уже установленного прогонщика.
#
# ⚠️ ВТОРОЕ ОСНОВАНИЕ — ГРАНИЦА ГОДНОСТИ МАРКЕРА, и она не подразумевается, а
# названа. Маркер заведён затем, чтобы правила записи можно было ПРОГНАТЬ И
# ДИАГНОСТИРОВАТЬ ПОРОЗНЬ от продуктовых, а НЕ затем, чтобы их не прогонять вовсе.
# `-m "not planning"` есть инструмент РАЗБОРА, а не разрешение выключить каталог:
# `tests/test_planning/` держит единственное машинное принуждение регламента отметок
# требований (планы 10-14, 10-17) и единственную защиту обхода от самозаверения
# (план 10-20), и их потеря стоит дороже любого неудобства. Добавление каталога в
